...
class ModelAbandonedCartsScript extends Model {
	public function getAbandonedCartsPeoples($data = array()) {
		...
		if(!empty($data['filter_days'])){
			$sql .= " AND DATE(date_added) = DATE_SUB(curdate(), INTERVAL ".$data['filter_days']." DAY)";
		}
	...
	public function getcart($cart_id) {
		$sql = "SELECT * FROM " . DB_PREFIX . "cart c LEFT JOIN " . DB_PREFIX . "abandonedcart ac ON (c.cart_id = ac.cart_id) WHERE c.cart_id = '" . $cart_id . "' AND c.customer_id = '0'";
		$query = $this->db->query($sql);
		return $query->row;
	}
	
	public function updateabandcart($newcart_id,$oldcart_id) {
		$this->db->query("UPDATE " . DB_PREFIX . "abandonedcart SET cart_id = '" . (int)$newcart_id . "' WHERE cart_id = '" . $oldcart_id . "'");
		$this->db->query("UPDATE " . DB_PREFIX . "abandonedcart_history SET cart_id = '" . (int)$newcart_id . "' WHERE cart_id = '" . $oldcart_id . "'");
	}
}