...
class ControllerKbetsyProduct extends Controller {
    ...
    public function index() {
        ...
        if ($this->config->get('kbetsy_demo_flag') == 0) {
            ...
            $this->load->model('kbetsy/shipping_profile');
            ...
            if (!empty($this->request->get['secure_key']) && $secure_key == $this->request->get['secure_key']) {
                ...
                $this->model_kbetsy_cron->auditLogEntry("Product Sync Started", $this->syncType);
                ...
                $this->model_kbetsy_cron->auditLogEntry("Product Sync Completed", $this->syncType);
    ...
    private function syncLocal() {
        ...
        if ($this->model_kbetsy_product->updateProductStatusByProfileStatus()) {
    ...
    private function prepareProductToList() {
        ...
        $this->model_kbetsy_product->syncProductsToModule();
    ...
    private function createListings() {
        ...
        $productsList = $this->model_kbetsy_product->getProductsToListOnEtsy($product_id, $this->syncLimit);
                  ...
                  //  print array_shift(array_keys($result));
                   ...
                   //  echo count($result)."MEssage".$result;
                    ...
                    if (count($result)==1){//(isset($result['error'])) 
                        ...
                        $this->model_kbetsy_cron->auditLogEntry(array_shift(array_keys($result)), $this->syncMethod);

                        $this->model_kbetsy_product->updateListingAddErrorStatus($product_id, array_shift(array_keys($result)));

          

                      //  $this->model_kbetsy_cron->auditLogEntry($result['error'], $this->syncMethod);

                       // $this->model_kbetsy_product->updateListingAddErrorStatus($product_id, $result['error']);

                    } elseif (isset($result['results'])) {

                        $this->model_kbetsy_product->updateListingAddStatus($result['results'][0], $product_id);
    ...
    private function updateListings() {
        ...
        $productsList = $this->model_kbetsy_product->getProductsToUpdateOnEtsy($product_id, $this->syncLimit);
    ...
    private function prepareArrayToSyncOnEtsy($product = array(), $language_id = '1') {
        ...
        $this->load->model('kbetsy/datatype');

        $occasions = $this->model_kbetsy_datatype->getDataType('Occasion');

        $recipients = $this->model_kbetsy_datatype->getDataType('Recipient');
        ...
        if (isset($product) && count($product) > 0 && !empty($language_id)) {
            ...
            //Get Profile Details
echo "allo";
            $profileDetails = $this->model_kbetsy_profile->getEtsyProfileDetails($product['id_etsy_profiles']);

            //Get Product Title & Description

            $productDetails = $this->model_kbetsy_product->getProductByProductId($product['id_product'], $language_id);
            ...
            if (!empty($productDetails)) {
                ...
                //echo "allo2";
                $shippingProfileID = $this->model_kbetsy_shipping_profile->getShippingProfileById($profileDetails['id_etsy_shipping_profiles']);
                ...
                if (!empty($shippingProfileID['shipping_profile_id']) && $shippingProfileID['shipping_profile_id'] != '0') {
                    ...
                    $default_currency = $this->model_kbetsy_product->getDefaultCurrency();
                    ...
                    if ($profileDetails['price_type'] == 1) {
                        ...
                        $price = $this->model_kbetsy_product->specialPrice($product['id_product']);
                    ...
                    if ($product['update_flag'] == 1 || $product['renew_flag'] == 1) {
                        ...
                        //$checkVariationExistence = $this->model_kbetsy_product->checkVariations($product['id_product']);
...
//        );
    ...
    private function prepareArrayToListVariationOnEtsy($product_id, $language_id = '') {
        ...
        $variations = $this->model_kbetsy_product->getVariations($product_id);

        if (!empty($variations)) {

            $product_details = $this->model_kbetsy_product->getProduct($product_id);

            $profile_details = $this->model_kbetsy_profile->getEtsyProfileByProduct($product_id);
            ...
            $variations_combinations = $this->model_kbetsy_product->get_combinations($productOptions);
            ...
            if ($variations_combinations) {
                ...
                foreach ($variations_combinations as $key_option_id => $variation) {
                    ...
                    if ($profile_details['price_type'] == 1) {
                        ...
                        $price = $this->model_kbetsy_product->specialPrice($product_id);
                    ...
                    foreach ($variation as $key_option_id => $option_value_id) {
                        ...
                        $option_details = $this->model_kbetsy_product->getAttributeMapping($product_id, $option_id, $option_value_id, $language_id);
                ...
                try {
                   ...
                   //print("<pre>".print_r ($result,true )."</pre>");
                    ...
                    if (count($result)==1) {
                        ...
                        $this->model_kbetsy_cron->auditLogEntry(array_shift(array_keys($result)) . ' (' . $listing['id_product'] . ') ' . __LINE__, $this->syncMethod);
                        ...
                        $variations = $this->model_kbetsy_product->getVariations($listing['id_product']);
                        ...
                        if (empty($variations)) {
                            ...
                            if (isset($getInventoryResult['error'])) {
                                ...
                                $this->model_kbetsy_cron->auditLogEntry($getInventoryResult['error'] . ' (' . $listing['id_product'] . ') ' . __LINE__, $this->syncMethod);
                                ...
                                if (isset($updateInventoryResult['error'])) {
                                    ...
                                    $this->model_kbetsy_cron->auditLogEntry($updateInventoryResult['error'], $this->syncMethod);
                    ...
                    $this->model_kbetsy_cron->auditLogEntry($e->getMessage(), $this->syncMethod);

                    $this->model_kbetsy_product->updateListingAddErrorStatus($listing['id_product'], $e->getMessage());
    ...
    private function getListingsStatus() {
        ...
        $productsList = $this->model_kbetsy_product->getProductsListedOnEtsy();
    ...
    private function syncImages($product_id, $listing_id) {
        ...
        $productImage = $this->model_kbetsy_product->getImagesByProductId($product_id);
        ...
        if (!empty($productImage['update'])) {
            ...
            foreach ($productImage['update'] as $prodImg) {
                ...
                if (isset($etsyResponse['error'])) {
                    ...
                    $this->model_kbetsy_cron->auditLogEntry($etsyResponse['error'], $this->syncMethod);

                    $this->model_kbetsy_product->updateListingAddErrorStatus($product_id, $etsyResponse['error']);
        ...
        if (!empty($productImage['delete'])) {
            ...
            foreach ($productImage['delete'] as $image_delete) {
                ...
                if (isset($delete_response['error'])) {
                    ...
                    $this->model_kbetsy_cron->auditLogEntry($delete_response['error'], $this->syncMethod);
    ...
    private function prepareTranslation($product_id, $listing_id) {
        ...
        if (!empty($sync_languages)) {
            ...
            foreach ($sync_languages as $sync_language) {
                ...
                $productDetails = $this->model_kbetsy_product->getProductByProductId($product_id, $sync_language['language_id']);
