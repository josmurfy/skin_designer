<?php
class ControllerExtensionFeedGoogleBaseCad extends Controller {
	public function index() {
		//if ($this->config->get('google_base_cad_status')) {
			...
			$this->load->model('extension/feed/google_base_cad');
			...
			$google_base_cad_categories = $this->model_extension_feed_google_base_cad->getCategories();

			foreach ($google_base_cad_categories as $google_base_cad_category) {
				$filter_data = array(
					'filter_category_id' => $google_base_cad_category['category_id'],
					'filter_filter'      => false
				);
				...
				foreach ($products as $product) {
					if (!in_array($product['product_id'], $product_data) && $product['description']&& $product['quantity']>0) {
						...
						$product['description'] = str_replace($str_toremove, ", ",$product['description']);
						$output .= '<description><![CDATA[' . preg_replace('/\(|\)\[|\]/','',strip_tags(html_entity_decode($product['description'], ENT_QUOTES, 'UTF-8'))) . ']]></description>';
						...
						if ($product['mpn']&& $product['mpn']!="" && $product['mpn']!="None") {
...
/* 						} */
						...
						if ((float)$product['special']) {
							$output .= '  <g:price>' .  /* $this->currency->format($this->tax->calculate( */number_format((float)$product['special']*1.34, 2, '.', '')/* , $product['tax_class_id']), $currency_code, $currency_value, false)  */. '</g:price>';
						} else {
							$output .= '  <g:price>' . /* $this->currency->format($this->tax->calculate( */number_format((float)$product['price']*1.34, 2, '.', '')/* , $product['tax_class_id']), $currency_code, $currency_value, false)  */. '</g:price>';
						}

						$output .= '  <g:google_product_category>' . $google_base_cad_category['google_base_category_id'] . '</g:google_product_category>';
