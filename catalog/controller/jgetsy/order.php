...
class ControllerJgetsyOrder extends Controller {
    ...
    public function index() {
        ...
        if ($this->config->get('jgetsy_demo_flag') == 0) {
            ...
            if (!empty($this->request->get['secure_key']) && $secure_key == $this->request->get['secure_key']) {
                ...
                $this->model_jgetsy_cron->auditLogEntry("Order Sync Started", $this->syncType);
                ...
                $this->model_jgetsy_cron->auditLogEntry("Order Sync Completed", $this->syncType);
            } else {
                       ...
                       //print("<pre>".print_r ($orderState,true )."</pre>");
                        ...
                        foreach ($receiptTransactionsList as $receiptTransactionList) {
                            ...
                            if ($productID->num_rows > 0) {
                                $productDetails = $this->model_jgetsy_product->getProductByProductId($productID->row['id_product'], $this->config->get('config_language_id'));
                        ...
                        $customer = $this->model_jgetsy_cron->getCustomerByEmail($receiptDetail['buyer_email']);
                        if ($customer) {
                        ...
                        $currency = $this->model_jgetsy_cron->getCurrencyByCode($receiptDetail['currency_code']);
        ...
        if (!empty($etsyCountryID)) {
            ...
            if ($countryDetail->num_rows > 0) {
                if (!empty($countryDetail->row['iso_code'])) {
                    $storeCountryID = $this->model_jgetsy_cron->getCountryByISOCode($countryDetail->row['iso_code']);
                } else {
                    if (!empty($countryDetail->row['country_name'])) {
                        $storeCountryID = $this->model_jgetsy_cron->getCountryByName($countryDetail->row['country_name']);
                    }
                }
            }
        }
    ...
    private function getStoreStateID($etsyState = '') {
        $storeStateID = 0;
        if (!empty($etsyState)) {
            $getStateByName = $this->model_jgetsy_cron->getStateByName($etsyState);
    ...
    private function writeOrderIntoDb($order) {
        $order_id = $this->model_jgetsy_order->createOrder($order);
        if ($order_id) {
            foreach ($order['products'] as $product) {
                $product_id = $this->model_jgetsy_order->createOrderProducts($product, $order_id);
                if ($product_id) {
                    foreach ($product['option'] as $option) {
                        $this->model_jgetsy_order->createOrderOptions($option, $order_id, $product_id);
                    }
                }
            }
            $this->model_jgetsy_order->addHistory($order['order_status_id'], $order_id);
            $this->model_jgetsy_order->insertOrderTotal($order, $order_id);
    ...
    public function syncOrderStatus() {
        ...
        if ($this->config->get('jgetsy_demo_flag') == 0) {
            ...
            if (!empty($this->request->get['secure_key']) && $secure_key == $this->request->get['secure_key']) {
                ...
                $this->load->model('jgetsy/cron');
                $this->load->model('jgetsy/order');
                $this->model_jgetsy_cron->auditLogEntry("Order Status Sync Started", $this->syncMethod);
                ...
                $this->model_jgetsy_cron->auditLogEntry("Order Status Sync Completed", $this->syncMethod);
                if ($this->syncError == true) {
                    echo "Success with some error(s). Refer to audit log for the details of the error.";
                } else {
                    echo "Success";
        ...
        if (!empty($lastDate) && !empty($paidStatus) && !empty($shippedStatus)) {
            ...
            if ($receipts->num_rows) {
                ...
                foreach ($receipts->rows as $receipt) {
                    ...
                    $reciptsUpdated++;
                    $this->model_jgetsy_cron->auditLogEntry("Order Status Update for Order ID:", $receipt['id_etsy_order']);
                }
                //Update date when orders status updated on Etsy Marketplaces
                $this->model_jgetsy_order->updateSyncStatusDate(date("Y-m-d H:i:s"));
            }
        }
        $this->model_jgetsy_cron->auditLogEntry("Total Order Status Sync:", $reciptsUpdated);
    }
