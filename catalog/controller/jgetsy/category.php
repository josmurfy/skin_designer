...
class ControllerJgetsyCategory extends Controller {
    ...
    public function index() {
        ...
        if ($this->config->get('jgetsy_demo_flag') == 0) {
            ...
            $this->model_jgetsy_cron->auditLogEntry("Category Sync Started", $this->syncType);
            //$this->syncOldCategory(0,0);
            $this->syncCategory(0);
            $this->model_jgetsy_cron->auditLogEntry("Category Sync Completed", $this->syncType);
    ...
    private function syncCategory($parent_id, $level = false, $data = array()) {
        ...
        if ($level == false) {
            ...
            $category_data = file_get_contents(DIR_SYSTEM . "library/jgetsy/category.json");
        ...
        foreach ($data as $category) {
            $category_inserted_id = $this->model_jgetsy_category->updateCategories($category['id'], $category['path'], $category['name'], $parent_id);
    ...
    private function syncFileUpdate() {
        ...
        file_put_contents(DIR_SYSTEM . "library/jgetsy/category.json", json_encode($result));
        die();
    }
    ...
    private function syncOldCategory($category_id, $parent_id = 0, $level = 0, $tag = "", $subtag = "") {
        ...
        if (isset($result['error'])) {
            $this->model_jgetsy_cron->auditLogEntry($result['error'], $this->syncType);
        } elseif (isset($result['results'])) {
            foreach ($result['results'] as $category) {
                $category_inserted_id = $this->model_jgetsy_category->updateCategories($category['category_id'], $category['name'], $category['short_name'], $parent_id);
                if ($category_inserted_id) {
                    if ($level == 0) {
