...
$(document).delegate('#button-cancel', 'click', function() {
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            if (json['success']) {
                $('#content').prepend('<div class="alert aler-success success squareup_spacer_10">' + json['success'] + '</div>');

                $('#button-cancel').hide();
            }

            if (json['error']) {
                $('#content').prepend('<div class="alert alert-warning warning text-danger squareup_spacer_10">' + json['error'] + '</div>');
            }
        },
