...
class ModelExport extends Model {
        ...
        if (!empty($order_data)) {
            ...
            foreach ($order_products as $product) {
                ...
                $products[] = array(
                    ...
                    'Name' => $product['name'],
                    'ImageUrl' => HTTP_CATALOG.$this->model_tool_image->resize($product['image'], $width, $height),
    ...
    public function getOrderProducts($order_id) {
        $query = $this->db->query("SELECT *, oP.quantity, op.price AS price FROM " . DB_PREFIX . "order_product op INNER JOIN " . DB_PREFIX . "product p ON op.product_id = p.product_id WHERE op.order_id = '" . (int) $order_id . "'");
        return $query->rows;
    }
