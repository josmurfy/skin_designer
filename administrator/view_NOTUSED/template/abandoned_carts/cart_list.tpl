				...
				<table class="table table-bordered table-hover">
						...
						<tr>
							<th class="text-center">
								...
								<span class="pull-left">
									&nbsp;&nbsp;&nbsp; &nbsp;<input type="checkbox" class="allcheckbox" onclick="$('.selected_data').prop('checked', this.checked);" />
								</span>
								<?php } ?>
							...
							<th class="text-left">Last Visit Page</th>
							<th class="text-left">Last Visit Time</th>
					...
					<tbody>
						...
						<tr>
										...
										<tbody>
											<tr>
												<td class="text-center" style="width: 1%;"> 
													<?php if ($abandoned_carts_email_send_status && $people['email']) { ?>
													<input type="checkbox" data-toggle="tooltip" title="<?php echo $text_email_reminder ?>" class="selected_data" name="selected_data[<?php echo $i; ?>][email]" value="<?php echo $people['email']; ?>" />
													<?php }else{ ?>
											...
											<tr>
												...
												<td><?php if($people['email']) { ?><a  href="mailto:<?php echo $people['email']; ?>"><?php echo $people['email']; ?></a><?php } else { ?><?php echo $text_not_available; ?><?php } ?></td>
											</tr>
											<tr>
							...
							<td><a target="_blank" href="<?php echo $people['lastvisit']; ?>"><?php echo $people['lastvisit']; ?></a></td>
							<td><?php echo $people['lastvisit_date']; ?></td>
							<?php if($abandoned_carts_deletecart_status) { ?>
...
$('button[id^=\'button-product-delete\']').on('click', function(e) {
	if (confirm('<?php echo $text_confirm; ?>')) {
		...
		$.ajax({
			url: 'index.php?route=abandoned_carts/abandonedcarts/deleteProdcut&token=<?php echo $token; ?>&cart_id=' + $(node).val(),
			dataType: 'json',
			crossDomain: true,
			beforeSend: function() {
				$(node).button('loading');
...
$('button[id^=\'button-cart-delete\']').on('click', function(e) {
	if (confirm('<?php echo $text_confirm; ?>')) {
		...
		$.ajax({
			url: 'index.php?route=abandoned_carts/abandonedcarts/deleteCart&token=<?php echo $token; ?>&email=' + $(node).val() + '&store_id=' + $(node).attr('rel'),
			dataType: 'json',
			crossDomain: true,
			beforeSend: function() {
				$(node).button('loading');
