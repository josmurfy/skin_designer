...
<script type="text/javascript"><!--
  ...
  function saveListingLink(id) {
      ...
      $.ajax({
          ...
          dataType: 'json',
          beforeSend: function() {
            $('#l-' + id + '-saveButton').html('<i class="fa fa-cog fa-lg fa-spin"></i>');
          },
          success: function(json) {
            $('#row' + id).fadeOut('slow');
            $('#l-' + id + '-saveButton').hide();
          },
          error: function (xhr, ajaxOptions, thrownError) {
            if (xhr.status != 0) { alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText); }
          }
      ...
      success: function(json) {
        if (json.data.items === null) {
          ...
          $.each (json.data.items, function(key, val) {
            ...
            html_inj += '<td class="text-center"><a class="btn btn-primary" style="display:none;" onclick="saveListingLink(' + key + '); return false;" id="l-' + key + '-saveButton"><span><?php echo $button_save; ?></span></a></td>';
            html_inj += '</tr>';
