...
function ChangeManufacturer(filter_name) {
    var token = document.querySelector('input[name="token"]').value;

    console.log('🔵 ChangeManufacturer started with filter_name:', filter_name);

    $.ajax({
        url: 'index.php?route=catalog/manufacturer/autocomplete&token=' + token + '&filter_name=' + encodeURIComponent(filter_name),
        dataType: 'json',
        success: function (json) {
            console.log('🟢 Ajax Success - Raw JSON:', json);
            ...
            var mapped = $.map(json, function (item) {
                return {
                    label: item['name'],
            ...
            console.log('🟠 Mapped manufacturers:', mapped);

            handleManufacturerResponse(mapped, filter_name);
        },
...
function handleManufacturerResponse(manufacturers, filter_name) {
    console.log('🟣 handleManufacturerResponse called with manufacturers:', manufacturers);

    if (manufacturers.length > 0) {
        // Cherche un vrai match sur le nom
        var selected = manufacturers.find(m => m.label.toLowerCase() === filter_name.toLowerCase());

        if (!selected) {
            console.log('🟤 No exact match found, selecting "None".');
            selected = manufacturers[0]; // Fallback sur "None"
        }

        console.log('🟤 Selected item after search:', selected);

        if (selected.value === 0) {
            console.log('⚪ Manufacturer "None" detected. Asking for confirmation.');
            if (confirm('Manufacturer not found. Do you want to add it?')) {
                console.log('⚡ User confirmed. Clicking #btn-add-manufacturer');
                $('#btn-add-manufacturer').click();
            } else {
                console.log('⛔ User cancelled. Filling fields with "None"');
                $('input[name="manufacturer"]').val(selected.label);
                $('input[name="manufacturer_id"]').val(selected.value);
            }
        } else {
            console.log('✅ Real manufacturer selected. Filling fields.');
            $('input[name="manufacturer"]').val(selected.label);
            $('input[name="manufacturer_id"]').val(selected.value);
        }
    } else {
        console.warn('⚠️ No manufacturers found.');
    }
}

function ChangeCategory(category_id) {
    var token = document.querySelector('input[name="token"]').value;
    var categoryId = category_id.trim(); // Corrected to use the passed parameter directly
            ...
            success: function(json) {
                if (!json.erreur) {
                    ...
                    if (category_id !== categoryId) {
                        // Uncomment if needed: generateProductSpecifics(categoryId);
                    }
                } else {
...
async function FeedProductForm() {
   // var productId = document.querySelector('input[name="product_id"]').value;
    ...
    return fetch(`index.php?route=shopmanager/catalog/product_search.product_source_info_feed_from_search&token=${token}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        .then(text => {
            try {
                // Essayer de convertir la réponse en JSON
                const data = JSON.parse(text);
                console.log('🟢 Ajax Success - Raw JSON:', data);
                if (searchResults) {
                    searchResults.innerHTML = data.html || '<p>MARDE.</p>';
                }
...
function attachFormListeners() {
    ...
    if (shippingCostInput) {
        ...
        console.log('✅ Event attached: input-shipping-cost');
    } else {
        console.warn('⚠️ input-shipping-cost not found');
    }
    ...
    if (priceWithShippingInput) {
        ...
        console.log('✅ Event attached: input-price-with-shipping');
    } else {
        console.warn('⚠️ input-price-with-shipping not found');
    }

    console.log('🟢 Done attaching all listeners.');
}
...
function SelectRow(checkbox) {
    ...
    function initializeSpecificHandlers() {
        ...
        if (!categoryIdInput) {
            console.error('Category ID input is missing.');
            return;
        }

        if (!categorySpecificNamesInput) {
            console.error('categorySpecificNamesInput Names JSON input is missing.');
            return;
        }
...
function checkFormStatus() {
    console.log('🔵 checkFormStatus triggered');

    var categoryInput = document.querySelector('input[name="category_id"]');
    console.log('🟢 category_id selected:', categoryInput.value);
    var categoryChecked = categoryInput && categoryInput.value !== '' && categoryInput.value !== '0' ? categoryInput : null;
    ...
    if (priceChecked) {
        console.log('🟢 Price checkbox selected:', priceChecked.id);

        var baseId = priceChecked.id.replace('condition_marketplace_item_id', '');

        var priceInput = document.querySelector(`#price_input_${baseId}`);
        var priceSpan = document.querySelector(`#price${baseId}`);

        if (priceInput) {
            const priceInputValue = parseFloat(priceInput.value.trim());
            console.log('🟠 Price input value:', priceInputValue);

            if (!isNaN(priceInputValue) && priceInputValue > 0) {
                isPriceValid = true;
                priceInput.style.border = '';
                console.log('✅ Price input is valid.');
            } else {
                isPriceValid = false;
                priceInput.style.border = '2px solid darkred';
                console.warn('⚠️ Price input invalid. Added red border.');
            }
        } else if (priceSpan) {
            const priceSpanValue = parseFloat(priceSpan.textContent.replace('$', '').trim());
            console.log('🟠 Price span value:', priceSpanValue);

            if (!isNaN(priceSpanValue) && priceSpanValue > 0) {
                isPriceValid = true;
                priceSpan.style.border = '';
                console.log('✅ Price span is valid.');
            } else {
                isPriceValid = false;
                priceSpan.style.border = '2px solid darkred';
                console.warn('⚠️ Price span invalid. Added red border.');
            }
        } else {
            console.error('🔴 No price input or span found for:', baseId);
        }
    } else {
        console.warn('⚠️ No price selected.');
    }
    ...
    saveButtons.forEach(button => {
        ...
        console.log(`  - Save button [${button.id || 'no id'}] ${button.disabled ? 'disabled' : 'enabled'}`);
    });
}
...
function updateCondition(conditionMarketplaceItemId) {
    console.log('🔵 updateCondition called with ID:', conditionMarketplaceItemId);
    ...
    // Trouver le prix lié à cette condition
    const baseId = conditionMarketplaceItemId;
    const priceInput = document.querySelector(`#price_input_${baseId}`);
    const priceSpan = document.querySelector(`#price${baseId}`);

    let priceInputValue = 0;

    if (priceInput) {
        priceInputValue = parseFloat(priceInput.value.trim());
        console.log('🟢 Found priceInput:', priceInputValue);
    } else if (priceSpan) {
        priceInputValue = parseFloat(priceSpan.textContent.replace('$', '').trim());
        console.log('🟠 Found priceSpan:', priceInputValue);
    } else {
        console.warn('⚠️ No price input or span found for baseId:', baseId);
    }
    ...
    if (targetRadio) {
        ...
        console.log('✅ Target radio checked:', targetRadio.id);
    } else {
        console.warn('⚠️ Target radio not found:', conditionMarketplaceItemId);
    }

    // Récupérer l'input #input-price-with-shipping
    const inputPriceWithShipping = document.getElementById('input-price-with-shipping');
    if (inputPriceWithShipping) {
        const currentValue = parseFloat(inputPriceWithShipping.value.trim()) || 0;
        if (currentValue <= 0 && priceInputValue > 0) {
            inputPriceWithShipping.value = priceInputValue.toFixed(2); // Fix 2 decimals
            console.log('🔵 Updated input-price-with-shipping to:', inputPriceWithShipping.value);
        } else {
            console.log('ℹ️ No update needed for input-price-with-shipping.');
        }
        updatePrice();
    } else {
        console.error('🔴 input-price-with-shipping not found.');
    }
}
   ...
   // Fonction qui met à jour le bouton radio save-data-radio basé sur la sélection de condition
    function updateConditionOLD(conditionMarketplaceItemId) {
        // Désélectionner tous les boutons radio save-data-radio
        const radios = document.querySelectorAll('.save-data-radio');
        const inputs = document.querySelectorAll('.price-input');
        var priceInput = document.querySelector(`#price_input_${baseId}`);
        ...
        var inputpricewithshipping = document.querySelectorAll('.price-input-with-shipping');
        radios.forEach(function(radio) {
            radio.checked = false; // Décocher tous les boutons
        });
        ...
        if (targetRadio) {
            ...
            if (inputpricewithshipping.values <=0) {
                inputpricewithshipping.values=priceInputValue;
            }
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!isNaN(price) && price > 0) {
        ...
        alert("Please enter a valid price or a valid mathematical expression.");
        ...
        xhr.onload = function () {
            if (xhr.status === 200) {
                ...
                if (response.success) {
                    ...
                    var priceInput = document.getElementById('input-price').value;
                    var shippingCostInput = document.getElementById('input-shipping-cost').value;
                    var priceWithShippingInput = document.getElementById('input-price-with-shipping').value;
                    if (priceWithShippingInput<=0 && priceInput>0 && shippingCostInput>0) {
                        updatePriceWithShipping();
                    } else if (priceInput==priceWithShippingInput && shippingCostInput>0) {
                        updatePrice();
                    } else if (priceInput<=0 && priceWithShippingInput>0 && shippingCostInput>0) {
                        updatePrice();
                    }
                } else {
...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".save-data-radio").forEach(radio => {
        radio.addEventListener("mousedown", function (event) {
            // Si le bouton radio est déjà sélectionné avant le clic
            if (this.checked) {
                this.wasChecked = true; // Stocker l'état précédent
            } else {
        ...
        radio.addEventListener("click", function (event) {
            // Si le bouton était déjà coché, alors on le désélectionne
            if (this.wasChecked) {
                this.checked = false;
            }
        });
