...
function updateQuantity(element,rel,quantity) {
    // Récupérer les valeurs de quantity et unallocated_quantity en convertissant en nombre
  //  var quantity = parseInt($('#' + element + rel).text()) || 0;
...
$(document).on('click','.pedit-quantity',function() {
    rel=$(this).attr('rel');
    rel1=$(this).attr('rel1');
  	 html ='<input type="text" id="quantity'+rel+'" name="quantity" class="form-control quantity'+rel+'" value="" />';
    $('#quantityedit'+rel).html(html);
	document.getElementById('quantity'+rel).autofocus = true
  });
     function checked_sku(sku) {
        ...
        // code that we will 'try' to run
			...
			//html='<i class="pquantity" rel="065935828921VG" rel1="0"> 0</i>
			html='<i  class="pquantity" rel="' +item+'" rel1="' + parseInt(qteval+1) +'"> ' + parseInt(qteval+1) +'</i>';
			$('#quantityedit' + item).html(html);
        } catch(error) {
	...
	function editQuantity(sku,quantity) {
		qteactuel=parseInt(document.getElementById('unallocated_quantity_hid' + sku).value);
		if(quantity==qteactuel){
            	document.getElementById('tr' + sku).style.backgroundColor='green';
			}else{
				document.getElementById('tr' + sku).style.backgroundColor='yellow';
			}
			
			document.getElementById('quantity_val' + sku).value=quantity;
 	
			//document.getElementById('quantitytab'+sku).style.backgroundColor='green';
			//document.getElementById('quantitytab'+sku).style.color='white';
