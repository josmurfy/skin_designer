...
$('input[name="category"]').on('change', function() {
    ...
    if ($.isNumeric(category_id)) {
      //  alert(category_id);
      loadCategoryDetails(category_id, token);
            ...
            fetch('index.php?route=shopmanager/ebay.getCategoryId&upc=' + encodeURIComponent(upc) + '&token=' + token)
                ...
                .then(json => {
                        ...
                        if (json.success) {
                            ...
                            console.log('Category ID from API:', category_id);
                            console.log('Current input category_id:', currentCategoryId);
                            ...
                            if (currentCategoryId != category_id) {
                                ...
                                console.log('Category ID is the same as the current input. No action taken.');
...
function ChangeCategory(category_id) {
    ...
    if ($.isNumeric(category_id)) {
        $.ajax({
            url: 'index.php?route=shopmanager/catalog/category.getCategoryDetails&category_id=' + encodeURIComponent(category_id) + '&token=' + token,
            dataType: 'json',
            success: function(json) {
                if (!json.erreur) {
...
function loadCategoryDetails(category_id, token) {
    $.ajax({
        url: 'index.php?route=shopmanager/catalog/category.getCategoryDetails&category_id=' + encodeURIComponent(category_id) + '&token=' + token,
        dataType: 'json',
        success: function(json) {
