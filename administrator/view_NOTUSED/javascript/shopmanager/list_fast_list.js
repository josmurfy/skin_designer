...
async function fastProductSearch(product_id, upc) {
    var token = document.querySelector('input[name="token"]').value;
    console.log('Load:', product_id);

    const url = 'index.php?route=shopmanager/catalog/product_search.product_source_info_fast_feed&view=fast_list&token=' + token;
    ...
    try {
        ...
        if (jsonResponse.html == 'category') {
            ...
            const url = `index.php?route=shopmanager/catalog/category.edit&token=${token}&category_id=${categoryId}`;
            window.open(url, 'category'); // Ouvrir dans une nouvelle fenêtre ou onglet
        }
