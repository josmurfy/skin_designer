...
class ModelSocialAutoPilotRequest extends Model {
	public function convertRequestInTasks($data) {
		...
		$filter_data = array(
			'filter_permission_list' => implode(",", $data['permission'])
		);

		$permissions = $this->model_social_autopilot_channel_permission->getPermissions($filter_data);

		if ($permissions) {
			$pages_data = array();

			foreach ($permissions as $permission_info) {
				$key = $permission_info['channel_id'];

				$pages_data[$key][] = array(
					'id'   => $permission_info['page_id'],
					'name' => $permission_info['name']
				);
			}

			if ($pages_data) {
				foreach ($pages_data as $channel_id => $channel_pages) {
					$tasks[$channel_id] = array(
						'request_uid'       => $request_uid,
						'channel_id'        => $channel_id,
						'pages_data'        => $channel_pages,
						'message'           => $data['message'],
						'link'              => rawurldecode($data['link']),
						'image'             => rawurldecode($data['image'])
					);
				}
			}
		}
	...
	public function getRequest($request_uid) {
		...
		if ($query->num_rows) {
			foreach($query->rows as $request) {
				$callback_url = HTTPS_CATALOG . 'index.php?route=social_autopilot/task&secret_key=' . $this->config->get('social_autopilot_api_key') . '&task_id=' . $request['task_id'];
