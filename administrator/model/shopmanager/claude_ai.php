...
class claude_ai extends \Opencart\System\Engine\Model {

    private $apiKey = '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';
    private $claudeApiKey = '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';
    ...
    private function sendOpenAiRequest($user_prompt, $systemPrompt, $maxTokens, $temperature, $output_json_needed = '') {
        ...
        $user_prompt .= "- The output **MUST** be a valid JSON array with double quotes around.  ";
        $user_prompt .= "- Use proper UTF-8 characters (é, à, ü, ñ, ç, etc.) not unicode escape sequences (\\uXXXX).  ";
        $user_prompt .= "- If the value includes double quotes or single quotes, escape them using backslashes like PHP's addslashes function (e.g., \\\" for double quotes and \\\' for single quotes).  ";
        $user_prompt .= "- DO NOT return any explanation, only the JSON array.  ";
        $user_prompt .= !empty($output_json_needed)?"- Return the result as a properly formatted JSON object like " . $output_json_needed:'';
        ...
        try {
            ...
            if ($e->hasResponse()) {
                ...
                $logFile = DIR_LOGS . 'claude_error.log';
                file_put_contents($logFile, date('Y-m-d H:i:s') . ' - Claude API Error Response: ' . $errorBody . "\n", FILE_APPEND);
            }
            $logFile = DIR_LOGS . 'claude_error.log';
            file_put_contents($logFile, date('Y-m-d H:i:s') . ' - Request failed: ' . $e->getMessage() . "\n", FILE_APPEND);
            throw new \Exception('Request to Claude failed: ' . $e->getMessage() . ($errorBody ? ' | Response: ' . substr($errorBody, 0, 200) : ''));
        } catch (\Exception $e) {
            $logFile = DIR_LOGS . 'claude_error.log';
    ...
    private function extractContent($responseData) {
        if (isset($responseData['content'][0]['text'])) {
            $content = trim($responseData['content'][0]['text']);
           // $content = preg_replace('/^```json\s*|\s*```$/', '', $content);
            ...
            if (is_array($decodedJson)) {
                // Clean up values: remove surrounding double quotes from string values
                $decodedJson = $this->cleanDoubleQuotesFromValues($decodedJson);
                return $decodedJson;
            } else {
                if (is_array($content)) {
                    return $content;
            ...
            throw new \Exception('❌ Aucun contenu trouvé dans la réponse Claude.');
        }
    }
    
    private function cleanDoubleQuotesFromValues($data) {
        if (is_array($data)) {
            foreach ($data as $key => $value) {
                if (is_string($value)) {
                    ...
                    // Decode unicode escape sequences (u2018, u00e9, etc.)
                    $value = preg_replace_callback('/u([0-9a-fA-F]{4})/', function($match) {
                        return mb_convert_encoding(pack('H*', $match[1]), 'UTF-8', 'UCS-2BE');
                    }, $value);
                    // Remove escaped line breaks (rn, \r\n, \\n, \\r\\n)
                    $value = str_replace(['\\r\\n', '\\n', '\\r', 'rn'], '', $value);
                    // Normalize whitespace around HTML tags
                    $value = preg_replace('/\s*(<li>)\s*/', '$1', $value);
                    $value = preg_replace('/\s*(<\/li>)\s*/', '$1', $value);
                    
                    $data[$key] = $value;
                } elseif (is_array($value)) {
                    $data[$key] = $this->cleanDoubleQuotesFromValues($value);
                }
            }
        }
        return $data;
    ...
    EOD;
        ...
        try {
            ...
            if (isset($response['content'][0]['text'])) {
                $fixed = trim($response['content'][0]['text']);
    ...
    private function repairInvalidJson($content) {
        ...
        if (isset($repaired['content'][0]['text'])) {
            $fixed = trim($repaired['content'][0]['text']);
        ...
        // NOTE: Image generation still uses OpenAI DALL-E (Claude doesn't have image generation)
        $openai_key = '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';
...
$user_prompt .= ". Here are the constraints for eBay category specifics: " . $specifics_text .
   ...
   // . ". Return only a valid JSON object with correct formatting and no additional text.";
      ...
      // Language name mapping for better translation quality
        ...
        $system_prompt = "You are an expert in JSON translation. Do not change any HTML tags if they exist. Return proper UTF-8 characters, not unicode escape sequences.";
        $user_prompt  = "Here is a JSON containing English values: " . $jsonData . 
                        ". Please translate it into " . ($languageNames[$target_language] ?? $target_language) . ". Use proper UTF-8 characters (é, à, ç, etc.) not escape codes (u00e9).  ";
...
public function countTokensOpenAI($json, $model = "claude-3-haiku-20240307") {
    // Claude uses approximate token counting: ~4 chars per token
    // For more accuracy, we estimate based on string length
    $text = is_string($json) ? $json : json_encode($json, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    $approxTokens = (int)ceil(strlen($text) / 4);
    
    // Add safety margin of 20% for system prompts
    return (int)($approxTokens * 1.2);
}
...
public function getTitle($titles, $category_id, $data = null) {
    ...
    // Limitation des tokens
    $max_tokens = min($this->countTokensOpenAI($user_prompt), 4096);
    //$max_tokens = 90;
    $temperature = 0.8;
  ...
  //  unset($product_info['category_id']);
    ...
    public function getDescriptionSupp($formdata, $title, $category_id) {
        ...
        if ($category_id == 617) {
            $system_prompt = "You are an expert HTML copywriter. Output ONLY formatted HTML with multiple sections, headings, lists, bold and italic tags throughout.";
            $user_prompt = "Create a product description for '".$title."' using THIS EXACT STRUCTURE:\n\n"
                . "<p>Opening paragraph with <strong>key details</strong> and <em>emotional words</em>. Include <strong>artist names</strong>, <strong>year</strong>, <strong>format</strong>, <strong>track count</strong>.</p>\n\n"
                . "<h4>What's Inside:</h4>\n"
                . "<ul>\n"
                . "<li><strong>Feature 1</strong> - description with <em>emphasis</em></li>\n"
                . "<li><strong>Feature 2</strong> - description</li>\n"
                . "<li><strong>Feature 3</strong> - description</li>\n"
                . "</ul>\n\n"
                . "<h4>Why You'll Love It:</h4>\n"
                . "<p>Benefits paragraph with <strong>multiple bold words</strong> and <em>emotional appeals</em>.</p>\n\n"
                . "<p>Closing paragraph encouraging purchase with <strong>more bold</strong> and <em>excitement</em>.</p>\n\n"
                . "CRITICAL: Output the HTML directly, not wrapped in quotes. Use extensive <strong> and <em> tags throughout.\n\n"
                . "Product details: " . json_encode($formdata, JSON_UNESCAPED_UNICODE);
        } elseif ($category_id == 139973) {
            $system_prompt = "You are an expert HTML copywriter. Output ONLY formatted HTML with multiple sections, headings, lists, bold and italic tags throughout.";
            $user_prompt = "Create a product description for '".$title."' using THIS EXACT STRUCTURE:\n\n"
                . "<p>Opening paragraph with <strong>game title</strong>, <strong>platform</strong>, and <em>exciting features</em>.</p>\n\n"
                . "<h4>Game Features:</h4>\n"
                . "<ul>\n"
                . "<li><strong>Feature 1</strong> - gameplay detail with <em>emphasis</em></li>\n"
                . "<li><strong>Feature 2</strong> - mechanic description</li>\n"
                . "<li><strong>Feature 3</strong> - unique aspect</li>\n"
                . "</ul>\n\n"
                . "<h4>Why Gamers Love It:</h4>\n"
                . "<p>Benefits with <strong>bold highlights</strong> and <em>exciting words</em>.</p>\n\n"
                . "<p>Closing with <strong>call to action</strong> and <em>enthusiasm</em>.</p>\n\n"
                . "CRITICAL: Output the HTML directly, not wrapped in quotes. Use extensive <strong> and <em> tags throughout.\n\n"
                . "Product details: " . json_encode($formdata, JSON_UNESCAPED_UNICODE);
        } else {
            $system_prompt = "You are an expert HTML copywriter. Output ONLY formatted HTML with multiple sections, headings, lists, bold and italic tags throughout.";
            $user_prompt = "Create a product description for '".$title."' using THIS EXACT STRUCTURE:\n\n"
                . "<p>Opening paragraph with <strong>product name</strong>, <strong>brand</strong>, and <em>key benefits</em>.</p>\n\n"
                . "<h4>Key Features:</h4>\n"
                . "<ul>\n"
                . "<li><strong>Feature 1</strong> - detailed description</li>\n"
                . "<li><strong>Feature 2</strong> - specification</li>\n"
                . "<li><strong>Feature 3</strong> - advantage</li>\n"
                . "</ul>\n\n"
                . "<h4>Benefits:</h4>\n"
                . "<p>Value proposition with <strong>highlights</strong> and <em>quality emphasis</em>.</p>\n\n"
                . "<p>Closing encouraging purchase with <strong>confidence</strong> and <em>reassurance</em>.</p>\n\n"
                . "CRITICAL: Output the HTML directly, not wrapped in quotes. Use extensive <strong> and <em> tags throughout.\n\n"
                . "Product details: " . json_encode($formdata, JSON_UNESCAPED_UNICODE);
        }
        $system_prompt .= " Do NOT wrap the HTML in JSON quotes - output raw HTML only.";
        $system_prompt .= " The description key in JSON should contain the raw HTML string.";
        ...
        // Retourne directement le HTML formaté
