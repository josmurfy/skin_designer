...
{country}';
		...
		if ($qry->num_rows) {
			foreach ($qry->rows as $row) {
				$data[$row['ebay_item_id']] = array(
					...
					'qty'           => $row['quantity'],
					'name'          => $row['name'],
					'link_edit'     => $this->url->link('catalog/product/edit', 'user_token=' . $this->session->data['token'] . '&product_id=' . $row['product_id'], true),
	...
	public function getCategorySpecifics($category_id) {
		$response['data']   = $this->openbay->ebay->call('listing/getCategorySpecifics/', array('id' => $category_id));
