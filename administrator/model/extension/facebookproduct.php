...
class ModelExtensionFacebookProduct extends Model {
  ...
  public function getFacebookProductAvailabiltyStatus($product_ids) {
    $sql = "SELECT p.product_id, fbp.facebook_product_id, " .
      ...
      "WHERE p.usa=1 and p.quantity>0 and p.price>0 and p.product_id IN (" . implode(",", $product_ids) . ")";
  ...
  public function getProducts($data = array()) {
    $sql = "SELECT p.*, " .
      ...
      "WHERE p.usa=1 and p.quantity>0 and p.price>0 and pd.language_id = '2'";
      ...
      "LEFT JOIN " .
        "(SELECT ptc.product_id, " .
          ...
          "LEFT JOIN " . DB_PREFIX . "category_description cd " .
            "ON (ptc.category_id = cd.category_id)) ptc " .
        "ON (p.product_id = ptc.product_id) " .
      "WHERE p.usa=1 and p.quantity>0 and p.price>0 and p.product_id = '" . (int)$product_id . "' " .
        "AND pd.language_id = 'fr'");
