...
class ModelAbandonedCartsAbandonedCarts extends Model {
	...
	public function getAbandonedCartsHistoriesPeoples($data = array()) {
		...
		$sql .= " WHERE abandonedcart_history_id > 0";
	...
	public function getAbandonedCartsProducts($data = array()) {
		...
		$sql .= " WHERE abandonedcart_id > 0 AND LCASE(email) = '" . $this->db->escape(utf8_strtolower($data['filter_email'])) . "'";
	...
	public function getAbandonedCartsHistoriesProducts($data = array()) {
		...
		$sql .= " WHERE abandonedcart_history_id > 0 AND LCASE(email) = '" . $this->db->escape(utf8_strtolower($data['filter_email'])) . "'";
					...
					if(!empty($result_value['email']) && isset($result_value['store_id'])) {
						...
						if($customer_info) {
							...
							foreach($products as $product) {
								$cart_page_link = HTTP_CATALOG.'index.php?route=checkout/cart&cart_id='.$product['cart_id'];
							...
							$stores = $this->model_setting_store->getStore($result_value['store_id']);
							if(!$stores){
								$stores =  array(
							...
							$replace = array(
								...
								'logo' 			 => '<img src="'. HTTP_CATALOG .'image/'. $this->config->get('config_image') .'" title="'. $stores['name'] .'" alt="'. $stores['name'] .'" />',
