...
class ListFast extends \Opencart\System\Engine\Model {
    ...
    public function __construct($registry) {
        ...
        // Chargez la valeur de HTTPS_CATALOG depuis la configuration
        ...
        public function getProduct($product_id) {
            ...
            WHERE (p.marketplace_item_id = 0 OR p.marketplace_item_id is NULL) AND (p.quantity+unallocated_quantity >0) p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");//, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'product_id=" . (int)$product_id . "') AS keyword
        ...
        public function getProducts($data = array()) {
            ...
            ptc.category_id, 
            pd.name,
            IF(pis.epid IS NOT NULL, pis.epid, '0') AS epid ,
            ...
            WHERE (p.marketplace_item_id = 0 OR p.marketplace_item_id is NULL) AND (p.quantity+unallocated_quantity >0) 
            ...
            if (!empty($data['filter_epid'])) {
                $sql .= " AND pis.epid = '" . $this->db->escape($data['filter_epid']) . "'";
            }
    	...
    	public function getTotalProducts($data = array()) {
            ...
            $sql	.= " WHERE (p.marketplace_item_id = 0 OR p.marketplace_item_id is NULL) AND (p.quantity+unallocated_quantity >0)  AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
            ...
            if (!empty($data['filter_epid'])) {
                $sql .= " AND pis.epid LIKE '" . $this->db->escape($data['filter_epid']) . "%'";
            }
