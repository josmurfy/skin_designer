...
class Country extends \Opencart\System\Engine\Model {
	 ...
	 *
	 * Legacy compatibility helper used by ShopManager code.
	 *
	 * @param string $name
	 *
	 * @return int
	...
	public function getCountryID(string $name): int {
		$query = $this->db->query("SELECT `c`.`country_id` FROM `" . DB_PREFIX . "country` `c` LEFT JOIN `" . DB_PREFIX . "country_description` `cd` ON (`c`.`country_id` = `cd`.`country_id`) WHERE LCASE(`cd`.`name`) = '" . $this->db->escape(oc_strtolower($name)) . "' AND `cd`.`language_id` = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");

		return (int)($query->row['country_id'] ?? 0);
	}
