 ...
 */
include_once(DIR_SYSTEM . 'library/jgetsy/JgOAuth.php');

class ControllerExtensionModuleJgetsy extends Controller
{
    ...
    public function __construct($registry)
    {
        ...
        if (VERSION <= '2.2.0') {
            $this->module_path = 'module/jgetsy';
        } else {
            $this->module_path = 'extension/module/jgetsy';
        }
    }
    ...
    public function generalSettings()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateGeneralSettings()) {
            if($this->demo_flag == 0) {
                ...
                if ($this->request->post['etsy']['general']['enable'] == 0) {
                    $enable_status['module_jgetsy_status'] = '0';
                } else {
                    $enable_status['module_jgetsy_status'] = '1';
                }
                $this->model_setting_setting->editSetting('module_jgetsy', $enable_status);                
                unset($this->request->post['etsy']);
                $this->model_jgetsy_jgetsy->editSetting('etsy_general_settings', $this->request->post);
                $this->model_jgetsy_jgetsy->editSetting('etsy_order_settings', $this->request->post);
        ...
        if ($this->model_jgetsy_jgetsy->getSetting('etsy_general_settings')) {
            $settings = $this->model_jgetsy_jgetsy->getSetting('etsy_general_settings');
            $order_settings = $this->model_jgetsy_jgetsy->getSetting('etsy_order_settings');
        ...
        if (isset($this->request->get['oauth_verifier'])) {
            try {
                ...
                if (empty($etsyRequestData)) {
                    ...
                    $this->model_jgetsy_jgetsy->editSetting('etsy_access_token', array('etsy_access_token' => $response->access_token));
                    $this->model_jgetsy_jgetsy->editSetting('etsy_access_token_secret', array('etsy_access_token_secret' => $response->access_token_secret));
        ...
        if (isset($this->request->get['action']) && $this->request->get['action'] == 'disconnect') {
            $this->model_jgetsy_jgetsy->deleteSetting('etsy_access_token');
            $this->model_jgetsy_jgetsy->deleteSetting('etsy_access_token_secret');
    ...
    public function profileManagement()
    {
        ...
        $this->load->model('setting/setting');
        $this->load->model('jgetsy/jgetsy');
        $this->load->model('jgetsy/attribute');

        $this->document->setTitle($this->language->get('heading_title_main'));

        /** Check if all the attributes are mapped */
        $all_options_mapped = true;
        $options = $this->model_jgetsy_attribute->getEtsyOCOptionsMapping();
        $optionIdArray = array();
        if (!empty($options)) {
        ...
        if (!empty($optionIdArray)) {
            $optionMappings = $this->model_jgetsy_attribute->getOptionsName($optionIdArray);
        ...
        $data['sort_shipping_profile_title'] = $this->url->link($this->module_path . '/profileManagement', $this->session_token_key . '=' . $this->session_token . '&sort=st.shipping_profile_title' . $sort_url, true);
        ...
        $profile_total = $this->model_jgetsy_jgetsy->getProfileTotal($filter_data);
        $profile_result = $this->model_jgetsy_jgetsy->getProfileDetails($filter_data);

        $data['profiles'] = array();
        foreach ($profile_result as $result) {
            $category_details = $this->model_jgetsy_jgetsy->getEtsyCategoryByCode($result['etsy_category_code']);
            ...
            $data['profiles'][] = array(
                ...
                'shipping_profile_title' => $result['shipping_profile_title'],
    ...
    public function profileUpdate()
    {
        ...
        $this->load->model('localisation/currency');
        $this->load->model('jgetsy/category');
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            $filter_data = array();
            $profile_details = $this->model_jgetsy_jgetsy->getProfileDetails($filter_data, $this->request->get['id_etsy_profiles']);
            $data['etsy'] = array();
            $data['profile_title'] = $profile_details[0]['profile_title'];
            $data['etsy_category'] = $profile_details[0]['etsy_category_code'];
            $data['etsy_templates'] = $profile_details[0]['id_etsy_shipping_profiles'];
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if (isset($this->request->post['id_etsy_profiles'])) {
                $this->model_jgetsy_profile->editProfile($this->request->post);
                $this->session->data['success'] = $this->language->get('etsy_profile_update_success');
            } else {
                $this->session->data['success'] = $this->language->get('etsy_profile_update_success');
                $this->model_jgetsy_profile->addProfile($this->request->post);
            }
        ...
        $data['etsy_categories'] = $this->model_jgetsy_category->getEtsyCategories();
        $data['etsy_ship_templates'] = $this->model_jgetsy_jgetsy->getShipTemplates();
        $data['etsy_shop_sections'] = $this->model_jgetsy_shop_section->getShopSections(array());
        ...
        $data['store_attributes'] = $this->model_jgetsy_jgetsy->getAttributes();
        ...
        $shippingProfile = $this->model_jgetsy_jgetsy->getShipTemplates();
        if (!empty($shippingProfile)) {
            $data['shipping_profile'] = true;
        } else {
            $data['shipping_profile'] = false;
        }
        $data['shipping_profile_not_avaliable_error'] = sprintf($this->language->get('shipping_profile_not_avaliable_error'), $this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token, 'SSL'));
        $data['category_not_avaliable_error'] = sprintf($this->language->get('category_not_avaliable_error'), HTTPS_CATALOG . 'index.php?route=jgetsy/category');
   ...
   public function displayPropertiesList($properties = array())
   {
    ...
    public function getSubcategories()
    {
        ...
        if (isset($this->request->get['category_id'])) {
            ...
            $data = $this->model_jgetsy_category->getEtsyCategories($category_id);
            if($data == false) {
    ...
    public function shopSection()
    {
        ...
        $shop_section_total = $this->model_jgetsy_shop_section->getShopSectionTotal($filter_data);
        $shop_sections = $this->model_jgetsy_shop_section->getShopSections($filter_data);
        ...
        $data['sync_shop_section_url'] = HTTPS_CATALOG . 'index.php?route=jgetsy/shop_section';
    ...
    public function shopSectionUpdate()
    {
        ...
        if (isset($this->request->get['shop_section_id'])) {
            ...
            $shop_section_details = $this->model_jgetsy_shop_section->getShopSections($filter_data, $this->request->get['shop_section_id']);
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if (isset($this->request->post['shop_section_id']) && $this->request->post['shop_section_id'] != "") {
                $this->model_jgetsy_shop_section->shopSectionUpdate($this->request->post);
    ...
    public function deleteShopSection()
    {
        ...
        if (!empty($this->request->get['shop_section_id'])) {
            if($this->demo_flag == 0) {            
                $this->model_jgetsy_shop_section->deleteShopSection($this->request->get['shop_section_id']);
    ...
    public function shippingProfiles()
    {
        ...
        if (isset($this->request->get['sort'])) {
            ...
            $sort = 'id_etsy_shipping_profiles';
        }
        if (isset($this->request->get['order'])) {
            $order = $this->request->get['order'];
        } else {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token . $filter_url, 'SSL'),
            'separator' => ' :: '
        );
        ...
        $this->document->setTitle($this->language->get('heading_title_main'));
        $data['sort_id_etsy_shipping_profiles'] = $this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token . '&sort=id_etsy_shipping_profiles' . $sort_url, true);
        $data['sort_shipping_profile_title'] = $this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token . '&sort=shipping_profile_title' . $sort_url, true);
        $data['sort_shipping_origin_country'] = $this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token . '&sort=shipping_origin_country' . $sort_url, true);
        $data['sort_shipping_min_process_days'] = $this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token . '&sort=shipping_min_process_days' . $sort_url, true);
        $data['sort_shipping_max_process_days'] = $this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token . '&sort=shipping_max_process_days' . $sort_url, true);
        
        $shipping_total = $this->model_jgetsy_jgetsy->getShippingTemplateTotal($filter_data);
        $shipping_profiles_details = $this->model_jgetsy_jgetsy->getShippingTemplateDetails($filter_data);
        
        $data['shipping_profiles'] = array();
        foreach ($shipping_profiles_details as $result) {
            $data['shipping_profiles'][] = array(
                'id_etsy_shipping_profiles' => $result['id_etsy_shipping_profiles'],
                'shipping_profile_title' => $result['shipping_profile_title'],
                ...
                'add' => $this->url->link($this->module_path . '/shippingProfileEntryUpdate', $this->session_token_key . '=' . $this->session_token . "&id_etsy_shipping_profiles=" . $result['id_etsy_shipping_profiles'] . $url, true),
                'view' => $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . "&id_etsy_shipping_profiles=" . $result['id_etsy_shipping_profiles'] . $url, true),
                'edit' => $this->url->link($this->module_path . '/shippingProfileUpdate', $this->session_token_key . '=' . $this->session_token . "&id_etsy_shipping_profiles=" . $result['id_etsy_shipping_profiles'] . $url, true),
                'delete' => $this->url->link($this->module_path . '/deleteShippingTemplates', $this->session_token_key . '=' . $this->session_token . "&id_etsy_shipping_profiles=" . $result['id_etsy_shipping_profiles'] . $url, true)
            );
        }
        ...
        $pagination->page = $page;
        $pagination->limit = $this->config->get('config_limit_admin');
        $pagination->url = $this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token . $url . '&page={page}', true);
        ...
        //Links
        $data['general_settings'] = $this->url->link($this->module_path . '/generalSettings', $this->session_token_key . '=' . $this->session_token, 'SSL');
        $data['action'] = $this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token, 'SSL');
        $data['route'] = $this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token, 'SSL');
        $data['cancel'] = $this->url->link($this->module_path, $this->session_token_key . '=' . $this->session_token, 'SSL');
        $data['add'] = $this->url->link($this->module_path . '/shippingProfileUpdate', $this->session_token_key . '=' . $this->session_token . $url, true);
        $data['delete'] = $this->url->link($this->module_path . '/deleteShippingTemplates', $this->session_token_key . '=' . $this->session_token . $url, true);
        $data['sync_template_url'] = HTTPS_CATALOG . 'index.php?route=jgetsy/template';
        ...
        $data['column_action'] = $this->language->get('column_action');
        $data['text_sync_shipping_profile'] = $this->language->get('text_sync_shipping_profile');
        $data['text_no_shipping_profile_error'] = sprintf($this->language->get('text_no_shipping_profile_error'), $data['sync_template_url']);
        $data['button_add'] = $this->language->get('button_add');
        ...
        $data['etsycountires'] = $this->model_jgetsy_jgetsy->getEtsyCountriesCount();
        $data['text_sync_now'] = $this->language->get('text_sync_now');
        $data['etsy_country_sync_url'] = HTTPS_CATALOG . 'index.php?route=jgetsy/country';
    ...
    public function shippingProfileUpdate()
    {
        ...
        if (isset($this->request->get['id_etsy_shipping_profiles'])) {
            $filter_data = array();
            $data['etsy'] = array();

            $shipping_profiles_details = $this->model_jgetsy_jgetsy->getShippingTemplateDetails($filter_data, $this->request->get['id_etsy_shipping_profiles']);
            if (!empty($shipping_profiles_details)) {
                $data['etsy'] = array(
                    'template' => array(
                        'template_title' => $shipping_profiles_details[0]['shipping_profile_title'],
                        'primary_cost' => $shipping_profiles_details[0]['shipping_primary_cost'],
                        'secondary_cost' => $shipping_profiles_details[0]['shipping_secondary_cost'],
                        'min_process_days' => $shipping_profiles_details[0]['shipping_min_process_days'],
                        'max_process_days' => $shipping_profiles_details[0]['shipping_max_process_days'],
                    )
                );
                $data['etsy_select_country'] = $shipping_profiles_details[0]['shipping_origin_country_id'];
                $data['id_etsy_shipping_profiles'] = $shipping_profiles_details[0]['id_etsy_shipping_profiles'];
            }
        } else {
            $data['etsy_select_country'] = 0;
            $data['id_etsy_shipping_profiles'] = "";
        }

        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if (isset($this->request->post['id_etsy_shipping_profiles']) && $this->request->post['id_etsy_shipping_profiles'] != "") {
                $this->model_jgetsy_jgetsy->shippingProfileUpdate($this->request->post);
            ...
            $this->response->redirect($this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token, 'SSL'));
        }
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title_shipping'),
            'href' => $this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token, 'SSL'),
            'separator' => ' :: '
        );
        ...
        if (isset($this->request->get['id_etsy_shipping_profiles'])) {
            ...
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->url->link($this->module_path . '/shippingProfileUpdate&id_etsy_shipping_profiles=' . $this->request->get['id_etsy_shipping_profiles'], $this->session_token_key . '=' . $this->session_token, 'SSL'),
                'separator' => ' :: '
            );
        } else {
            ...
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->url->link($this->module_path . '/shippingProfileUpdate', $this->session_token_key . '=' . $this->session_token, 'SSL'),
                'separator' => ' :: '
            );
        }
        ...
        //links
        if (isset($this->request->get['id_etsy_shipping_profiles'])) {
            $data['action'] = $this->url->link($this->module_path . '/shippingProfileUpdate', 'id_etsy_shipping_profiles=' . $this->request->get['id_etsy_shipping_profiles'] . '&' . $this->session_token_key . '=' . $this->session_token, 'SSL');
        } else {
            $data['action'] = $this->url->link($this->module_path . '/shippingProfileUpdate', $this->session_token_key . '=' . $this->session_token, 'SSL');
        }

        $data['route'] = $this->url->link($this->module_path . '/shippingProfileUpdate', $this->session_token_key . '=' . $this->session_token, 'SSL');
        $data['cancel'] = $this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token, 'SSL');
        ...
        $data['text_etsy_api_secret_tooltip'] = $this->language->get('text_etsy_api_secret_tooltip');
        $data['text_etsy_api_host_type_tooltip'] = $this->language->get('text_etsy_api_host_type_tooltip');
        $data['text_edit_shipping_profile_add'] = $this->language->get('text_edit_shipping_profile_add');
        ...
        $data['etsy_countries'] = $this->model_jgetsy_jgetsy->getEtsyCountries();
        ...
        if (VERSION >= '2.2.0.0') {
            $this->response->setOutput($this->load->view($this->module_path . '/shipping_profile_form', $data));
        } else {
            $this->response->setOutput($this->load->view($this->module_path . '/shipping_profile_form.tpl', $data));
        }
    }
    ...
    {
        $this->load->language($this->module_path);
        if (!empty($this->request->get['id_etsy_shipping_profiles'])) {
            if($this->demo_flag == 0) {
                $this->load->model('jgetsy/jgetsy');
                $id_etsy_shipping_profiles = $this->request->get['id_etsy_shipping_profiles'];
                if ($this->model_jgetsy_jgetsy->checkProfileMapping($id_etsy_shipping_profiles)) {
                    $this->model_jgetsy_jgetsy->deleteShippingTemplate($id_etsy_shipping_profiles);
                    ...
                    $this->session->data['error'] = $this->language->get('error_shipping_profile_already_mapped');
                }
            } else {
                $this->session->data['error'] = $this->language->get('text_demo_mode_action');
            }
    ...
    public function shippingProfileEntries()
    {
        if (isset($this->request->get['filter_profile_name'])) {
            $filter_profile_name = $this->request->get['filter_profile_name'];
        } else {
        ...
        if (isset($this->request->get['id_etsy_shipping_profiles'])) {
            $id_etsy_shipping_profiles = $this->request->get['id_etsy_shipping_profiles'];
            $shippingProfileData = $this->model_jgetsy_jgetsy->getShippingTemplateById($id_etsy_shipping_profiles);
            if ($shippingProfileData == false) {
                $this->response->redirect($this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token, 'SSL'));
            }
        } else {
            /* Redirect to Etsy Shippin Template page if id_etsy_shipping_profiles is not avaliable on URL */
            $this->response->redirect($this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token, 'SSL'));
        }
        if (isset($this->request->get['sort'])) {
            $sort = $this->request->get['sort'];
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . '&id_etsy_shipping_profiles=' . $this->request->get['id_etsy_shipping_profiles'] . $url, 'SSL'),
            'separator' => ' :: '
        );
        ...
        $filter_data = array(
            ...
            'id_etsy_shipping_profiles' => $id_etsy_shipping_profiles,
            'sort' => $sort,
            'order' => $order,
            'start' => ($page - 1) * $this->config->get('config_limit_admin'),
            'limit' => $this->config->get('config_limit_admin')
        ...
        $data['sort_id_etsy_shipping_profiles'] = $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . '&sort=id_etsy_shipping_profiles&id_etsy_shipping_profiles=' . $this->request->get['id_etsy_shipping_profiles'] . $url, true);
        $data['sort_shipping_profile_title'] = $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . '&sort=shipping_profile_title&id_etsy_shipping_profiles=' . $this->request->get['id_etsy_shipping_profiles'] . $url, true);
        $data['sort_shipping_origin_country'] = $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . '&sort=shipping_origin_country&id_etsy_shipping_profiles=' . $this->request->get['id_etsy_shipping_profiles'] . $url, true);
        $data['sort_shipping_entry_destination_country'] = $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . '&sort=shipping_entry_destination_country&id_etsy_shipping_profiles=' . $this->request->get['id_etsy_shipping_profiles'] . $url, true);
        $data['sort_shipping_entry_destination_region'] = $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . '&sort=shipping_entry_destination_region&id_etsy_shipping_profiles=' . $this->request->get['id_etsy_shipping_profiles'] . $url, true);
        $data['sort_shipping_entry_primary_cost'] = $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . '&sort=shipping_entry_primary_cost&id_etsy_shipping_profiles=' . $this->request->get['id_etsy_shipping_profiles'] . $url, true);
        $data['sort_shipping_entry_secondary_cost'] = $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . '&sort=shipping_entry_secondary_cost&id_etsy_shipping_profiles=' . $this->request->get['id_etsy_shipping_profiles'] . $url, true);

        $shipping_total = $this->model_jgetsy_jgetsy->getTotalShippingTemplateEntries($filter_data);
        $shipping_profiles_details = $this->model_jgetsy_jgetsy->getShippingTemplateEntries($filter_data);

        $data['shipping_profile_entries'] = array();
        foreach ($shipping_profiles_details as $result) {
            $data['shipping_profile_entries'][] = array(
                'id_etsy_shipping_profiles_entries' => $result['id_etsy_shipping_profiles_entries'],
                'shipping_profile_title' => $result['shipping_profile_title'],
                'shipping_origin_country' => $result['shipping_origin_country'],
                'shipping_profile_title' => $result['shipping_profile_title'],
                ...
                'edit' => $this->url->link($this->module_path . '/shippingProfileEntryUpdate', $this->session_token_key . '=' . $this->session_token . "&id_etsy_shipping_profiles_entries=" . $result['id_etsy_shipping_profiles_entries'] . $url, true),
                'delete' => $this->url->link($this->module_path . '/deleteShippingTemplateEntries', $this->session_token_key . '=' . $this->session_token . "&id_etsy_shipping_profiles=" . $result['id_etsy_shipping_profiles'] . "&id_etsy_shipping_profiles_entries=" . $result['id_etsy_shipping_profiles_entries'] . $url, true)
            );
        }
        $pagination = new Pagination();
        $pagination->total = $shipping_total;
        $pagination->page = $page;
        $pagination->limit = $this->config->get('config_limit_admin');
        $pagination->url = $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . $url . '&page={page}', true);

        $data['pagination'] = $pagination->render();
        $data['results'] = sprintf($this->language->get('text_pagination'), ($shipping_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($shipping_total - $this->config->get('config_limit_admin'))) ? $shipping_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $shipping_total, ceil($shipping_total / $this->config->get('config_limit_admin')));
        //links
        $data['general_settings'] = $this->url->link($this->module_path . '/generalSettings', $this->session_token_key . '=' . $this->session_token, 'SSL');
        $data['action'] = $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token, 'SSL');
        $data['route'] = $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token, 'SSL');
        $data['cancel'] = $this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token, 'SSL');
        $data['add'] = $this->url->link($this->module_path . '/shippingProfileEntryUpdate', $this->session_token_key . '=' . $this->session_token . $url . "&id_etsy_shipping_profiles=" . $id_etsy_shipping_profiles, true);
        ...
        $data['heading_title'] = $this->language->get('heading_title');
        $data['heading_title_main'] = $this->language->get('heading_title_main');
        $data['heading_title_shipping_entries'] = $this->language->get('heading_title_shipping_entries') . " (" . $shippingProfileData['shipping_profile_title'] . ")";
    ...
    public function shippingProfileEntryUpdate()
    {
        // load settings for Etsy plugin from database or from default settings
        $this->load->language($this->module_path);
        ...
        if (isset($this->request->get['id_etsy_shipping_profiles_entries'])) {
            $shipping_profiles_details = $this->model_jgetsy_jgetsy->getShippingTemplateEntries(array('id_etsy_shipping_profiles_entries' => $this->request->get['id_etsy_shipping_profiles_entries']));
            if (count($shipping_profiles_details) > 0) {
                $data['etsy'] = array();
                $data['etsy'] = array(
                    'template' => array(
                        'template_title' => $shipping_profiles_details[0]['shipping_profile_title'],
                        'primary_cost' => $shipping_profiles_details[0]['shipping_entry_primary_cost'],
                        'secondary_cost' => $shipping_profiles_details[0]['shipping_entry_secondary_cost'],
                        'shipping_entry_destination_country_id' => $shipping_profiles_details[0]['shipping_entry_destination_country_id'],
                        'shipping_entry_destination_region_id' => $shipping_profiles_details[0]['shipping_entry_destination_region_id'],
                    )
                );
                $data['destination_type'] = $shipping_profiles_details[0]['shipping_entry_destination_country_id'] ? 'country' : 'region';
                $data['etsy_select_country'] = $shipping_profiles_details[0]['shipping_origin_country_id'];
                $data['id_etsy_shipping_profiles_entries'] = $shipping_profiles_details[0]['id_etsy_shipping_profiles_entries'];
                $data['id_etsy_shipping_profiles'] = $shipping_profiles_details[0]['id_etsy_shipping_profiles'];
            } else {
                $this->session->data['error'] = $this->language->get('wrong_url_reached');
                $this->response->redirect($this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token, 'SSL'));
            }
        } else if (isset($this->request->get['id_etsy_shipping_profiles'])) {
            $shipping_profiles_details = $this->model_jgetsy_jgetsy->getShippingTemplateDetails(array(), $this->request->get['id_etsy_shipping_profiles']);
            if (count($shipping_profiles_details) > 0) {
                $data['etsy'] = array();
                $data['etsy'] = array(
                    'template' => array(
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if (isset($this->request->post['id_etsy_shipping_profiles_entries'])) {
                $this->model_jgetsy_jgetsy->shippingProfileEntryUpdate($this->request->post);
                $this->session->data['success'] = $this->language->get('shipping_profile_location_updated');
                $this->executeCRON("template/updateShippingTemplateEntryRequest");
            } elseif (isset($this->request->post['id_etsy_shipping_profiles'])) {
                $this->session->data['success'] = $this->language->get('shipping_profile_location_updated');
                $this->model_jgetsy_jgetsy->shippingProfileEntryUpdate($this->request->post);
                $this->executeCRON("template/createShippingTemplateEntryRequest");
            }
            if (isset($this->request->get['id_etsy_shipping_profiles_entries'])) {
                $this->response->redirect($this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . '&id_etsy_shipping_profiles=' . $data['id_etsy_shipping_profiles'], 'SSL'));
            } else {
                $this->response->redirect($this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . '&id_etsy_shipping_profiles=' . $this->request->post['id_etsy_shipping_profiles'], 'SSL'));
            }
        }
        ...
        if (isset($this->request->get['id_etsy_shipping_profiles_entries'])) {
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . "&id_etsy_shipping_profiles=" . $data['id_etsy_shipping_profiles'], 'SSL'),
                'separator' => ' :: '
            );

            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('heading_title_shipping_entry_edit'),
                'href' => $this->url->link($this->module_path . '/shippingProfileEntryUpdate', $this->session_token_key . '=' . $this->session_token . "&id_etsy_shipping_profiles_entries=" . $this->request->get['id_etsy_shipping_profiles_entries'], 'SSL'),
                'separator' => ' :: '
            );
        } else {
            ...
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('heading_title_shipping_entry'),
                'href' => $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . "&id_etsy_shipping_profiles=" . $this->request->get['id_etsy_shipping_profiles'], 'SSL'),
                'separator' => ' :: '
            );

            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('heading_title_shipping_entry_add'),
                'href' => $this->url->link($this->module_path . '/shippingProfileEntryUpdate', $this->session_token_key . '=' . $this->session_token, 'SSL'),
                'separator' => ' :: '
            );
        }
        ...
        //links
        if (isset($this->request->get['id_etsy_shipping_profiles_entries'])) {
            $data['action'] = $this->url->link($this->module_path . '/shippingProfileEntryUpdate', $this->session_token_key . '=' . $this->session_token . "&id_etsy_shipping_profiles_entries=" . $this->request->get['id_etsy_shipping_profiles_entries'], 'SSL');
        } else if (isset($this->request->get['id_etsy_shipping_profiles'])) {
            $data['action'] = $this->url->link($this->module_path . '/shippingProfileEntryUpdate', 'id_etsy_shipping_profiles=' . $this->request->get['id_etsy_shipping_profiles'] . '&' . $this->session_token_key . '=' . $this->session_token, 'SSL');
        }

        if (isset($this->request->get['id_etsy_shipping_profiles_entries'])) {
            $data['cancel'] = $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . '&id_etsy_shipping_profiles=' . $data['id_etsy_shipping_profiles'], 'SSL');
        } else {
            $data['cancel'] = $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . '&id_etsy_shipping_profiles=' . $this->request->get['id_etsy_shipping_profiles'], 'SSL');
        }
        ...
        $data['etsy_countries'] = $this->model_jgetsy_jgetsy->getEtsyCountries();
        $data['etsy_regions'] = $this->model_jgetsy_jgetsy->getEtsyRegions();
        ...
        if (isset($this->request->post['etsy']['template']['primary_cost'])) {
            $data['etsy']['template']['primary_cost'] = $this->request->post['etsy']['template']['primary_cost'];
        } else if (isset($this->request->get['id_etsy_shipping_profiles_entries'])) {
        ...
        if (VERSION >= '2.2.0.0') {
            $this->response->setOutput($this->load->view($this->module_path . '/shipping_profile_entry_form', $data));
        } else {
            $this->response->setOutput($this->load->view($this->module_path . '/shipping_profile_entry_form.tpl', $data));
        }
    }
    ...
    public function deleteShippingTemplateEntries()
    {
        ...
        if (!empty($this->request->get['id_etsy_shipping_profiles'])) {
            if($this->demo_flag == 0) {
                $this->load->model('jgetsy/jgetsy');
                $id_etsy_shipping_profiles_entries = $this->request->get['id_etsy_shipping_profiles_entries'];
                $this->model_jgetsy_jgetsy->deleteShippingTemplateEntries($id_etsy_shipping_profiles_entries);
                ...
                if (!empty($this->request->get['id_etsy_shipping_profiles'])) {
                    $this->response->redirect($this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token . "&id_etsy_shipping_profiles=" . $this->request->get['id_etsy_shipping_profiles'], 'SSL'));
                } else {
                    $this->response->redirect($this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token, 'SSL'));
                }
            } else {
                $this->session->data['error'] = $this->language->get('text_demo_mode_action');
                $this->response->redirect($this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token, 'SSL'));
    ...
    public function orderListing()
    {
        ...
        $orders_total = $this->model_jgetsy_jgetsy->getTotalOrders($filter_data);
        $results = $this->model_jgetsy_jgetsy->getOrders($filter_data);
    ...
    public function productListing()
    {
        ...
        $data['etsy_profiles'] = $this->model_jgetsy_profile->getAllProfiles();

        $orders_total = $this->model_jgetsy_jgetsy->getTotalProducts($filter_data);
        $results = $this->model_jgetsy_jgetsy->getProducts($filter_data);
        ...
        $this->load->model('setting/setting');
        $secure_key = $this->model_setting_setting->getSetting('jgetsy_secure_key');
        $data['secure_key'] = $secure_key['jgetsy_secure_key'];
    ...
    public function auditLog()
    {
        ...
        $audit_total = $this->model_jgetsy_jgetsy->getAuditLogTotal($filter_data);
        $audit_details = $this->model_jgetsy_jgetsy->getAuditLog($filter_data);
    ...
    private function validate()
    {
        ...
        if (isset($this->request->post['etsy']['template']) && isset($this->request->post['shipping_profiles'])) {
            ...
            } else if (isset($this->request->post['etsy']['template']['template_title']) && !isset($this->request->post['id_etsy_shipping_profiles'])) {
                if ($this->model_jgetsy_jgetsy->checkTemplateExists(trim($this->request->post['etsy']['template']['template_title']))) {
        ...
        } else if (isset($this->request->post['etsy']['template']) && isset($this->request->post['shipping_profiles_entries'])) {
            if (($this->request->post['etsy']['template']['primary_cost'] == '') || !$this->validatePriceFormat($this->request->post['etsy']['template']['primary_cost'])) {
                $this->error['etsy_primary_cost'] = $this->language->get('error_etsy_primary_cost');
            }
            if (($this->request->post['etsy']['template']['secondary_cost'] == '') || !$this->validatePriceFormat($this->request->post['etsy']['template']['secondary_cost'])) {
            ...
            if ($this->request->post['etsy']['template']['destination_type'] == 'country') {
                if ($this->request->post['etsy']['template']['shipping_entry_destination_country_id'] == "") {
                    ...
                    if (isset($this->request->post['id_etsy_shipping_profiles_entries'])) {
                        $status = $this->model_jgetsy_jgetsy->checkDestinationCountryExists($this->request->post['etsy']['template']['shipping_entry_destination_country_id'], $this->request->post['id_etsy_shipping_profiles'], $this->request->post['id_etsy_shipping_profiles_entries']);
                    } else {
                        $status = $this->model_jgetsy_jgetsy->checkDestinationCountryExists($this->request->post['etsy']['template']['shipping_entry_destination_country_id'], $this->request->post['id_etsy_shipping_profiles']);
                    }
                    if ($status) {
                ...
                if (empty($this->request->post['etsy']['template']['shipping_entry_destination_region_id'])) {
                    ...
                    if (isset($this->request->post['id_etsy_shipping_profiles_entries'])) {
                        $status = $this->model_jgetsy_jgetsy->checkDestinationRegionExists($this->request->post['etsy']['template']['shipping_entry_destination_region_id'], $this->request->post['id_etsy_shipping_profiles'], $this->request->post['id_etsy_shipping_profiles_entries']);
                    } else {
                        $status = $this->model_jgetsy_jgetsy->checkDestinationRegionExists($this->request->post['etsy']['template']['shipping_entry_destination_region_id'], $this->request->post['id_etsy_shipping_profiles']);
                    }
                    if ($status) {
        ...
        if (isset($this->request->post['etsy']['shop_section'])) {
            ...
            if (!$this->request->post['etsy']['shop_section']['title']) {
                ...
                if (isset($this->request->post['shop_section_id'])) {
                    $status = $this->model_jgetsy_shop_section->checkSectionExist($this->request->post['etsy']['shop_section']['title'], $this->request->post['shop_section']);
                } else {
                    $status = $this->model_jgetsy_shop_section->checkSectionExist($this->request->post['etsy']['shop_section']['title']);
                }
                if($status) {
        ...
        if (isset($this->request->post['etsy']['profile'])) {
            ...
            if (!isset($this->request->post['product_category'])) {
                ...
                if (isset($this->request->post['id_etsy_profiles'])) {
                    $status = $this->model_jgetsy_profile->checkProfileCategory($this->request->post['product_category'], $this->request->post['id_etsy_profiles']);
                } else {
                    $status = $this->model_jgetsy_profile->checkProfileCategory($this->request->post['product_category']);
                }
                if (!empty($status)) {
    ...
    public function install()
    {
        if (VERSION >= '2.0.0.0' && VERSION < '2.0.1.0') {
            ...
            $this->model_tool_event->addEvent('jgetsy', 'post.order.history.add', 'ebay_feed/cron/on_order_history_add');
            $this->model_tool_event->addEvent('jgetsy', 'post.admin.product.edit', 'module/kbebay/onProductUpdate');
        } elseif (VERSION >= '2.0.1.0' && VERSION <= '2.1.0.2') {
            $this->load->model('extension/event');
            $this->model_extension_event->addEvent('jgetsy', 'post.order.history.add', 'jgetsy/cron/on_order_history_add');
            $this->model_extension_event->addEvent('jgetsy', 'post.admin.product.edit', 'module/jgetsy/onProductUpdate');
        } elseif (VERSION >= '2.2.0.0' && VERSION < '3.0.0.0') {
            $this->load->model('extension/event');
            $this->model_extension_event->addEvent('jgetsy', 'catalog/model/checkout/order/addOrderHistory/after', 'jgetsy/cron/on_order_history_add');
            $this->model_extension_event->addEvent('jgetsy', 'admin/model/catalog/product/editProduct/after', 'extension/module/jgetsy/onProductUpdate');
        } elseif (VERSION >= '3.0.0.0') {
            $this->load->model('setting/event');
            $this->model_setting_event->addEvent('jgetsy', 'catalog/model/checkout/order/addOrderHistory/after', 'jgetsy/cron/on_order_history_add');
            $this->model_setting_event->addEvent('jgetsy', 'admin/model/catalog/product/editProduct/after', 'extension/module/jgetsy/onProductUpdate');
        }
        $this->load->model('jgetsy/jgetsy');
        $this->load->model('setting/setting');

        $demo_flag['jgetsy_demo_flag'] = $this->demo_flag;
        $this->model_setting_setting->editSetting('jgetsy_demo_flag', $demo_flag);

        $this->model_jgetsy_jgetsy->install();
    }
    ...
    public function uninstall()
    {
        if (VERSION >= '2.0.0.0' && VERSION < '2.0.1.0') {
            $this->load->model('tool/event');
            $this->model_tool_event->deleteEvent('jgetsy');
        } elseif (VERSION >= '2.0.1.0' && VERSION < '3.0.0.0') {
            $this->load->model('extension/event');
            $this->model_extension_event->deleteEvent('jgetsy');
        } elseif (VERSION >= '3.0.0.0') {
            $this->load->model('setting/event');
            $this->model_setting_event->deleteEvent('jgetsy');
        }
    }
    ...
    public function synchronization()
    {
        ...
        $data['text_shipping_profile_syncronization'] = $this->language->get('text_shipping_profile_syncronization');
    ...
    public function deleteProfile()
    {
        ...
        if($this->demo_flag == 0) {
            if (!empty($this->request->get['id_etsy_profiles'])) {
                ...
                if ($this->model_jgetsy_profile->deleteProfile($id_etsy_profiles)) {
        ...
        if (!empty($this->request->post['selected'])) {
            if (isset($this->request->post['selected']) && $this->validateDelete()) {
                foreach ($this->request->post['selected'] as $id_etsy_profiles) {
                    if ($this->model_jgetsy_jgetsy->deleteProfile($id_etsy_profiles)) {
                        $this->session->data['success'] = $this->language->get('text_profile_delete_success');
                    } else {
                        $this->session->data['error'] = $this->language->get('text_profile_delete_failure');
                    }
    ...
    public function enableProfile()
    {
        ...
        if (isset($this->request->post['selected']) && $this->validateDelete()) {
            foreach ($this->request->post['selected'] as $id_etsy_profiles) {
                if ($this->model_jgetsy_jgetsy->enableProfile($id_etsy_profiles)) {
    ...
    public function disableProfile()
    {
        ...
        if (isset($this->request->post['selected']) && $this->validateDelete()) {
            foreach ($this->request->post['selected'] as $id_etsy_profiles) {
                if ($this->model_jgetsy_jgetsy->disableProfile($id_etsy_profiles)) {
    ...
    public function attributeMapping()
    {
        ...
        if ($options == false) {
            ...
            $data['options'] = true;
            $data['optionMappings'] = $this->model_jgetsy_attribute->getOptionsName($optionId);
        }

        $data['etsy_options'][] = array("id" => "", "name" => $this->language->get('select_etsy_option'));
        $etsy_options = $this->model_jgetsy_attribute->getEtsyOptions();
    ...
    public function saveAttributeMapping()
    {
        ...
        if (isset($this->request->post['etsy_option_id']) && isset($this->request->post['opencart_option_id'])) {
            if ($this->request->post['etsy_option_id'] == "" || $this->request->post['opencart_option_id'] == "") {
                ...
                $this->model_jgetsy_attribute->saveMapping($this->request->post['etsy_option_id'], $this->request->post['opencart_option_id']);
    ...
    public function tabs($data)
    {
        ...
        $data['shipping_profiles'] = $this->url->link($this->module_path . '/shippingProfiles', $this->session_token_key . '=' . $this->session_token, 'SSL');
        $data['shop_section'] = $this->url->link($this->module_path . '/shopSection', $this->session_token_key . '=' . $this->session_token, 'SSL');
        $data['shipping_profile_entries'] = $this->url->link($this->module_path . '/shippingProfileEntries', $this->session_token_key . '=' . $this->session_token, 'SSL');
    ...
    private function checkSettings()
    {
        $this->load->language($this->module_path);
        $this->load->model('jgetsy/jgetsy');
        $config = $this->model_jgetsy_jgetsy->getSetting('etsy_general_settings');
        if (empty($config)) {
            $this->session->data['error'] = $this->language->get('text_save_general_setting');
            $this->response->redirect($this->url->link($this->module_path . '/generalSettings', $this->session_token_key . '=' . $this->session_token, 'SSL'));
        } else {
            $etsy_access_token = $this->model_jgetsy_jgetsy->getSetting('etsy_access_token');
            $etsy_access_token_secret = $this->model_jgetsy_jgetsy->getSetting('etsy_access_token_secret');
            if (empty($etsy_access_token) || empty($etsy_access_token_secret)) {
    ...
    {
        $url = HTTPS_CATALOG . "index.php?route=jgetsy/" . $name;
        $curl = curl_init();
