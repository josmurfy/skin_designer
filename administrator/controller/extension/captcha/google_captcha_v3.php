...
class ControllerExtensionCaptchaGoogleCaptchaV3 extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('extension/extension', 'user_token=' . $this->session->data['token'] . '&type=captcha', true));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/captcha/google_captcha_v3', 'user_token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/captcha/google_captcha_v3', 'user_token=' . $this->session->data['token'], true);
