...
class ControllerExtensionPaymentStripe extends Controller { 
	...
	public function index() {
		...
		$data['settings'][] = array(
			'type'		=> 'html',
			'title'		=> '',
			'content'	=> '<a href="index.php?route=extension/' . $this->type . '/' . $this->name . '/launch&token=' . $data['token'] . '"><img width="190" height="33" src="view/image/payment/stripe-connect.png" /></a><br><br>',
		);
		$data['settings'][] = array(
			'type'		=> 'html',
		...
		$data['settings'][] = array(
			...
			'attributes'=> array('readonly' => 'readonly', 'onclick' => 'this.select()', 'style' => 'background: #F5F5F5; cursor: pointer; font-family: monospace; width: 100% !important;'),
		);
	...
	public function launch() {
		...
		$this->response->redirect('https://www.getclearthinking.com/index.php?route=stripe_connect&origin=' . base64_encode($this->request->server['HTTP_REFERER']) . '&key=' . $this->session->data['stripe_encryption_key']);
	}
