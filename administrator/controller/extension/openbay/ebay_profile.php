...
class ControllerExtensionOpenbayEbayProfile extends Controller {
	...
	public function profileAll() {
		...
		$data['add'] = $this->url->link('extension/openbay/ebay_profile/add', 'user_token=' . $this->session->data['token'], true);
		...
		$data['breadcrumbs'][] = array(
			'href' => $this->url->link('extension/openbay/ebay_profile/profileAll', 'user_token=' . $this->session->data['token'], true),
	...
	public function add() {
		...
		$data['btn_save'] = $this->url->link('extension/openbay/ebay_profile/add', 'user_token=' . $this->session->data['token'], true);
		$data['cancel'] = $this->url->link('extension/openbay/ebay_profile/profileAll', 'user_token=' . $this->session->data['token'], true);
		...
		if (!isset($this->request->post['step1'])) {
			if ($this->request->post && $this->profileValidate()) {
				...
				$this->response->redirect($this->url->link('extension/openbay/ebay_profile/profileAll', 'user_token=' . $this->session->data['token'], true));
			}
		}
	...
	public function profileForm($data) {
		...
		if (empty($setting['dispatch_times']) || empty($setting['countries']) || empty($setting['returns'])){
			...
			$this->response->redirect($this->url->link('extension/openbay/ebay/syncronise', 'user_token=' . $this->session->data['token'], true));
		}
		...
		if (isset($this->request->get['ebay_profile_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			$data['action'] = $this->url->link('extension/openbay/ebay_profile/edit', 'user_token=' . $this->session->data['token'], true);
		} else {
			$data['action'] = $this->url->link('extension/openbay/ebay_profile/add', 'user_token=' . $this->session->data['token'], true);
		...
		if (!array_key_exists($type, $data['types'])) {
			...
			$this->response->redirect($this->url->link('extension/openbay/ebay_profile/profilAall', 'user_token=' . $this->session->data['token'], true));
		}
