<?php
class ControllerAbandonedCartsAbandonedCarts extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->response->setOutput($this->load->view('abandoned_carts/abandonedcarts_list.tpl', $data));
	}
	...
	public function getCarts() {
		...
		$abandoned_carts_total = 0;
		$abandoned_carts_total = $this->model_abandoned_carts_abandoned_carts->getTotalAbandonedCartsPeoples($filter_data);

		$results = $this->model_abandoned_carts_abandoned_carts->getAbandonedCartsPeoples($filter_data);
		...
		foreach ($results as $result) {
			...
			$products = $this->model_abandoned_carts_abandoned_carts->getAbandonedCartsProducts($filter_data);
			...
			$data['peoples'][] = array(
				...
				'name'					=> !empty(trim($result['name'])) ? $result['name'] : $this->language->get('text_not_available'),
				'email'					=> $result['email'],
				'telephone'			=> !empty($result['telephone']) ? $result['telephone'] : $this->language->get('text_not_available'),
				...
				'lastvisit_date'	=> date($this->language->get('datetime_format'), strtotime($result['lastvisit_date'])),
			);
		}
		...
		$this->response->setOutput($this->load->view('abandoned_carts/cart_list.tpl', $data));
	}
	...
	public function send() {
		...
		if (!empty($this->request->get['abandoned_template_id']) && !empty($this->request->post['selected_data'])) {
			$filter_data = array(
				'filter_name'	  							=> $filter_name,
				'filter_email'	  						=> $filter_email,
				'filter_store_id'	  					=> $filter_store_id,
				...
				'filter_account' 							=> $filter_account,
				'filter_date_added' 					=> $filter_date_added,
				'selected_data'								=> (isset($this->request->post['selected_data'])) ? $this->request->post['selected_data'] : array(),
			);
			
			$this->model_abandoned_carts_abandoned_carts->SendEmailTemplate($this->request->get['abandoned_template_id'], $filter_data);
	...
	public function deleteProdcut() {
		...
		if ($cart_id) {
			$this->model_abandoned_carts_abandoned_carts->deleteAbandonedCartProdcut($cart_id);
	...
	public function deleteCart() {
		...
		$cart_peoples = $this->model_abandoned_carts_abandoned_carts->getAbandonedCartByEmailStoreRows($email, $store_id);

		if ($cart_peoples) {
			foreach($cart_peoples as $cart_people) {
				$this->model_abandoned_carts_abandoned_carts->deleteAbandonedCartProdcut($cart_people['cart_id']);
			}
			$json['success'] = $this->language->get('text_success_delete');
		} else {
			$json['error'] = $this->language->get('error_not_found_product');
