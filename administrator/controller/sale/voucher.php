...
class ControllerSaleVoucher extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('sale/voucher', 'user_token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/voucher', 'user_token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('sale/voucher/add', 'user_token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('sale/voucher/delete', 'user_token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			if ($result['order_id']) {	
				$order_href = $this->url->link('sale/order/info', 'user_token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, true);
			} else {
				$order_href = '';
			}
			...
			$data['vouchers'][] = array(
				...
				'edit'       => $this->url->link('sale/voucher/edit', 'user_token=' . $this->session->data['token'] . '&voucher_id=' . $result['voucher_id'] . $url, true),
				'order'      => $order_href
			);
		}
		...
		$data['sort_code'] = $this->url->link('sale/voucher', 'user_token=' . $this->session->data['token'] . '&sort=v.code' . $url, true);
		$data['sort_from'] = $this->url->link('sale/voucher', 'user_token=' . $this->session->data['token'] . '&sort=v.from_name' . $url, true);
		$data['sort_to'] = $this->url->link('sale/voucher', 'user_token=' . $this->session->data['token'] . '&sort=v.to_name' . $url, true);
		$data['sort_theme'] = $this->url->link('sale/voucher', 'user_token=' . $this->session->data['token'] . '&sort=theme' . $url, true);
		$data['sort_amount'] = $this->url->link('sale/voucher', 'user_token=' . $this->session->data['token'] . '&sort=v.amount' . $url, true);
		$data['sort_status'] = $this->url->link('sale/voucher', 'user_token=' . $this->session->data['token'] . '&sort=v.status' . $url, true);
		$data['sort_date_added'] = $this->url->link('sale/voucher', 'user_token=' . $this->session->data['token'] . '&sort=v.date_added' . $url, true);
		...
		$pagination->url = $this->url->link('sale/voucher', 'user_token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['voucher_id'])) {
			$data['action'] = $this->url->link('sale/voucher/add', 'user_token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('sale/voucher/edit', 'user_token=' . $this->session->data['token'] . '&voucher_id=' . $this->request->get['voucher_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('sale/voucher', 'user_token=' . $this->session->data['token'] . $url, true);
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $voucher_id) {
			...
			if ($order_voucher_info) {
				$this->error['warning'] = sprintf($this->language->get('error_order'), $this->url->link('sale/order/info', 'user_token=' . $this->session->data['token'] . '&order_id=' . $order_voucher_info['order_id'], true));
	...
	public function history() {
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('sale/voucher/history', 'user_token=' . $this->session->data['token'] . '&voucher_id=' . $this->request->get['voucher_id'] . '&page={page}', true);
