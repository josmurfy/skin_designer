...
class ControllerReportCustomerSearch extends Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			'href' => $this->url->link('report/customer_search', 'user_token=' . $this->session->data['token'] . $url, true),
		...
		foreach ($results as $result) {
			...
			if ($result['customer_id'] > 0) {
				$customer = sprintf($this->language->get('text_customer'), $this->url->link('customer/customer/edit', 'user_token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'], true), $result['customer']);
			} else {
		...
		$pagination->url = $this->url->link('report/customer_search', 'user_token=' . $this->session->data['token'] . $url . '&page={page}', true);
