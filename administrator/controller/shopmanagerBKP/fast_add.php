<?php
class ControllerShopmanagerFastAdd extends Controller {
    private $error = array();
	...
	public function delete() {
		...
		//	echo count($this->request->post['selected']);
			...
			$this->response->redirect($this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	//	$this->document->addScript('view/javascript/fontawesome/css/fontawesome.css');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['product_search'] = $this->url->link('shopmanager/fast_add/product_search', 'token=' . $this->session->data['token']. $url, true);
	
		$data['print_report'] = $this->url->link('shopmanager/print_report', 'type_report=fast_add&token=' . $this->session->data['token'] . $url, true);
		$data['add'] = $this->url->link('shopmanager/fast_add/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['copy'] = $this->url->link('shopmanager/fast_add/copy', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('shopmanager/fast_add/delete', 'token=' . $this->session->data['token'] . $url, true);
		$data['enable'] = $this->url->link('shopmanager/fast_add/enable', 'token=' . $this->session->data['token'] . $url, true);
		$data['disable'] = $this->url->link('shopmanager/fast_add/disable', 'token=' . $this->session->data['token'] . $url, true);
...
//print("<pre>".print_r ($result['marketplace_accounts_id'] ,true )."</pre>");
		...
		//print("<pre>".print_r($categorySpecifics, true)."</pre>");
			...
			$data['products'][] = array('product_id' => $result['product_id'],
				...
				//'edit'       => ($result['upc']=='' || $result['has_specifics']) ? $this->url->link('shopmanager/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, true) : $this->url->link('shopmanager/product_search', 'token=' . $this->session->data['token'] . '&upc='.$result['upc'].'&product_id=' . $result['product_id']. '&condition_id=' . $result['condition_id'] , true)
				'edit'       => $result['has_specifics'] ? $this->url->link('shopmanager/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, true) : $this->url->link('shopmanager/product_search', 'token=' . $this->session->data['token'] . '&upc='.$result['upc'].'&product_id=' . $result['product_id']. '&condition_id=' . $result['condition_id'] . $url, true)
			);
		}
		...
		$data['sort_condition_id'] = $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . '&sort=p.condition_id' . $url, true);
		$data['sort_marketplace_item_id'] = $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . '&sort=p.marketplace_item_id' . $url, true);
		$data['sort_name'] = $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, true);
		$data['sort_model'] = $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . '&sort=p.model' . $url, true);
		$data['sort_price'] = $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . '&sort=p.price' . $url, true);
		$data['sort_quantity'] = $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . '&sort=p.quantity' . $url, true);
        $data['sort_unallocated_quantity'] = $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . '&sort=p.unallocated_quantity' . $url, true);
		$data['sort_location'] = $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . '&sort=p.location' . $url, true);
		$data['sort_sources'] = $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . '&sort=has_sources' . $url, true);
		$data['sort_specifics'] = $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . '&sort=has_specifics' . $url, true);
        $data['sort_status'] = $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, true);
		$data['sort_order'] = $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('shopmanager/fast_add',  'token=' . $this->session->data['token'] . $url . '&page={page}&limit='.$limit , true);
		$data['limit_link'] = $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'] . $url . '&page={page}&limit=', true);
	...
	/*	if(count($results)==1 && !isset($this->request->get['updated']) ){
			...
			$this->response->redirect($result['has_specifics'] ? $this->url->link('shopmanager/fast_add/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, true) : $this->url->link('shopmanager/fast_add/product_search', 'token=' . $this->session->data['token'] . '&filter_sku='.$result['sku'].'&product_id=' . $result['product_id']. '&condition_id=' . $result['condition_id'] . $url, true));
		}else{*/
    ...
    protected function getForm() {
        ...
        $data['action'] = !isset($this->request->get['product_id']) ? $this->url->link('shopmanager/fast_add/add', 'token=' . $this->session->data['token'], true) : $this->url->link('shopmanager/fast_add/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'], true);
        $data['cancel'] = $this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'], true);
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (($product_id &&  $this->request->post['condition_id']!='1000') || $this->request->post['upc'] == '') {
				...
				echo '<script>
					window.open("index.php?route=shopmanager/tools.create_label&sku=' . $sku . '&quantity=' . $quantity . '&upc=' . $upc . '&token=' . $this->session->data['token'] . '", "printWindow", "width=288,height=96");
				</script>';		
		
		}
			
			// Rediriger vers la page de succès ou de liste si nécessaire
			// $this->response->redirect($this->url->link('shopmanager/fast_add/add', 'token=' . $this->session->data['token'] . $url, true));
		}
    ...
    public function edit() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->session->data['success'] = $this->language->get('text_success');
            $this->response->redirect($this->url->link('shopmanager/fast_add', 'token=' . $this->session->data['token'], true));
        }
