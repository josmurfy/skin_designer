<?php
class ControllerShopmanagerCategory extends Controller {
	private $error = array();
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_shopmanager_catalog_category->addCategory($this->request->post);
			...
			$this->response->redirect($this->url->link('shopmanager/catalog/category', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function edit() {
		...
		$url_redirect='category';

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_shopmanager_catalog_category->editCategory($this->request->get['category_id'], $this->request->post);
//print("<pre>".print_r ($this->request->post,true )."</pre>");
			...
			//	//print("<pre>".print_r ($this->request->get,true )."</pre>");
				$url_redirect='product/edit';
				$url .= '&bypass=yes&upc=' . urlencode(html_entity_decode($this->request->get['upc'], ENT_QUOTES, 'UTF-8')) .'&product_id=' . urlencode(html_entity_decode($this->request->get['product_id'], ENT_QUOTES, 'UTF-8'));
			}
	
			if (isset($this->request->get['marketplace_item_id'])) {
				$url_redirect='product/edit';
				$url .= '&bypass=yes&marketplace_item_id=' . urlencode(html_entity_decode($this->request->get['marketplace_item_id'], ENT_QUOTES, 'UTF-8'));

			}
			$this->response->redirect($this->url->link('shopmanager/'.$url_redirect, 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('shopmanager/catalog/category', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('shopmanager/catalog/category/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('shopmanager/catalog/category/delete', 'token=' . $this->session->data['token'] . $url, true);
		$data['repair'] = $this->url->link('shopmanager/catalog/category/repair', 'token=' . $this->session->data['token'] . $url, true);
		...
		//	//print("<pre>".print_r ($result,true )."</pre>");
			...
			$data['categories'][] = array(
				...
				'edit'        => $this->url->link('shopmanager/catalog/category/edit', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'] . $url, true),
				'delete'      => $this->url->link('shopmanager/catalog/category/delete', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('shopmanager/catalog/category', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		$data['sort_order'] = $this->url->link('shopmanager/catalog/category', 'token=' . $this->session->data['token'] . '&sort=order' . $url, true);
		$data['sort_status'] = $this->url->link('shopmanager/catalog/category', 'token=' . $this->session->data['token'] . '&sort=c1.status' . $url, true);
		$data['sort_leaf'] = $this->url->link('shopmanager/catalog/category', 'token=' . $this->session->data['token'] . '&sort=c1.leaf' . $url, true);
		$data['sort_category_id'] = $this->url->link('shopmanager/catalog/category', 'token=' . $this->session->data['token'] . '&sort=category_id' . $url, true);
		...
		$pagination->url = $this->url->link('shopmanager/catalog/category', 'token=' . $this->session->data['token'] . $url . '&page={page}&limit='.$limit , true);
		$data['limit_link'] = $this->url->link('shopmanager/catalog/category', 'token=' . $this->session->data['token'] . $url . '&page={page}&limit=', true);
	...
	//	$data['results'] = sprintf($this->language->get('text_pagination'), ($category_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($category_total - $this->config->get('config_limit_admin'))) ? $category_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $category_total, ceil($category_total / $this->config->get('config_limit_admin')));
		...
		$result=$this->model_shopmanager_catalog_category_specific->getCategoriesWithoutSpecifics();

		$this->response->setOutput($this->load->view('shopmanager/catalog/category_list', $data));
	}
	...
	protected function getForm() {
		...
		$data['button_ai_description_category'] = $this->language->get('button_ai_description_category');
		...
		if (!isset($this->request->get['category_id'])) {
			$data['action'] = $this->url->link('shopmanager/catalog/category/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('shopmanager/catalog/category/edit', 'token=' . $this->session->data['token'] . '&category_id=' . $this->request->get['category_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('shopmanager/catalog/category', 'token=' . $this->session->data['token'] . $url, true);

		if (isset($this->request->get['category_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$category_info = $this->model_shopmanager_catalog_category->getCategory($this->request->get['category_id']);
		}
	//	//print("<pre>".print_r (714,true )."</pre>");
	//	//print("<pre>".print_r ($category_info,true )."</pre>");
		...
		$data['leaf']=$this->model_shopmanager_catalog_category->getCategoryLeaf($this->request->get['category_id'])??0;
		
		if (isset($this->request->post['category_description'])) {
			$data['category_description'] = $this->request->post['category_description'];
		} elseif (isset($this->request->get['category_id'])) {
			$data['category_description'] = $this->model_shopmanager_catalog_category->getCategoryDescriptions($this->request->get['category_id']);
			if(!is_array($data['category_description'][1]['specifics']) && isset($data['leaf']) && $data['leaf']==1){
...
//		//print("<pre>".print_r ($data,true )."</pre>");
	...
	//	//print("<pre>".print_r ($data,true )."</pre>");
		...
		if (isset($this->request->post['category_filter'])) {
			...
			$filters = $this->model_shopmanager_catalog_category->getCategoryFilters($this->request->get['category_id']);
		} else {
			$filters = array();
		}
		...
		if (isset($this->request->post['category_store'])) {
			...
			$data['category_store'] = $this->model_shopmanager_catalog_category->getCategoryStores($this->request->get['category_id']);
		} else {
	...
	/*	if (isset($this->request->post['keyword'])) {
		...
		if (isset($this->request->post['category_layout'])) {
			...
			$data['category_layout'] = $this->model_shopmanager_catalog_category->getCategoryLayouts($this->request->get['category_id']);
		} else {
			$data['category_layout'] = array();
		}
		//print("<pre>" . print_r(value: '884:CATEGORY.php') . "</pre>");
		$data['conditions']=$this->model_shopmanager_condition->getConditionDetails($data['category_id'],null,null,$data['site_id']??0);
	...
	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'shopmanager/catalog/category')) {
		...
		if (isset($this->request->get['category_id']) && $this->request->post['parent_id']) {
			$results = $this->model_shopmanager_catalog_category->getCategoryPath($this->request->post['parent_id']);
	...
	public function getCategoryDetails() {
		...
		if(isset($this->request->get['product_id'])){
			$this->model_shopmanager_product->removeProductSpecifics($this->request->get['product_id']);
		}
		$category_info =  $this->model_shopmanager_catalog_category->getCategoryDetails($category_id);
