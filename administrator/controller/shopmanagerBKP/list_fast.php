<?php
class ControllerShopmanagerListFast extends Controller {
    private $error = array();
	...
	public function delete() {
		...
		//	echo count($this->request->post['selected']);
			...
			$this->response->redirect($this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	//	$this->document->addScript('view/javascript/fontawesome/css/fontawesome.css');
		...
		if (isset($this->request->get['filter_epid'])) {
			$url .= '&filter_epid=' . $this->request->get['filter_epid'];
		} 
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['product_search'] = $this->url->link('shopmanager/list_fast/product_search', 'token=' . $this->session->data['token']. $url, true);
		$data['print_report'] = $this->url->link('shopmanager/print_report', 'type_report=list_fast&token=' . $this->session->data['token'] . $url, true);

		$data['add'] = $this->url->link('shopmanager/list_fast/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['copy'] = $this->url->link('shopmanager/list_fast/copy', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('shopmanager/list_fast/delete', 'token=' . $this->session->data['token'] . $url, true);
		$data['enable'] = $this->url->link('shopmanager/list_fast/enable', 'token=' . $this->session->data['token'] . $url, true);
		$data['disable'] = $this->url->link('shopmanager/list_fast/disable', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'marketplace_item_id' => $result['marketplace_item_id'],
				'image'      => $image,
				'name'       => $result['name'],
				'model'      => $result['model'],
				'price'      => round($result['price'],2),
				...
				'epid'   => $result['epid'],
				'has_sources'   => $result['has_sources'] ? $this->language->get('text_set') : $this->language->get('text_not_set'),
				'has_specifics'   => $result['has_specifics'] ? $this->language->get('text_set') : $this->language->get('text_not_set'),
				'status'     => $result['status'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled'),
				'edit' => ($result['has_specifics'] && $result['has_sources']) 
					? $this->url->link('shopmanager/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, true) 
					: ($result['has_sources'] 
						? "javascript:void(0);\" onclick=\"fastProductSearch(" . $result['product_id'] . ",'" . $result['upc'] . "')"
						: $this->url->link('shopmanager/list_fast/product_search', 'view=fast_list&token=' . $this->session->data['token'] . '&upc=' . $result['upc'] . '&product_id=' . $result['product_id'] . '&condition_id=' . $result['condition_id'] . $url, true)
					)
		...
		$data['text_epid_not_set'] = $this->language->get('text_epid_not_set');
		$data['text_epid_set'] = $this->language->get('text_epid_set');
		$data['text_epid_error'] = $this->language->get('text_epid_error');
		...
		$data['column_epid'] = $this->language->get('column_epid');
		...
		$data['entry_epid'] = $this->language->get('entry_epid');
		...
		$data['sort_condition_id'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . '&sort=p.condition_id' . $url, true);
		$data['sort_marketplace_item_id'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . '&sort=p.marketplace_item_id' . $url, true);
		$data['sort_epid'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . '&sort=pis.epid' . $url, true);
		$data['sort_name'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, true);
		$data['sort_model'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . '&sort=p.model' . $url, true);
		$data['sort_price'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . '&sort=p.price' . $url, true);
		$data['sort_quantity'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . '&sort=p.quantity' . $url, true);
        $data['sort_unallocated_quantity'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . '&sort=p.unallocated_quantity' . $url, true);
		$data['sort_location'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . '&sort=p.location' . $url, true);
		$data['sort_sources'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . '&sort=has_sources' . $url, true);
		$data['sort_specifics'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . '&sort=has_specifics' . $url, true);
        $data['sort_status'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, true);
		$data['sort_order'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('shopmanager/list_fast',  'token=' . $this->session->data['token'] . $url . '&page={page}&limit='.$limit , true);
		$data['limit_link'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'] . $url . '&page={page}&limit=', true);
	...
	/*	if(count($results)==1 && !isset($this->request->get['updated']) ){
			...
			$this->response->redirect($result['has_specifics'] ? $this->url->link('shopmanager/list_fast/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, true) : $this->url->link('shopmanager/list_fast/product_search', 'token=' . $this->session->data['token'] . '&filter_sku='.$result['sku'].'&product_id=' . $result['product_id']. '&condition_id=' . $result['condition_id'] . $url, true));
		}else{*/
    ...
    protected function getForm() {
        ...
        $data['error_invalid_upc'] = $this->language->get('error_invalid_upc');
        ...
        $data['action'] = !isset($this->request->get['product_id']) ? $this->url->link('shopmanager/list_fast/add', 'token=' . $this->session->data['token'], true) : $this->url->link('shopmanager/list_fast/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'], true);
        $data['cancel'] = $this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'], true);
    ...
    //print("<pre>".print_r ($current_language,true )."</pre>");
        ...
        $data['language_id'] = $data['languages']['language_id'];
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($data) && $data) {
				...
				if ($data['total_quantity'] < 1) {
					$this->model_shopmanager_product->editProductSku($data['product_id']);
					$sku = $data['product_id'];
				} else {
					$sku = $data['sku'];
				}
				...
				$this->session->data['warning'] = $this->language->get('text_success_add_quantity');
			} else {
				...
				$this->model_shopmanager_product->editProductSku($product_id);
				$sku = $product_id;
				$this->session->data['success'] = $this->language->get('text_success_add_product');
			}
			...
			if ($product_id) {
                ...
                var printWindow = window.open('index.php?route=shopmanager/tools.create_label&text=" . $sku . "&quantity=" . $quantity . "&upc=" . $upc . "&token=" . $this->session->data['token'] . "', 'printWindow', 'width=288,height=96');
                printWindow.print();
           </script>";
				 /*	setTimeout(function() {
                    printWindow.close();
                }, 10000);*/ // Fermer la fenêtre après 10 secondes
				echo $script; // Afficher le script pour qu'il soit exécuté après le chargement de la page
			}
			
			// Rediriger vers la page de succès ou de liste si nécessaire
			// $this->response->redirect($this->url->link('shopmanager/list_fast/add', 'token=' . $this->session->data['token'] . $url, true));
		}
    ...
    public function edit() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->session->data['success'] = $this->language->get('text_success');
            $this->response->redirect($this->url->link('shopmanager/list_fast', 'token=' . $this->session->data['token'], true));
        }
...
//	if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->request->post['selected']))  {
	...
	public function product_search() {
			...
			$test_selected = array('27061'=>'27061');
			$test_selected=$this->request->get['product_id']?array($this->request->get['product_id']=>$this->request->get['product_id']):$test_selected ;
	...
	//	if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->request->post['selected']))  {
		if ( isset($this->request->post['selected']))  {
			...
			foreach($selected as $product_id){
				...
				//	//print("<pre>".print_r ('1058:list_fast.php',true )."</pre>");
				//	//print("<pre>".print_r ($product_info,true )."</pre>");
		...
		// Ajout des informations Algopix et eBay
				...
				if(isset($upc_tmp_search['error']) && isset($algopix_search['error']) && isset($product_id)){
					$this->response->redirect($this->url->link('shopmanager/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $product_id, true));
				}
	...
	public function product_feed() {
			...
			if(isset($this->request->post['product_id'])){
				...
				/* if (isset($this->request->post['specifics_checkbox']) && isset($this->request->post['specifics_select'])) {
					...
					$this->model_shopmanager_product->updateProductSpecifics($specifics_checkbox, $specifics_select, $this->request->post['category_id']);
				}*/
