<?php
class ControllerShopmanagerCategorySpecific extends Controller {
    private $error = array();
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('shopmanager/catalog/category_specific', 'token=' . $this->session->data['token'] . $url, true)
		);
	
		$data['add'] = $this->url->link('shopmanager/catalog/category_specific/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('shopmanager/catalog/category_specific/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['category_specifics'][] = array(
				...
				'exclude' => $result['exclude']==1?$this->language->get('text_exclude'):$this->language->get('text_not_exclude'),
				'languages'     => $language_data,
				'edit'          => $this->url->link('shopmanager/catalog/category_specific/edit', 'token=' . $this->session->data['token'] . '&specific_name=' . urlencode($result['specific_name']), true),
				'delete'        => $this->url->link('shopmanager/catalog/category_specific/delete', 'token=' . $this->session->data['token'] . '&specific_name=' . urlencode($result['specific_name']), true)
			);
		...
		$data['column_specific_name'] = $this->language->get('column_specific_name');
		...
		$data['search_action'] = $this->url->link('shopmanager/catalog/category_specific', 'token=' . $this->session->data['token'], true);
		$data['edit_action'] = $this->url->link('shopmanager/catalog/category_specific/edit', 'token=' . $this->session->data['token'], true);
	
		$data['entry_specific_name'] = $this->language->get('entry_specific_name');
		...
		// Sorting URLs
		$data['sort_specific_name'] = $this->url->link('shopmanager/catalog/category_specific', 'token=' . $this->session->data['token'] . '&sort=specific_name' . $url, true);
	
		// Pagination
		$pagination = new Pagination();
		$pagination->total = $category_specific_total;
		$pagination->page = $page;
		$pagination->limit = $limit;
		$pagination->url = $this->url->link('shopmanager/catalog/category_specific', 'token=' . $this->session->data['token'] . $url . '&page={page}&limit=' . $limit, true);
	
		$data['pagination'] = $pagination->render();
		$data['results'] = sprintf($this->language->get('text_pagination'), ($category_specific_total) ? (($page - 1) * $limit) + 1 : 0, ((($page - 1) * $limit) > ($category_specific_total - $limit)) ? $category_specific_total : ((($page - 1) * $limit) + $limit), $category_specific_total, ceil($category_specific_total / $limit));
    ...
    public function edit() {
        ...
        $this->response->redirect($this->url->link('shopmanager/catalog/category_specific', 'token=' . $this->session->data['token'], true));
    }
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			$this->response->redirect($this->url->link('shopmanager/catalog/category_specific', 'token=' . $this->session->data['token'] . $url, true));
		}
