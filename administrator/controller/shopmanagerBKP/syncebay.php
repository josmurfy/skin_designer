<?php
class ControllerShopmanagerSyncebay extends Controller {
	private $error = array();
	...
	public function index() {
        ...
        $products = $this->model_shopmanager_product->getProductsToSyncEbay($limit, $offset);
    ...
    //print("<pre>".print_r ($ebayResponse,true )."</pre>");
       ...
       // echo '<script>setTimeout(() => { window.location.href = "' . $this->response->redirect('shopmanager/Syncebay', 'token=' . $this->session->data['token'].'&offset=' . $nextOffset, true) . '"; }, 5000);</script>';
       echo '<script>setTimeout(() => { window.location.href = "' . 
       html_entity_decode($this->url->link('shopmanager/syncebay', 'token=' . $this->session->data['token'] . '&offset=' . $nextOffset, true)) . 
       '"; }, 5000);</script>';
