...
// Fonction pour gérer le fichier et envoyer la requête avec le user_token
async function handleFile(files, user_token) {
    const formData = new FormData();
    formData.append('image', files[0]);

    // Soumettre la requête si un fichier est présent
    if (files.length > 0) {
        fetch('index.php?route=shopmanager/ocr.upload&user_token=' + user_token, {
            method: 'POST',
            body: formData
        })
