...
//alert(fieldName);
        ...
        for (var targetLanguageId in languages) {
            if (!languages.hasOwnProperty(targetLanguageId)) {
                continue;
            }
...
function generateMetaTag(language_id) {
    ...
    var keyword = document.querySelector('input[name="category_seo_url[0][' + language_id + ']"]');

    // Check if required elements exist
    if (!nameInput || !nameDescriptionElement) {
        console.warn('Required elements not found for language_id:', language_id);
        return;
    }
    ...
    tempDiv.innerHTML = additionalDescriptionText;
    additionalDescriptionText = tempDiv.textContent || tempDiv.innerText || "";

    metaTagTitle.value =  name ;
    metaTagDescription.value = additionalDescriptionText;
    var tagkeywords =  name ;
    // Remplacer & et / avant nettoyage
    tagkeywords = tagkeywords.replace(/&/g, ' and ').replace(/\//g, ' ');
    // Normaliser les accents
    var accentMap = {'é':'e','è':'e','ê':'e','ë':'e','à':'a','â':'a','ä':'a','ô':'o','ö':'o','û':'u','ù':'u','ü':'u','ç':'c','î':'i','ï':'i','É':'E','È':'E','Ê':'E','Ë':'E','À':'A','Â':'A','Ä':'A','Ô':'O','Ö':'O','Û':'U','Ù':'U','Ü':'U','Ç':'C','Î':'I','Ï':'I'};
    tagkeywords = tagkeywords.replace(/[éèêëàâäôöûùüçîïÉÈÊËÀÂÄÔÖÛÙÜÇÎÏ]/g, function(m) { return accentMap[m]; });
    ...
    // Première lettre de CHAQUE mot en majuscule (séparés par virgules)
    if (tagkeywords.length > 0) {
      tagkeywords = tagkeywords.split(',').map(function(word) {
    ...
    var keywords = name;
    // Remplacer & et / avant nettoyage
    keywords = keywords.replace(/&/g, ' and ').replace(/\//g, ' ');
    // Normaliser les accents
    var accentMap = {'é':'e','è':'e','ê':'e','ë':'e','à':'a','â':'a','ä':'a','ô':'o','ö':'o','û':'u','ù':'u','ü':'u','ç':'c','î':'i','ï':'i','É':'E','È':'E','Ê':'E','Ë':'E','À':'A','Â':'A','Ä':'A','Ô':'O','Ö':'O','Û':'U','Ù':'U','Ü':'U','Ç':'C','Î':'I','Ï':'I'};
    keywords = keywords.replace(/[éèêëàâäôöûùüçîïÉÈÊËÀÂÄÔÖÛÙÜÇÎÏ]/g, function(m) { return accentMap[m]; });
    // Supprimer caractères spéciaux
    keywords = keywords.replace(/[.,;:'"\{\}\[\]\(\)@%$\-]/g, '');
    var keywords = keywords.split(/\s+/).join('-').toLowerCase(); // Tout en minuscules pour SEO URL
    // Supprimer doubles tirets
    keywords = keywords.replace(/\-+/g, '-');
    // Supprimer le dernier caractère s'il est un tiret
            ...
            for (var specificName in specifics) {
                if (specifics.hasOwnProperty(specificName)) {
                    ...
                    if (constraint.aspectMode === 'SELECTION_ONLY') {
                        inputField = '<select id="category-description-' + languageId + '-' + specifics_row + '" name="category_description[' + languageId + '][specifics][' + specificName + '][Value]" class="form-select" ' + required + ' disabled>';
                        ...
                        inputField = '<select id="category-description-' + languageId + '-' + specifics_row + '" name="category_description[' + languageId + '][specifics][' + specificName + '][Value][]" class="form-select" multiple ' + required + ' size="' + Math.max(2, values.length + 2) + '" disabled>';
                        values.forEach(function(value) {
                            inputField += '<option value="' + value.localizedValue + '"' + (current_value.includes(value.localizedValue) ? ' selected' : '') + '>' + value.localizedValue + '</option>';
                        });
                        inputField += '</select>';
                    } else {
                        inputField = '<input type="text" id="category-description-' + languageId + '-' + specifics_row + '" name="category_description[' + languageId + '][specifics][' + specificName + '][Value]" placeholder="Texte libre" class="form-control" ' + required + ' value="' + current_value.join(', ') + '" readonly />';
                    }
                    ...
                    rowHtml.innerHTML = 
                        ...
                        '</td>' +
                        '<td class="text-left">' +
                            '<button type="button" onclick="removeSpecificsRow(' + specifics_row + ',' + specifics_row + ',\'' + specificName + '\');" data-bs-toggle="tooltip" title="Supprimer" class="btn btn-danger">' +
...
async function uploadFromLink(imageUrl) {
    ...
    .then(data => {
      if (data && data.success) {
        // Update input-image value FIRST
        var inputImageElement = document.getElementById('input-image');
      /*  if (inputImageElement) {
            inputImageElement.value = data.image_url;
        }
        
        // Reload image preview
        var thumbImage = $('#thumb-image');
        if (thumbImage.length) {
            thumbImage.attr('src', 'image/' + data.image_url + '?t=' + new Date().getTime());
        }*/
        
        // Update buttons
        ButtonDownload.prop('disabled', true).text('Downloaded').css('display', 'none');
...
function generateDescriptionOLD(language_id) {
  ...
  specificsRows.forEach(function(row) {
      var nameElem = row.querySelector('div[id^="category-description-"][id$="_Name"]');
  ...
  if (descriptionTextarea) {
      ...
      var displayDivId = descriptionTextarea.id.replace('category-description-', 'display_category-description-');
...
function trfValue(specificsRow) {
    ...
    var targetElement = $('#category-description-1-' + specificsRow);
    ...
    $('#original-value-' + specificsRow).val(originalValue);
...
function undoValue(specificsRow) {
    ...
    var originalValue = $('#original-value-' + specificsRow).val();
...
window.addEventListener('load', function() {
    ...
    var form = document.getElementById('form-category');
    var cancelLink = document.querySelector('a.btn-light[data-bs-toggle="tooltip"]'); // Bootstrap 5: btn-light and data-bs-toggle
    ...
    if (!cancelLink) {
        ...
        // Don't return, continue without cancel link functionality
    }
