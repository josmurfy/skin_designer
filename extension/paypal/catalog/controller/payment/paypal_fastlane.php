...
class PayPalFastlane extends \Opencart\System\Engine\Controller {
	private $error = [];
	private $separator = '';
	...
	public function index(): string {
		...
		if ($this->config->get('payment_paypal_status') && $this->config->get('payment_paypal_client_id') && $this->config->get('payment_paypal_secret') && $agree_status) {
			...
			return $this->load->view('extension/paypal/payment/paypal_fastlane', $data);
		}
	...
	public function modal(): void {
		...
		$this->response->setOutput($this->load->view('extension/paypal/payment/paypal_fastlane_modal', $data));
	}
}