...
namespace Opencart\Admin\Controller\Extension\Mpproductsgrid\Module;
...
class Mpproductsgrid extends \Opencart\System\Engine\Controller {
	...
	public function install(): void {
		...
		$action = 'extension/mpproductsgrid/module/mpproductsgrid|column_left';
		if (VERSION > '4.0.1.1') {
	...
	public function column_left(string &$route, array &$data, string &$code): void {
		...
		if (!in_array('mpproductsgrid.mpproductsgrid', $installed)) {
			return;
		}

		if ($this->user->hasPermission('access', 'extension/mpproductsgrid/module/mpproductsgrid')) {
			$menu = [
				'name'	   => $this->language->get('heading_title'),
				'href'     => $this->url->link('extension/mpproductsgrid/module/mpproductsgrid', 'user_token=' . $this->session->data['user_token']),
				'children' => []
			];
		}
	...
	public function index(): void {
		...
		if (!isset($this->request->get['module_id'])) {
			...
			$data['breadcrumbs'][] = [
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/mpproductsgrid/module/mpproductsgrid', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'])
			];
		}

		if (!isset($this->request->get['module_id'])) {
			$data['save'] = $this->url->link('extension/mpproductsgrid/module/mpproductsgrid|save', 'user_token=' . $this->session->data['user_token']);
		} else {
			$data['save'] = $this->url->link('extension/mpproductsgrid/module/mpproductsgrid|save', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id']);
		}
		...
		foreach ($grid_categories as $category_id) {
			...
			if (!empty($categoryinfo)) {
				...
				$categoryproducts = $this->model_extension_mpproductsgrid_product->getProductsByCategoryId($categoryinfo['category_id']);
				...
				$data['category_products'][ $categoryinfo['category_id'] ] = [
					'category_id' => $categoryinfo['category_id'],
					'name' => $categoryinfo['name'],
		...
		$data['grid_manufacturers'] = [];
		$data['manufacturer_products'] = [];
		...
		foreach ($grid_manufacturers as $manufacturer_id) {
			...
			if (!empty($manufacturerinfo)) {
				...
				$data['manufacturer_products'][ $manufacturerinfo['manufacturer_id'] ] = [
					'manufacturer_id' => $manufacturerinfo['manufacturer_id'],
					'name' => $manufacturerinfo['name'],
					'highlightproducts' => $highlightproducts,
		...
		$this->response->setOutput($this->load->view('extension/mpproductsgrid/module/mpproductsgrid', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/mpproductsgrid/module/mpproductsgrid')) {
			$json['error']['warning'] = $this->language->get('error_permission');
		}

		if ((mp_strlen($this->request->post['name']) < 3) || (mp_strlen($this->request->post['name']) > 64)) {
		...
		if (!$json) {
			...
			if (!$this->request->post['module_id']) {
				$this->model_setting_module->addModule('mpproductsgrid.mpproductsgrid', $this->request->post);
			} else {
