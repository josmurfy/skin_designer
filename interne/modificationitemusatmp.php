...
if ($_POST['name_product']!="" && (string)$_POST['sku'] !="" 
	&& $_POST['model']!="" && $_POST['manufacturer']!="" && $_POST['new']==1){
		
	if($_POST['etat']!=$_POST['etatavant']){
		$_POST['skuold']=(string)$_POST['sku'] ;
		if($_POST['etat']=="9")(string)$_POST['sku'] =substr((string)$_POST['sku'] ,0,12);
		if($_POST['etat']=="99")(string)$_POST['sku'] =substr((string)$_POST['sku'] ,0,12)."NO";
		if($_POST['etat']=="2")(string)$_POST['sku'] =substr((string)$_POST['sku'] ,0,12)."MR";
		if($_POST['etat']=="22")(string)$_POST['sku'] =substr((string)$_POST['sku'] ,0,12)."R";
		if($_POST['etat']=="8")(string)$_POST['sku'] =substr((string)$_POST['sku'] ,0,12)."U";
	...
	//echo $sql;
		...
		if($rowverif==0){
			...
			$sql6 .=' VALUES ("7","3","1","'.strtoupper($data['model']).'", "'.(string)$_POST['sku'] .'", "'.(string)$_POST['sku'] .'", "'.strtoupper($data['model']).'","0","'.$data['manufacturer_id'].'",';
			...
			$req7 = mysqli_query($db,$sql7);
			$sql = 'SELECT * FROM `oc_product_to_category` where product_id = "'.$data['product_id'].'"';
			//echo $sql;
			$req = mysqli_query($db,$sql);
			while($data = mysqli_fetch_assoc($req)){
				$sql7 = "INSERT INTO `oc_product_to_category` (`product_id`, `category_id`) VALUES ('".$product_id."', '".$data[category_id]."')";
				$req7 = mysqli_query($db,$sql7);
			}
			$sql = 'SELECT * FROM `oc_product_image` where product_id = "'.$data['product_id'].'"';
			//echo $sql;
			$req = mysqli_query($db,$sql);
			while($data = mysqli_fetch_assoc($req)){
				$sql7 = "INSERT INTO `oc_product_image` (`product_id`, `image`) VALUES ('".$product_id."', '".$data[image]."')";
				$req7 = mysqli_query($db,$sql7);
			}
		...
		$sql2 = 'UPDATE `oc_product` SET `inventaire`=1, `invoice`="'.$_POST['invoice'].'",`model`="'.strtoupper($_POST['model']).'", `upc`="'.(string)$_POST['sku'] .'"';
		$sql2 .=', `mpn` = "'.strtoupper($_POST['model']).'",`quantity` = "0'.$_POST['quantity'].'",`manufacturer_id` = "'.$_POST['manufacturer'].'"';
		$sql2 .=', `priceebaysold` ="'.$_POST['priceebaysold'].'", `priceterasold` = "'.$_POST['priceterasold'].'", `priceebaynow` ="'.$_POST['priceebaynow'].'"';
...
if (isset($_POST['category_id']) && $_POST['category_id']!=""){
		...
		$category_id=0;
		$categoryarbonum="";
		//if($_POST['newsku']==1)(string)$_POST['sku'] =mt_rand ( 100000000000 , 999999999999 );
...
foreach($conditionsupp as $conditioncheck){
	...
	//echo $sql;
			...
			$description.='<tr><td style=\"text-align: center;\" align=\"center\" valign=\"middle\"><img src=\"https://www.phoenixliquidation.ca/image/'.$data['image'].'\" width=\"450\"</td></tr>';
			...
			{
				if($i<13){
					$description.='<tr><td style=\"text-align: center;\" align=\"center\" valign=\"middle\"><img src=\"https://www.phoenixliquidation.ca/image/'.$data2['image'].'\" width=\"450\"</td></tr>';
				$i++;
				}
			}
...
if($_POST['etat']=="9")$algoetat="New";
if($_POST['etat']=="99")$algoetat="New";
if($_POST['etat']=="2")$algoetat=="Refurbished";
if($_POST['etat']=="22")$algoetat="Refurbished";
if($_POST['etat']=="8")$algoetat="Used";

			$new=1;
	  		$sql9 = 'SELECT * FROM `oc_product_analyse` where upc="'.substr ((string)$_POST['sku'] ,0,12).'" group by upc';
			//echo $sql9;
			$req9 = mysqli_query($db,$sql9);
			$data9 = mysqli_fetch_assoc($req9);
			$sql8 = 'SELECT * FROM `oc_product_reception` where upc="'.substr ((string)$_POST['sku'] ,0,12).'" group by upc';
			//echo $sql8;
...
<form id="form_67341" class="appnitro" action="modificationitemusa.php" method="post">
<div class="form_description">


</div>
<h3>SKU <?if($new==1){?><a href="modificationitemusa.php" >Changer d'item</a><?}?> <a href="interneusa.php" >Retour au MENU</a></h3>
<input id="sku"  type="text" name="sku"  value="<?echo (string)$_POST['sku'] ;?>" maxlength="255" <?if($new==1)echo "disabled";?> autofocus>
...
window.open("https://algopix.com/search?query=<?echo substr ((string)$_POST['sku'] ,0,12);?>&markets=AMAZON_US,EBAY_US,WALMART_US&from=US&shipping_method=carrier&costNew=<?echo $data8[pricecost];?>&costCurrencyCode=USD&itemConditions=<?echo $algoetat;?>&cost<?echo $algoetat;?>=<?echo $data8[pricecost];?>","algopix");
</script>
<?
if($picexterne!="")
{
...
<table width="100%" >
	...
	<table width="100%">
	  ...
	  <tr>
		...
		<td><h3><a href="createbarcodeusa.php?product_id=<?echo $_POST['product_id'];?>" target="_blank" style="color:#ff0000"><strong>Creation LABEL</strong></a></h3></td>
	  </tr>
