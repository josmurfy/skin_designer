...
class TCPDF {
	 ...
	 * @since 5.9.152 (2012-03-23)
	 */
	public function setOver($stroking=true, $nonstroking=null, $mode=0) {
		if ($this->state != 2) {
			return;
		}
	...
	protected function getHtmlDomArray($html) {
		...
		$csstagarray = '<cssarray>'.(json_encode($css)).'</cssarray>';//htmlentities
				...
				if (!empty($dom[$dom[$key]['parent']]['text-transform'])) {
					...
					if (function_exists('mb_convert_case')) {
						...
						switch ($dom[$dom[$key]['parent']]['text-transform']) {
							case 'capitalize': {
								$element = strtoupper(strtolower($element));
								break;
							}
							case 'uppercase': {
