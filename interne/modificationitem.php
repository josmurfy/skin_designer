...
if($_POST['skucheck']!="")
	(string)$_POST['sku'] =$_POST['skucheck'];
...
if((string)$_POST['hid_sku_ancien']!=(string)$_POST['sku'] && $_POST['sku']!=""){
	...
	if($_POST['marketplace_item_id']!="" && $_POST['marketplace_item_id']>0){
...
//	mise_en_page_description($connectionapi,$_POST['product_id'],$db); 
 ...
 if (isset($_POST['product_id'] ) && (string)$_POST['product_id'] !=""){
	...
	//		$_POST['accessoryfr']=$data3['accessory'];
		...
		//			//print("<pre>".print_r ($_POST,true )."</pre>");
			$sql4 = 'SELECT * FROM `oc_product_to_category`,`oc_category` where oc_product_to_category.category_id=oc_category.category_id and product_id="'.$_POST['product_id'].'" and leaf=1 GROUP BY oc_category.CATEGORY_ID';
			//	echo $sql4;
    ...
    <tbody>
      <tr align="center">
        <td colspan="4" rowspan="1" style="vertical-align:  middle; height: 50px;text-align:center;">
			<img style="width: 488px; height: 145px;" alt="" src="https://www.phoenixliquidation.ca/image/catalog/cie/entetelow.jpg">
		</td>
      </tr>
	 </tbody>
	 </table>
...
<tbody>
      <tr>
		...
		<input id="sku" type="text" name="sku" value="<?echo (string)$_POST['sku'] ;?>" maxlength="80" autofocus="">
...
?>
	...
	<tr>
        ...
        <td style="vertical-align:  middle; height: 50px; background-color: #030282; color: white; width: 200px;">
			<label>English Title: (<?echo strlen($_POST['nameen']);?>)</label>
...
?>		
		<label>Add if not in the list:</label> <input id="manufacturersupp"  type="text" name="manufacturersupp" value="" maxlength="80" />
		</div>
        </td>
...
<br>
     ...
     <tr>
	  ...
	  <tr>
		...
		<input type="hidden" name="price_with_shipping" value="<?echo $_POST['price_with_shipping'];?>"/> 
  </table>
  </td>
</tr>
</table>
