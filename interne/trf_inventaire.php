...
if (($_POST['quantitytrf']>0 || $_POST['qtyadd'] >0 )&& $_POST['new']==1){
	if($_POST['quantitytrf']==0 && $_POST['qtyadd']==0)
	{
		...
		//$qtysold=$_POST['quantity']-$_POST['quantitytrf'];
			...
			$sql2 = 'UPDATE `oc_product` SET unallocated_quantity=unallocated_quantity-'.$_POST['quantitytrf'].',quantity=quantity+'.$quantityadd.', location ="'.strtoupper($_POST['location_entrepot']).'" where product_id='.$_POST['product_id'];
	...
	if($_POST['createlabel']=="yes"){
		echo '<script>window.open("createlabeltablette.php?tablette='.$_POST['location_entrepot'].'","etiquette")</script>';
	}
}
