...
$date_et = new DateTime('now', $et_timezone);

//echo "<br>Heure actuelle dans l'Eastern Time (ET): " . $date_et->format('H:i:s') . "\n";
...
while ($data = mysqli_fetch_assoc($req)){
  //print("<pre>".print_r ($data,true )."</pre>");
    ...
    $date_ca = new DateTime('now', $et_timezone);
    //echo "<br>Heure actuelle dans l'Eastern Time (ET): " . $date_ca->format('H:i:s') . "\n";

    $timestamp_et = $date_et->getTimestamp();
    $timestamp_ca = $date_ca->getTimestamp();
    
    // Calculer la différence de temps en secondes entre l'ET et la Californie
    $diff_seconds = abs($timestamp_et - $timestamp_ca);
    
    echo "Différence de temps : " . $diff_seconds . " secondes\n";
    echo "Différence de temps : " . $diff_seconds/60 . " minutes\n";
