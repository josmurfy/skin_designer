...
unlink('/home/n7f9655/public_html/phoenixliquidation/interne/download/algopix'.date("Y-m-d").'___.csv');
link('/home/n7f9655/public_html/phoenixliquidation/interne/download/algopix'.date("Y-m-d").'___.csv','/home/n7f9655/public_html/phoenixliquidation/admin/interne/download/algopix__'.date("Y-m-d").'.csv');
$fp = fopen('/home/n7f9655/public_html/phoenixliquidation/interne/download/algopix'.date("Y-m-d").'___.csv', 'w');
...
where pd.name="" and pd.language_id=1 and p.quantity>0 and verif_fait is null order by p.product_id desc'; 
...
while($data = mysqli_fetch_assoc($req))
    {
		...
		fputs($fp, "\n");
		$sql2="UPDATE oc_product SET verif_fait='1' WHERE product_id='".$data['product_id']."'"; 
		//echo $sql2;
...
//Header('Content-type: text/xml');
//print($domtree->asXML());
echo '<a href="download/algopix'.date("Y-m-d").'___.csv" target="_blank" style="color:#ff0000"><strong>Download fichier</strong></a>';
mysqli_close($db); 
?> 
