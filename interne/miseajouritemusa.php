...
/* 		$sql2 = 'UPDATE `oc_product` SET `status` = 1,stock_status_id=5,price=price_with_shipping where quantity=0 and price=0';
		...
		$sql2 = 'UPDATE `oc_product` SET `status` = 0,stock_status_id=5 where quantity<1 or location=""';
		//echo $sql2.'<br><br>';
		$req2 = mysqli_query($db,$sql2);
...
/* 		//flush category
		...
		while($data = mysqli_fetch_assoc($req)){ 
			...
			while($data2 = mysqli_fetch_assoc($req2)){
 			...
 			$sql = 'SELECT * FROM `oc_product`,`oc_product_description` where language_id=1 and oc_product.product_id=oc_product_description.product_id  ';
			//echo $sql."<br>";
			$req = mysqli_query($db,$sql);
			$calcitem=0;
	while($data = mysqli_fetch_assoc($req)){ 
			$name=$data['name'];
			$calcitem=$calcitem+1;
			$sql8 = "SELECT name,product_id FROM oc_product_description where language_id=2 and product_id=".$data['product_id'];
			...
			//description
/* 			$sql5 = 'SELECT * FROM `oc_product_index` where upc='.substr((string)$data['upc'],0,12);

			// on envoie la requête
			//echo $sql5;
			$req5 = mysqli_query($db,$sql5);
			$data5 = mysqli_fetch_assoc($req5);
 			$sql2 = 'UPDATE `oc_product` SET upc="'.substr((string)$data['upc'],0,12).'" WHERE `product_id` ='.$data['product_id'];
			//echo $sql2;
			$req2 = mysqli_query($db,$sql2); 
			...
			$descriptionf.='<strong>Titre : </strong>'.$namef.'<br><strong>'.$modele.' : </strong>'.$data['model'];
			...
			$descriptionf.='<strong>Couleur : </strong>'.$data['color'].'<br>';
...
//echo $sql3;
			...
			$descriptionf.=addslashes($data3['condition']."<br>");
			$conditionnamef=$data3['name'];
			...
			$descriptionf.='<tr><td style=\"text-align: center;\" align=\"center\" valign=\"middle\"><img src=\"https://www.phoenixliquidation.ca/image/'.$data['image'].'\" width=\"450\"</td></tr>';
			...
			while($data2 = mysqli_fetch_assoc($req2))
			{
				if($i<13){
					...
					$descriptionf.='<tr><td style=\"text-align: center;\" align=\"center\" valign=\"middle\"><img src=\"https://www.phoenixliquidation.ca/image/'.$data2['image'].'\" width=\"450\"</td></tr>';
				$i++;
				}
			}
...
/* 			$sql2 = "UPDATE `oc_product` SET image ='catalog/product/".str_replace('catalog/product/', '',$data['image'])."' where product_id=".$data['product_id']."";
			echo $sql2;
			$req2 = mysqli_query($db,$sql2); */
			$sql2 = 'UPDATE `oc_product_description` SET tag="'.$conditionname.','. str_replace(' ', ',', addslashes(strtoupper($name))).','.(string)$data['upc'].'",meta_keyword="'.$conditionname.','.strtoupper($keywordindex).','. str_replace(' ', ',', addslashes(strtoupper($name))).','.(string)$data['upc'].'", meta_title="'.$conditionname." ".addslashes(strtoupper($name)).'",name="'.addslashes(strtoupper($name)).'", description="'.$description.'", meta_description="Liquidation '.addslashes(strtoupper($categoryname.' '.strtoupper($keywordindex).' '.$name)).' '.$conditionname.' at the lowest price of $'.$prixcad.' CAD Model: '.$data['model'].' UPC:'.(string)$data['upc'].'" WHERE language_id=1 and `product_id` ='.$data['product_id'];
			//echo $sql2;
			$req2 = mysqli_query($db,$sql2);
			

			
			$sql2 = 'UPDATE `oc_product_description` SET tag="'.$conditionnamef.','. str_replace(' ', ',', addslashes($namef)).','.(string)$data['upc'].'",meta_keyword="'.$conditionnamef.','. str_replace(' ', ',', addslashes($namef)).','.(string)$data['upc'].'", meta_title="'.$conditionnamef." ".addslashes($namef).'",name="'.addslashes($namef).'", description="'.$descriptionf.'", meta_description="Liquidation '.addslashes($categorynamef.' '.$namef).' '.$conditionnamef.' au meilleur prix de $'.$prixcad.' CAD Modele: '.$data['model'].' UPC:'.(string)$data['upc'].'"  WHERE language_id=2 and `product_id` ='.$data['product_id'];
