...
// mettre ajour les orphelins non declaré
...
//echo $sql;
		...
		while ($data = mysqli_fetch_assoc($req))
		{
				...
				$sql2 = 'UPDATE `oc_product_reception` quantityrecu=0,quantity='.$data[quantityrecu].',SET model="'.$data3[model].'",title="'.$data3[title].'",brand="'.$data3[brand].'",priceretailnew='.$walmart.',close='.$close.',  pricedetailusd='.$pricedetailusd.' where  title="" and upc='.$data[upc].' and invoice='.$_POST['invoice'];
...
<body bgcolor="ffbaba">
<form id="form_67341" class="appnitro" action="calculcostreception.php" method="post">
