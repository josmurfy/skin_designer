...
if(isset($_POST['analyseinput']))
{
		...
		foreach($analyseinputnametab as $analyseinputname) 
		{
			$analyseinputnameline=explode("###", $analyseinputname);
				...
				$sql = 'INSERT INTO `oc_product_reception` (invoice,upc,model,brand,title,quantity,pricedetailusd,pricedetailcad)';
				$sql = $sql.'values ("';
						$i=0;
				//echo $analyseinputnameline[0]."<br>";
							...
							$sql=$sql.htmlentities($analyseinputnameline[0]).'","'.htmlentities($analyseinputnameline[1]).'","'.htmlentities($analyseinputnameline[2]).'","'.htmlentities($analyseinputnameline[3]).'","'.htmlentities($analyseinputnameline[4]).'","'.htmlentities($analyseinputnameline[5]).'","'.htmlentities($analyseinputnameline[6]).'","'.htmlentities($analyseinputnameline[6]*1.3).'")';
...
<body bgcolor="a8c6fe">
<form id="form_67341" class="appnitro" action="insertionpricereceptiondirectliquidation.php" method="post">
