...
if(isset($_POST['analyseinput']))
{
		...
		foreach($analyseinputnametab as $analyseinputname) 
		{
			$analyseinputnameline=explode("\t", $analyseinputname);
				...
				$sql = 'INSERT INTO `oc_product_analyse` (title,upc,pricecost,Amazonprice,Amazondemand,AmazonRank,Shippingcost,AmazonProfit,';
				$sql = $sql.'eBayPrice,eBayDemand,eBayAmazonrank,eBayProfit,walmartprice,GoogleSearchPhrase,GoogleSearchVolume,GoogleCPC,';
				$sql = $sql.'GoogleCompetition,Brand,Model,Color,Width,Height,Length,Weight,Description,EAN,ASIN,imageurl)';
				$sql = $sql.'values ("';
						$i=0;
				//echo $analyseinputnameline[0]."<br>";
				for($i=0; $i<=49; $i++) 
				{
					if($i==0 || $i==1 || $i==6 ||$i==7 || $i==8 || $i==10 || $i==15 || $i==16 || $i==17 || $i==18 || $i==20 ||  $i==25 || $i==26 ||($i>33 && $i!=46 ))
					{
						
							$sql=$sql.addslashes($analyseinputnameline[$i]);
							if($i==49){
							$sql=$sql.'")';	
							}else{
								$sql=$sql.'","';	
...
<body bgcolor="a8c6fe">
<form id="form_67341" class="appnitro" action="insertionpriceanalyse.php" method="post">
<div class="form_description">
<h1>Insertion Analyse produit</h1>

<h3><label class="description" for="categorie">Analyse :</label></h3>
