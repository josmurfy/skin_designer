		...
		foreach($analyseinputnametab as $analyseinputname) 
		{
			$analyseinputnameline=explode("\t", $analyseinputname);
				...
				//echo $analyseinputnameline[0]."<br>"; 
						    ...
						    $price=floatval(ltrim($analyseinputnameline[9], '$'));
							$sql=$sql.addslashes($analyseinputnameline[0]).'","'.addslashes($analyseinputnameline[1]).'","'.addslashes($analyseinputnameline[3]).'","'.htmlentities($analyseinputnameline[6]).'","'.htmlentities($analyseinputnameline[7]).'","'.$price.'","'.$price*1.3.'","'.htmlentities($analyseinputnameline[7]).'")';
...
<body bgcolor="a8c6fe">
<form id="form_67341" class="appnitro" action="insertionpricereceptiondirectliquidation2.php" method="post">
<div class="form_description">
<h1>Insertion Reception DIRECT LIQUIDATION</h1>
