...
// savoir ledernier id 
		...
		$sql = 'SELECT * FROM `oc_product`,`oc_product_description`,oc_product_index,oc_product_to_category where oc_product.product_id=oc_product_to_category.product_id and oc_product.product_id=oc_product_description.product_id and oc_product_index.upc=oc_product.upc and oc_product.usa=1 and oc_product.inventaire=1 and oc_product.quantity>0 and oc_product.status=1 group by  oc_product_index.upc order by oc_product_to_category.category_id desc';
//echo $sql;
...
<?while($data = mysql_fetch_assoc($req)){
	...
	<td bgcolor="<?echo $bgcolor;?>"> 
	<?echo number_format((float)$data['pricedetailusd']*1.3, 2, '.', '');?>
	</td>

	<td bgcolor="<?echo $bgcolor;?>">
	<?echo number_format((float)$data['price']*1.3, 2, '.', '');?>
	</td>
	<td bgcolor="<?echo $bgcolor;?>">
	<?echo $data['quantity'];?>
	</td>
