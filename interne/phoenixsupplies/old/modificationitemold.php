...
if ($_POST['name_product']!="" && $_POST['sku']!="" 
	&& $_POST['model']!="" && $_POST['manufacturer']!="" && $_POST['new']==1){
		$sql2 = 'UPDATE `oc_product` SET `inventaire`=1, `invoice`="'.$_POST['invoice'].'",`model`="'.strtoupper($_POST['model']).'", `upc`="'.$_POST['upc'].'"';
		$sql2 .=', `mpn` = "'.strtoupper($_POST['model']).'",`quantity` = quantity+0'.$_POST['quantity'].',`manufacturer_id` = "'.$_POST['manufacturer'].'"';
		$sql2 .=', `priceebaysold` ="'.$_POST['priceebaysold'].'", `priceterasold` = "'.$_POST['priceterasold'].'", `priceebaynow` ="'.$_POST['priceebaynow'].'"';
		$sql2 .=', `condition_id`= "'.$_POST['etat'].'",`location`="'.$_POST['location'].'" ,`weight`="'.$_POST['weight'].'",`height`="'.$_POST['height'].'"';
		...
		$req2 = mysql_query($sql2);
			...
			$description.='<h2>Description :</h2><strong>Title : </strong>'.strtoupper($_POST['name_product']).'<br><strong>Model : </strong>'.strtoupper($_POST['model']);
...
<h3>SKU <?if($new==1){?><a href="modificationitem.php" class="button--style-red">Changer d'item</a><?}?> <a href="interne.php" class="button--style-red">Retour au MENU</a></h3>
...
<?}?>
	...
	<table width="100%">
	  ...
	  <tr>
		<td><h3>Quantit&eacute; (<?echo $_POST['quantity'];?>): </h3></td>
		<td><input id="quantity"  type="text" name="quantity" value="" size="10" /></td>
