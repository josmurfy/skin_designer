...
if ($_POST['name_product']!="" && $_POST['sku']!="" && $_POST['new']==1){
	...
	//echo $sql;
		...
		$sql2 = 'UPDATE `oc_product` SET `inventaire`=1, `invoice`="'.$_POST['invoice'].'",`model`="'.strtoupper($_POST['model']).'",`color`="'.strtoupper($_POST['color']).'", `upc`="'.substr($_POST['sku'],0,12).'"';
		...
		$sql2 .=', `weight`="'.$_POST['weight'].'",`height`="'.$_POST['height'].'"';
		...
		//mettre a jour la description recu de algopix
			  		...
			  		$sql9 = 'update `oc_product_index` set description="'.addslashes($_POST['andescription']).'" where upc="'.substr ($_POST['sku'],0,12).'"';
			//echo $sql9;
...
if (isset($_POST['category_id']) && $_POST['category_id']!=""){
	...
	//echo $sql;
		...
		//if (isset($_POST['manufacturer_id'])==false){$_POST['manufacturer_id']=0;}
			...
			$description.='<strong>UPC : </strong>'.substr($_POST['sku'],0,12).'<br>';
...
if($erreurvide=="")	$sql2 = 'UPDATE `oc_product_description` SET description_mod=1,name="'.addslashes(strtoupper($_POST['name_product'])).'", description="'.$description.'",`condition`="'.$_POST['condition'].'",`accessory`="'.$_POST['accessory'].'",`test`="'.$_POST['test'].'" WHERE `product_id` ='.$_POST['product_id'];


			//echo $sql2;
			$req2 = mysql_query($sql2);
			if($_POST['status']==1 && $_POST['titreok']=="ok" && $_POST['modelok']=="ok" && $_POST['colorok']=="ok" && $_POST['descok']=="ok"){
			header("location: pretlisterusa.php?sku=".$_POST['sku']."&product_id=".$_POST['product_id']."&action=menulisting"); 
			}
		$new=1;
}
...
<h3>SKU <?if($new==1){?><a href="modificationitemusa.php" class="button--style-red">Changer d'item</a><?}?> 
<a href="interneusa.php" class="button--style-red">Retour au MENU</a> 
<a href="multiuploadusa.php?sku=<?echo $_POST['sku']?>" class="button--style-red">Menu Photo</a> 
<?if ($_POST['etat']==9)$default="default";?>
<a href="listingusa.php?sku=<?echo $_POST['sku']?>&action=<?echo $default;?>&etat=<?echo $_POST['etat'];?>&product_id=<?echo $_POST['product_id']?>" class="button--style-red">Menu Listing</a></h3>
...
<h3><label class="description" for="element_1">Description Trouvee du produit:</label>
		<input type="checkbox" name="descok" value="ok"<?if($_POST['descok']=="ok")echo "checked"?>/> VERIFIE?
		<textarea id="andescription" name="andescription" rows="10" cols="50"><?echo $_POST['andescription'];?></textarea> </h3>


		<table width="100%">
		  <tr>
			<td valign="top"><h3>Brand :<select name="manufacturer_id">
...
<table width="100%" >
		...
		<td><h3>Model : <input type="checkbox" name="modelok" value="ok"<?if($_POST['modelok']=="ok")echo "checked"?>/> VERIFIE?<input id="model" type="text" name="model" value="<?echo $_POST['model'];?>" maxlength="80" /></h3></td>
	  		 ...
	  		 <tr>
		<td><h3>Color : <input type="checkbox" name="colorok" value="ok"<?if($_POST['colorok']=="ok")echo "checked"?>/> VERIFIE?<input id="color" type="text" name="color" value="<?echo $_POST['color'];?>" maxlength="80" /></h3></td>
...
<input id="price" type="text" name="price" value="<?echo $_POST['price'];?>" size="10" /></h3></td>
	  ...
	  <tr>
		...
		<td><h3><a href="createsmallbarcode.php?product_id=<?echo $_POST['product_id'];?>" target="label" style="color:#ff0000"><strong>SMALL LABEL</strong></a> <a href="createbarcodeusa.php?product_id=<?echo $_POST['product_id'];?>" target="_blank" style="color:#ff0000"><strong>Creation LABEL</strong></a></h3></td>
	  </tr>
