...
//echo "ca passe encore";
			...
			//echo $_POST['manufacturerok'];
			//echo $_POST['sku'];
			$sql = 'SELECT count(oc_product.product_id) as total FROM `oc_product`,`oc_product_description` where oc_product.product_id=oc_product_description.product_id and inventaire=1 and description_mod <2 and usa=0 and quantity>0 order by price';
			//echo $sql;
			$req = mysql_query($sql);
			$data = mysql_fetch_assoc($req);
			echo $data['total']/2;
			$sql = 'SELECT * FROM `oc_product`,`oc_product_description` where oc_product.product_id=oc_product_description.product_id and inventaire=1 and description_mod <2 and usa=0 and quantity>0 order by price limit 1';
			//echo $sql;
			$req = mysql_query($sql);
			$data = mysql_fetch_assoc($req);
...
<form id="form_67341" class="appnitro" action="modificationitemtemp2.php" method="post">
