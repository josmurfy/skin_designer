...
$sql2 = 'SELECT * FROM `admin_paypal` WHERE (name="PhoenixSuppliesUSA" OR name="PhoenixSuppliesca" OR type_transaction="General Account Correction") AND type_transaction_ok=0 order by date_transaction';
$req2 = mysqli_query($db,$sql2); 
while ($data2 = mysqli_fetch_assoc($req2)){
...
while ($data2 = mysqli_fetch_assoc($req2)){
					...
					//echo $sql."<br>";
					$req = mysqli_query($db,$sql); $numrows=mysqli_insert_id($db);
					$mysqlierr=mysqli_error($req);if($mysqlierr!="")echo $mysqlierr."<br>";if($mysqlierr==""){$sql = 'UPDATE `admin_paypal` SET row_check="'.$numrows.'",to_table="admin_ebay_fee",type_transaction_ok=3 WHERE admin_paypal_id="'.$data2['admin_paypal_id'].'"';
					//echo $sql."<br>";
					$req = mysqli_query($db,$sql); $numrows=mysqli_insert_id($db);}
					$j++;
}
...
$j=0;
$sql2 = 'SELECT * FROM `admin_paypal` WHERE type_transaction="Website Payment" AND (from_email="nostalgik@gmail.com" 
		OR from_email="humanalana@gmail.com" OR from_email="lianehm@yahoo.com" OR from_email="info@minimomotivation.com" OR 
		from_email="paypal@shipshopboutique.com" OR from_email="dealerquebec@aol.com" OR from_email="rogerpaulin1977@gmail.com")
		AND item_title!="" AND type_transaction_ok=0 order by date_transaction';
		//echo "<br>".$sql2."<br>";
$req2 = mysqli_query($db,$sql2); 
while ($data2 = mysqli_fetch_assoc($req2)){
					$sql = 'INSERT INTO `admin_canuship_sales` (`admin_paypal_id`, `date_transaction`, `timezone`, `status`';
					$sql = $sql.', `item_id`, `item_title`, `quantity`, `gross`, `net`, `fee`, `sales_taxes`, `shipping_handling`';
					$sql = $sql.', `currency`, `balance_impact`, `name`, `shipping_address`, `state_province`, `country`, `type_transaction`) ';
					$sql = $sql.'VALUES ( "'.$data2['admin_paypal_id'].'", "'.$data2['date_transaction'].'", "'.$data2['timezone'].'"';
					$sql = $sql.', "'.$data2['status'].'", "'.$data2['item_id'].'", "'.$data2['item_title'].'"';
					...
					$req = mysqli_query($db,$sql); $numrows=mysqli_insert_id($db);
					//echo $sql."<br>";
					$mysqlierr=mysqli_error($req);if($mysqlierr!="")echo $mysqlierr."<br>";
					if($mysqlierr==""){
						$sql = 'UPDATE `admin_paypal` SET row_check="'.$numrows.'",to_table="admin_canuship_sales",type_transaction_ok=66 WHERE admin_paypal_id="'.$data2['admin_paypal_id'].'"';
					//echo $sql."<br>";
					$req = mysqli_query($db,$sql); $numrows=mysqli_insert_id($db);}
					$j++;
}
echo "CANUSHIP Sales:<p style=\"color:red\">".$j." Importée</p><br>";
$j=0;
$sql2 = 'SELECT * FROM `admin_paypal` WHERE (from_email="admin@earnref.com" OR from_email="lp.mathurin@hotmail.com" OR from_email="opeth_eternal@sbcglobal.net" OR(type_transaction="Website Payment" AND (to_email="info@phoenixsupplies.ca" OR item_title like "%PhoenixSupplies%"))) AND type_transaction_ok=0 order by date_transaction'; 
$req2 = mysqli_query($db,$sql2); 
//echo $sql2."<br>";
while ($data2 = mysqli_fetch_assoc($req2)){
					...
					if($mysqlierr!="")echo $mysqlierr."<br>";
					if($mysqlierr==""){
						$sql = 'UPDATE `admin_paypal` SET row_check="'.$numrows.'",to_table="admin_invoice_sales",type_transaction_ok=5 WHERE admin_paypal_id="'.$data2['admin_paypal_id'].'"';
...
while ($data2 = mysqli_fetch_assoc($req2)){
					...
					if($mysqlierr!="")echo $mysqlierr."<br>";
					if($mysqlierr==""){
						$sql = 'UPDATE `admin_paypal` SET row_check="'.$numrows.'",to_table="admin_website_sales",type_transaction_ok=6 WHERE admin_paypal_id="'.$data2['admin_paypal_id'].'"';
					//echo $sql."<br>";
					$req = mysqli_query($db,$sql); $numrows=mysqli_insert_id($db);}
					$j++;
}
echo "Website Sales:<p style=\"color:red\">".$j." Importée</p><br>";
 ...
 $j=0;
$sql2 = 'SELECT * FROM `admin_paypal` WHERE type_transaction="General Payment" AND to_email="mrktplc@outlook.com" AND gross>0 AND type_transaction_ok=0 order by date_transaction';
$req2 = mysqli_query($db,$sql2); 
while ($data2 = mysqli_fetch_assoc($req2)){
					$sql = 'INSERT INTO `admin_transfers` (`admin_paypal_id`, `date_transaction`, `timezone`, `status`';
					$sql = $sql.', `gross`, `net`, `currency`, `name`) ';
