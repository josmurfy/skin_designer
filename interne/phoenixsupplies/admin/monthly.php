...
if(date('Y-m-d', strtotime($date_select))>date('Y-m-d', strtotime($data2['date_transaction']))){// ||($_POST['year']<=$verfi_date_tmp[0] && $_POST['month']>$verfi_date_tmp[1])){
	$erreur="<br><p style=\"color:red\">Donnee manquante (".$data2['date_transaction'].")</p><BR><BR>";
}else{
	$erreur="Derniere mise a jour (".$data2['date_transaction'].")</p><BR><BR>";
}
...
<div class="divTableCell"><?echo $data['name'];?></div>
<div class="divTableCell"><?echo $data['currency'];?></div>
<div class="divTableCell"><?if($data['link']!=""){echo $data['link'];}else{echo "PayPal";}?></div>
<div class="divTableCell"><?if($data['currency']=="USD")echo "$ ".number_format($data['gross'],2);?></div>
<div class="divTableCell"><?if($data['currency']=="USD"){echo "$ ".number_format($data['gross']*$rate,2);}else{echo "$ ".number_format($data['gross'],2);}?></div>
<div class="divTableCell"><?if($data['country']=="CA")echo "$ ".number_format($data['sales_taxes'],2);?></div>
<div class="divTableCell"><?echo number_format($rate,2);?></div>
</div>
<?
}
if($data['currency']=="USD" && $data['name']=="eBay Inc Shipping"){
	$ebayus=$ebayus+$data['gross'];
	$ebayca=$ebayca+($data['gross']*$rate);
}
if($data['currency']=="USD" && $data['name']=="POSTPONY LLC"){
	$postponyca=$postponyca+($data['gross']*$rate);
	$postponyus=$postponyus+($data['gross']);
}
if($data['currency']=="USD" && ($data['name']=="SHIPPO" || $data['name']=="Shippo")){
	$shippoca=$shippoca+($data['gross']*$rate);
	$shippous=$shippous+($data['gross']);
}
$totalshipping=$totalshipping+$data['gross'];
$totalshippingcad=$totalshippingcad+($data['gross']*$rate);
...
<div class="divTableCell"></div>
<div class="divTableCell"></div>
<div class="divTableCell"><?echo "$ ".number_format($totalshipping,2);?> USD</div>
<div class="divTableCell"><?echo "$ ".number_format($totalshippingcad,2);?> CAD</div>
<div class="divTableCell"><?echo "$ ".number_format($taxes,2);?> CAD</div>
<div class="divTableCell"></div>
...
Shipping par Compagnie
<div class="divTable blueTable">
<div class="divTableHeading">
<div class="divTableRow">
<div class="divTableHead">eBay (USD)</div>
<div class="divTableHead">eBay (CAD)</div>
<div class="divTableHead">PostPony (USD)</div>
<div class="divTableHead">PostPony (CAD)</div>
<div class="divTableHead">Shippo (USD)</div>
<div class="divTableHead">Shippo (CAD)</div>
</div>
</div>
<div class="divTableBody">
<div class="divTableRow">
<div class="divTableCell"<?if($ebayus<0){echo 'style="background-color:green; color: white;"';}?>><?echo "$ ".number_format($ebayus,2);?> USD</div>
<div class="divTableCell"<?if($ebayca<0){echo 'style="background-color:green; color: white;"';}?>><?echo "$ ".number_format($ebayca,2);?> CAD</div>
<div class="divTableCell"<?if($postponyus<0){echo 'style="background-color:green; color: white;"';}?>><?echo "$ ".number_format($postponyus,2);?> USD</div>
<div class="divTableCell"<?if($postponyca<0){echo 'style="background-color:green; color: white;"';}?>><?echo "$ ".number_format($postponyca,2);?> CAD</div>
<div class="divTableCell"<?if($shippous<0){echo 'style="background-color:green; color: white;"';}?>><?echo "$ ".number_format($shippous,2);?> USD</div>
<div class="divTableCell"<?if($shippoca<0){echo 'style="background-color:green; color: white;"';}?>><?echo "$ ".number_format($shippoca,2);?> CAD</div>
</div>
</div>
...
<div class="blueTable outerTableFooter">
<div class="tableFootStyle">
<p style="color:red">VERIFICATION: (<?echo number_format(($totalshipping-$ebayus-$postponyus-$shippous),2);?>)</p>
</div>
</div>
</div>
<?
...
<div class="divTableRow">
<div class="divTableCell"><?echo $data['date_transaction'];?></div>
<div class="divTableCell"><?echo $data['status'];?></div>
<div class="divTableCell"><?echo $data['type_transaction'];?></div>
<div class="divTableCell"><?echo $data['name'];?></div>
<div class="divTableCell"><?echo $data['currency'];?></div>
<div class="divTableCell"><?if($data['link']!=""){echo $data['link'];}else{echo "PayPal";}?></div>
<div class="divTableCell"><?if($data['currency']=="USD")echo "$ ".number_format($data['net'],2);?></div>
<div class="divTableCell"><?if($data['currency']=="USD"){echo "$ ".number_format($data['net']*$rate,2);}else{echo "$ ".number_format($data['net'],2);}?></div>
<div class="divTableCell"><?echo number_format($rate,2);?></div>
</div>
<?
}

$totalshipping=$totalshipping+$data['net'];
$totalshippingcad=$totalshippingcad+($data['net']*$rate);
...
<?
$datas=array();
$datatmp=get_transaction($_POST['year'],$_POST['year'],$_POST['month'],$_POST['month'],"purchases_paypal","",$db);
//print("<pre>".print_r ($datas,true )."</pre>");
foreach($datatmp as $data){
	$date_transaction=$data['date_transaction'];
	$datas[$date_transaction]['paypal']=$data;
	$sql2 = 'select * FROM `admin_purchases_cc` WHERE date_transaction="'.$data['date_transaction'].'"';
					//echo $sql."<br>";
	$req2 = mysqli_query($db,$sql2);
	$data2 = mysqli_fetch_assoc($req2);
	$datas[$date_transaction]['credit']=$data2;
}
//print("<pre>".print_r ($datas,true )."</pre>");
$sales=0;
$salescad=0;
...
<?
foreach($datas as $data){
	$date_tmp=explode(" ",$data['paypal']['date_transaction']);
	$date_info=$date_tmp[0];
	$rate=get_fxrate($date_info,$db)*1.04;
?>
<div class="divTableRow">
<div class="divTableCell"><?echo $data['paypal']['date_transaction'];?></div>
<div class="divTableCell"><?echo $data['paypal']['item_title'];?></div>
<div class="divTableCell"><?echo $data['paypal']['type_transaction'];?></div>
<div class="divTableCell"><?echo $data['paypal']['name'];?></div>
<div class="divTableCell"><?echo $data['paypal']['currency'];?></div>
<div class="divTableCell"><?if($data['credit']['admin_purchases_cc_id']!=""){if($data['credit']['currency']=="USD"){echo "Carte Credit USD";}else{echo "Carte Credit CAD";}}else{echo "PayPal";}?></div>
<div class="divTableCell" <?if($data['credit']['currency']!="" && $data['paypal']['currency']=="USD" && ($data['paypal']['gross']<>-($data['credit']['gross']))){echo 'style="background-color:red; color: white;"';}?>><?if($data['paypal']['currency']=="USD"){echo "$ ".number_format($data['paypal']['gross'],2);}elseif($data['credit']['currency']=="USD"){echo "$ ".number_format(-$data['credit']['gross'],2);}?>
<?if($data['credit']['currency']!="" && $data['paypal']['currency']=="USD" && ($data['paypal']['gross']<>-($data['credit']['gross']))){echo "<br>(".number_format($data['paypal']['gross']+$data['credit']['gross'],2).")";}?>
</div>
<div class="divTableCell"><?if($data['paypal']['currency']=="USD"){echo "$ ".number_format($data['paypal']['gross']*$rate,2);}else{echo "$ ".number_format($data['paypal']['gross'],2);}?></div>
<div class="divTableCell"><?if($data['paypal']['country']=="CA")echo "$ ".number_format($data['paypal']['sales_taxes']*$rate,2);?></div>
<div class="divTableCell"><?if($data['paypal']['country']=="CA")echo $data['paypal']['state_province'];?></div>
<div class="divTableCell"><?echo number_format($rate,2);?></div>
</div>
<?
if($data['paypal']['currency']=="USD"){
	$sales=$sales+$data['paypal']['gross'];
}elseif($data['credit']['currency']=="USD"){
	//echo "oui";
	$sales=$sales-$data['credit']['gross'];
	}
if($data['paypal']['currency']=="USD")$salescad=$salescad+($data['paypal']['gross']*$rate);
if($data['paypal']['currency']=="CAD")$salescad=$salescad+($data['paypal']['gross']);
$fee=$fee+($data['paypal']['fee']*$rate);
if($data['paypal']['country']=="CA")$taxes=$taxes+($data['paypal']['sales_taxes']*$rate);

if($data['state_province']=="QC")$txqc=$txqc+($data['paypal']['sales_taxes']*$rate);
if($data['state_province']=="ON")$txon=$txon+($data['paypal']['sales_taxes']*$rate);
if($data['state_province']=="BC")$txbc=$txbc+($data['paypal']['sales_taxes']*$rate);
if($data['state_province']=="AB")$txab=$txab+($data['paypal']['sales_taxes']*$rate);
if($data['state_province']=="SK")$txsk=$txsk+($data['paypal']['sales_taxes']*$rate);
if($data['state_province']=="MB")$txmb=$txmb+($data['paypal']['sales_taxes']*$rate);
if($data['state_province']=="NS")$txns=$txns+($data['paypal']['sales_taxes']*$rate);
if($data['state_province']=="NB")$txnb=$txnb+($data['paypal']['sales_taxes']*$rate);
if($data['state_province']=="NL")$txnl=$txnl+($data['paypal']['sales_taxes']*$rate);
if($data['state_province']=="YK")$txyk=$txyk+($data['paypal']['sales_taxes']*$rate);
if($data['state_province']=="PEI")$txpei=$txpei+($data['paypal']['sales_taxes']*$rate);
if($data['state_province']=="NWT")$txnwt=$txnwt+($data['paypal']['sales_taxes']*$rate);
}?>
<div class="divTableRow">
<div class="divTableCell"></div>
<div class="divTableCell"></div>
