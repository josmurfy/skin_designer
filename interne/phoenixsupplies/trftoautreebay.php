...
$connectionapi['APIEBAYURL']='https://api.ebay.com/ws/api.dll';
	...
	$sql = "SELECT * FROM `oc_product` WHERE `ebay_id_old` LIKE 'ERREUR' and `ebay_id` NOT LIKE '' AND quantity >0 order by product_id DESC ";
	...
	$req = mysqli_query($db,$sql);
			...
			while($data_import = mysqli_fetch_assoc($req))
			{
				...
				$json=add_ebay_item($connectionapi,$ebayresult,$data_import,$db);
				if(isset($json['ItemID'])&& $json['ItemID']!=""){
					$new_ebay_id=$json['ItemID'];
					$sql2 = 'UPDATE `oc_product`SET ebay_id="'.$new_ebay_id.'",ebay_id_old="'.$data_import['marketplace_item_id'].'" WHERE `oc_product`.`product_id` ='.$data_import['product_id'];
					echo $sql2.'<br><br>';
					$req2 = mysqli_query($db,$sql2);
					echo $data_import['product_id']."<br>";
				}else{
					$sql2 = 'UPDATE `oc_product`SET ebay_id_old="ERREUR" WHERE `oc_product`.`product_id` ='.$data_import['product_id'];
					$req2 = mysqli_query($db,$sql2);
		...
		//revise_ebay_product($connectionapi,'294401135207',101,2,$db,"non");
			...
			/* while($data_import = mysqli_fetch_assoc($req))
			{
					...
					import_ebay($connectionapi,$data_import['marketplace_item_id'],$data_import['product_id'],$db);
		...
		//revise_ebay_product($connectionapi,'294401135207',101,2,$db,"non");
			...
			while($data_import = mysqli_fetch_assoc($req))
			{
					...
					end_to_ebay($connectionapi,$data_import['marketplace_item_id']);
	 ...
	 if(($piclink!="") ){ //|| $piclink!=""
		...
		$image = file_get_contents($picexterne2);
						...
						if($principal==1) {
							...
							$dir_name=$SITE_ROOT."/".$uploads_dir."/".$product_id.$rdproduct_id.$pos;
...
/* 		 if(isset($post['lien_a_cloner'])&&$post['lien_a_cloner']!=""){
	...
	//print("<pre>".print_r ($json,true )."</pre>"); 
		...
		//$json['Item']['Title']=substr($json['Item']['Title'],0,80);
			...
			//unset ($json['Item']['SKU']);
			$json['Item']['SKU']="COM_".$json['Item']['SKU'];
