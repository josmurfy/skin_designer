...
if(!isset($_GET['order']))$_GET['order']='P.product_id';//P.location
...
if(isset($_GET['ebayerrorprix'])){
	...
	$_GET['order4']='((P.quantity+P.unallocated_quantity)<>P.ebay_quantity) ';
	//((P.quantity+P.unallocated_quantity)<>P.quantity)||
}
//echo "allo";
//echo $_POST['process2'];
//echo "allo";
...
if(isset($_POST['majancientinventaire'])){
		foreach($_POST['majancientinventaire'] as $majancientinventaire)  
			{	
				$sql = 'SELECT P.location as location_entreprise,P.quantity,P.unallocated_quantity AS unallocated_quantity 
				FROM `oc_product` AS P 
				WHERE  P.product_id="'.$majancientinventaire.'"';
				$req = mysqli_query($db,$sql);
				$data = mysqli_fetch_assoc($req);
				//echo $sql.'<br><br>';
				if($data['quantity']<>$data['quantity_total'] && $data['quantity']>0 && $data['unallocated_quantity']==0){
				//	echo $sql2.'<br><br>quantity<br>';
					$sql2 = 'UPDATE `oc_product` SET quantity="'.$data['quantity_total'].'" where product_id="'.$majancientinventaire.'"'; 
				//echo $sql2.'<br><br>';
				$req2 = mysqli_query($db,$sql2);  
				}elseif($data['unallocated_quantity']<>$data['quantity_total'] && $data['unallocated_quantity']>0 && $data['quantity']==0){
					//echo $sql2.'<br><br>unallocated_quantity<br>';
					$sql2 = 'UPDATE `oc_product` SET unallocated_quantity="'.$data['quantity_total'].'" where product_id="'.$majancientinventaire.'"'; 
				//echo $sql2.'<br><br>';
				$req2 = mysqli_query($db,$sql2);  
				}elseif($data['unallocated_quantity']<>0 && $data['quantity']<>0){
					//echo $sql2.'<br><br>quantity_des deux<br>';
					if($data['location_entreprise']!=""){
					$sql2 = 'UPDATE `oc_product` SET quantity="'.$data['quantity_total'].'" where product_id="'.$majancientinventaire.'"'; 
				//echo $sql2.'<br><br>';
				$req2 = mysqli_query($db,$sql2);  
					$sql2 = 'UPDATE `oc_product` SET unallocated_quantity="0" where product_id="'.$majancientinventaire.'"'; 
				//echo $sql2.'<br><br>';
				$req2 = mysqli_query($db,$sql2);  
					}else{
						$sql2 = 'UPDATE `oc_product` SET quantity="0" where product_id="'.$majancientinventaire.'"'; 
						//echo $sql2.'<br><br>';
						$req2 = mysqli_query($db,$sql2);  
							$sql2 = 'UPDATE `oc_product` SET unallocated_quantity="'.$data['quantity_total'].'" where product_id="'.$majancientinventaire.'"'; 
						//echo $sql2.'<br><br>';
				...
				//$sql2 = 'UPDATE `oc_product` SET location="",quantity="'.$updquantity.'",ebay_last_check="2020-09-01" where product_id="'.$majancientinventaire.'"'; 
				//echo $sql2.'<br><br>';
				//$req2 = mysqli_query($db,$sql2);  
			}
				//$sql2 = 'UPDATE `oc_product` SET location="" where quantity=0';
...
if(isset($_POST['process2'])){
		$nbmaj=0;
		$sql = 'SELECT *,P.sku AS psku,P.quantity_anc,p.unallocated_quantity,P.location 
		AS location_anc,P.quantity,P.location 
		AS location_entrepot FROM `oc_product` AS P,`oc_product_description` AS PD
		where P.product_id=PD.product_id    
		AND P.ebay_id >"0" and PD.language_id="1" and (P.quantity+P.unallocated_quantity)<>ebay_quantity ';
//echo $sql.'<br><br>';
...
if(isset($_POST['get_ebay_inventory'])){
	$nbmaj=0;
	/*$sql = 'SELECT * from oc_product where  `ebay_id` =0 and quantity>0 order by product_id limit 1';
	//and `location` not like "OLD_%" quantity>0 and and `product_id`="60"
echo $sql.'<br><br>';
	// on envoie la requête
	$req = mysqli_query($db,$sql);*/
	//	while($data = mysqli_fetch_assoc($req)){
			//2022-01-01T00:00:00.005Z
			$minusdate=-840;
		for($j=$minusdate;$j<=0;$j=$j+120){
			echo "Loop:".$j."<br><br>";
			$StartTimeTo = new DateTime('now');
			$StartTimeTo->modify(($j+120).' day'); // or you can use '-90 day' for deduct
			$StartTimeTo = $StartTimeTo->format('Y-m-d');

			$StartTimeFrom = new DateTime('now');
			$StartTimeFrom->modify($j.' day'); // or you can use '-90 day' for deduct
			$StartTimeFrom = $StartTimeFrom->format('Y-m-d');
			$limit=200;
			$page=1;
			//echo "StartTimeTo: ".$StartTimeTo."<br>StartTimeFrom: ".$StartTimeFrom."<br>";
				$result=get_ebay_inventory($connectionapi,$StartTimeFrom,$StartTimeTo,$limit,$page);
				$json = json_decode($result, true);
				$nb_page=$json['PaginationResult']['TotalNumberOfPages'];
				echo "Nb_Pages: ".$nb_page."<br>";
				//$nb_page=1;
				for($i=1;$i<=$nb_page;$i++){
					$result=get_ebay_inventory($connectionapi,$StartTimeFrom,$StartTimeTo,$limit,$i);
					$json = json_decode($result, true);
					foreach($json['ItemArray']['Item'] as $Item){
						$dateverif = date('Y-m-d');
							$dateendtab= explode("T",$Item['ListingDetails']['EndTime']);
					//		echo "Date Verif: ".$dateverif."<br>Date End: ".$dateendtab[0]."<br>";
							$dateend = $dateendtab[0];
							//$dateverif="2022-02-21";
							//$dateend = date('Y-m-d',$dateendtab[0]);
													if ($dateverif > $dateend )
							{
							echo '<span style="color:RED;"><strong>ENDED<br>';
							echo 'Product_id:'.$Item['SKU'].'<br>Date End: '.$dateend.'<br><br></strong></span>';
							}
							else
							{
								if(strpos($Item['SKU'],'COM_',0)!==false){
									echo '<span style="color:GREEN;"><strong>Commercial product_id:'.$Item['SKU'].'<br>';
									echo "Date Verif: ".$dateverif."<br>Date End: ".$dateend."<br>";

									$product_id=str_replace('COM_','',$Item['SKU']);
									$sql2 = 'UPDATE `oc_product`SET ebay_id="'.$Item['ItemID'].'",ebay_price="'.$Item['SellingStatus']['CurrentPrice'].'",ebay_quantity="'.$Item['Quantity'].'",ebay_last_check="2020-09-01" WHERE `oc_product`.`product_id` ="'.$product_id.'"';
										echo $sql2.'<br><br></strong></span>';
										$nbmaj++;
									$req2 = mysqli_query($db,$sql2);
								}else{
									$skutab=explode("_",$Item['SKU']);
									$Item['SKU']=$skutab[0];
									//echo "Retail product_id:".$Item['SKU']."<br><br>";
								} 
							}
	...
	//	}
echo '<span style="color:RED;"><strong>Nb produit traiter effectuer:'.$nbmaj.'</strong><br><br></span>';
}	
...
if(isset($_POST['check_ebay'])){
					...
					$dateverification->modify('-30 day'); // or you can use '-90 day' for deduct
...
//while($data = mysqli_fetch_assoc($req)){
	...
	echo "Nb a verifier:".count($ebay_ids)."<br><br>";
		if(count($ebay_ids)>0){
...
$sql = 'SELECT count(*) as nb_item FROM `oc_product` AS P 
LEFT JOIN `oc_product_description` AS PD on P.product_id=PD.product_id 
 
 
where ((('.$_GET['order4'].') and P.ebay_id >"0") '.$dateverification_slq.') and PD.language_id=1'.$_GET['order3'].' 
Order by '.$_GET['order'].' '.$_GET['order2'].''; //and P.ebay_id=292612778604';  $sql.'<br><br>';
// on envoie la requête
//echo $sql."<br><br>";
$req = mysqli_query($db,$sql);
$data = mysqli_fetch_assoc($req);
echo "Nombre item sur ebay avec erreur:".$data['nb_item']."<BR><BR>";

$sql = 'SELECT *,P.quantity_anc,p.unallocated_quantity,P.location 
AS location_anc,P.quantity,P.location 
...
<body bgcolor="ffffff">
<form id="form_67341" class="appnitro" action="inventairesyncebay.php" method="post" >
<input id="saveForm" class="button_text" type="submit" name="submit" value="Submit" /><br>
Check Qty Ebay<input type="checkbox" name="check_ebay" value="check_ebay"/><br>
====================================================<br>
...
====================================================<br>
TRANSFERT Entrepot<input type="checkbox" name="transfert_location_entrepot" value="transfert_location_entrepot"/><br>
Get_ebay_inventory <input type="checkbox" name="get_ebay_inventory" value="get_ebay_inventory"/><br>
====================================================<br>
...
<h3><label class="description" for="categorie">INVENTAIRE Declaré sur ebay: <?if ($_GET['imp']=="oui"){ echo "(Page: ".$pagenu.")"; $pagenu++;}?></label></h3>
	<a href="inventairesyncebay.php?order3=oui&order=P.location" >ENTREPOT</a> <a href="inventairesyncebay.php?verifier=oui&order=P.location" >A vérifier</a> <a href="inventairesyncebay.php?ebayerrorprix=oui&order=P.location" >Probleme de prix</a>
<table border="1" width="100%">
	...
	<th bgcolor="ff6251">
	<a href="inventairesyncebay.php?order2=<?echo $_GET['order2'];?>&order=P.sku" >SKU</a>
	</th>
		<th bgcolor="ff6251">
		<a href="inventairesyncebay.php?order2=<?echo $_GET['order2'];?>&order=PD.name" >Titre</a>
	</th>
	
	<th bgcolor="ff6251">
	<a href="inventairesyncebay.php?order2=<?echo $_GET['order2'];?>&order=P.quantity" >Qty Total</a>
	
	</th>
	<th bgcolor="ff6251">
	<a href="inventairesyncebay.php?order2=<?echo $_GET['order2'];?>&order=P.location" >Entrepot</a>
	
	</th>
	<th bgcolor="ff6251">
	<a href="inventairesyncebay.php?order2=<?echo $_GET['order2'];?>&order=P.quantity" >Qty</a>
	
	</th>
	
	<th bgcolor="ff6251">
	<a href="inventairesyncebay.php?order2=<?echo $_GET['order2'];?>&order=P.unallocated_quantity" >A Placer</a>
	
	</th>
	<th bgcolor="ff6251">
	<a href="inventairesyncebay.php?order2=<?echo $_GET['order2'];?>&order=P.ebay_id" >Ebay-ID</a>
	
	</th>
	<th bgcolor="ff6251">
	<a href="inventairesyncebay.php?ebayerror=<?echo $_GET['ebayerror'];?>&order=P.ebay_quantity" >Qty eBay</a>
	
	</th>
	<th bgcolor="ff6251">
	<a href="inventairesyncebay.php?order2=<?echo $_GET['order2'];?>&order=P.ebay_last_check" >Dernier MAJ</a>
...
<?
	...
	?>
		...
		<?if ($data['remarque_interne']!=""){?>
								<br>
								<a href="modificationitem.php?product_id=<?echo $data['product_id'];?>" target="modif" >A vérifier</a>
		<?}?>
						</td>
...
JsBarcode(".barcode").init();
						...
						<td <?if((($data['quantity']+$data['unallocated_quantity'])<>$data['quantity_anc']) || !isset($data['quantity']) || $data['quantity_anc']<0){?>style="background-color: red; color: white;"<?}?>>
						<?if(($data['quantity']+$data['unallocated_quantity'])<>$data['quantity_anc'] || $data['quantity_anc']<0){?><input type="checkbox" name="majancientinventaire[]" value="<?echo $data['product_id'];?>"/><?}?>
						<?echo $data['quantity_anc'];?>

						</td>
						<td <?if(($data['quantity']<>$data['quantity_anc'] && $data['location_magasin']=="") ){?>style="background-color: yellow;"<?}elseif(strpos($data['location_entrepot'],"agasin") || !isset($data['quantity'])){?>style="background-color: red; color: white;"<?}?>>
						<?echo $data['location_entrepot'];?>
						</td>
						<td <?if(($data['quantity']<>$data['quantity_anc'] && $data['location_magasin']=="") ){?>style="background-color: yellow;"<?}elseif(strpos($data['location_entrepot'],"agasin") || !isset($data['quantity'])|| $data['quantity']<0){?>style="background-color: red; color: white;"<?}?>>
						...
						?>
						<td <?if(!isset($data['unallocated_quantity'])|| $data['unallocated_quantity']<0){?>style="background-color: red; color: white;"<?}?>>
...
/interne/phoenixsupplies//inventairesyncebay.php" >Orders Business</a></h1>
