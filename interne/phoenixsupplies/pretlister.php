...
if($_POST['sku_old']==$_POST['sku']){
	...
	/* 		if($_POST['quantity_actuel']<>($data2['quantity']+$_POST['quantitymagasin_ajouter']+$_POST['unallocated_quantity'])){
				...
				$updquantity=$_POST['quantityentrepot_ajouter']+$_POST['quantity'];
	/* 		} */
			$_POST['quantitymagasin_ajouter']=0;
			$_POST['quantityentrepot_ajouter']=0;
			$status=1;
	...
	//	
		if(count($data)>0){
			...
			//echo $sql4;
					...
					//		//print("<pre>".print_r ($data,true )."</pre>");
							$info_shipping=get_shipping ($connectionapi,$data['weight'],$data['length'],$data['width'],$data['height'],$db,(string)$data['upc'],12919,$_POST['category_id']);//,$data);
						}
					//	$info_shipping=get_shipping ($connectionapi,$data['weight'],$data['length'],$data['width'],$data['height'],$db,(string)$data['upc'],12919,$_POST['category_id']);
...
/* 					} */
					...
					if($data['price']<0){
						$_POST['price']=$data['price_with_shipping'];
					}else{
...
/* 			if($dateverification > $dateretail)  print_r($dateretail);
			...
			//	$_POST['suggest']=$price_replace[0]+$endprix;
				...
				}elseif($_POST['suggest']<0){
					$_POST['price']=$_POST['price_with_shipping'];
...
<body bgcolor="ffffff">
<form id="form_67341" class="appnitro" action="pretlister.php" method="post">
<?/*<h3>
 ...
 <table style="text-align: left; width: 1000px; height: 50%; margin-left: auto; margin-right: auto;" border="1" cellpadding="2" cellspacing="2">
    <tbody>
      ...
      <tr>
		...
		<input id="quantitymagasin_ajouter"  class="element text currency" type="text" name="quantitymagasin_ajouter" value="" size="10" />       
		</td>
		        <td style="vertical-align:  middle; text-align:center;height: 25px; ">
				<?$_POST['location_magasin']=strtolower($_POST['location_magasin']);
					$user= '';
		...
		Location:
		 ...
		 <td colspan="1" rowspan="1" style="vertical-align:  middle; text-align: center; height: 0px; ">
		<input id="quantityentrepot_ajouter"  class="" type="text" name="quantityentrepot_ajouter" value="" size="10"  autofocus />		
		</td>
        <td style="vertical-align:  middle; text-align:center;height: 25px; ">
...
<tr>
		...
		if($_POST['price']=$_POST['price_with_shipping'])$_POST['price']=0;?>
		<?if(($dateverification > $dateretail)){?>background-color: red; color: white;<?}else{?>background-color: green; color: white;<?}?>
		height: 0px; ">
     ...
     <tr>
	  ...
	  <td style="vertical-align:  middle; text-align:center;height: 0px; background-color: #030282; color: white; width: 200px;">
		...
		<td colspan="2" rowspan="1" style="vertical-align: middle; text-align:center;<?if(($_POST['suggest']<$_POST['price'])||($dateverification > $datemagasin)){?>background-color: red; color: white;<?}else{?>background-color: green; color: white;<?}?>  height: 0px; ">
...
<br>
 ...
 <tr>
        ...
        <?if($_POST['etsy_id']==0||$_POST['etsy_id']==""){?>
		<td colspan="2" rowspan="1" style="background-color:red; color: white;vertical-align:  middle; text-align:center; height: 0px; ">
		<?}else{?><td colspan="2" rowspan="1" style="background-color:green; color: white;vertical-align:  middle; text-align:center; height: 0px; ">
			<a href="https://www.etsy.com/your/shops/me/listing-editor/edit/<?echo $_POST['etsy_id'];?>/about" target='etsy' ><?echo $_POST['etsy_id'];?></a>
<br>
		<?}?>  
		</td>
</tr>
 <?if($_POST['etsy_id']==0||$_POST['etsy_id']==""){?>
			<tr>
		 <td style="vertical-align:  middle; text-align:center; height: 0px; background-color: #030282; color: white; width: 200px;">
			etsy ID &agrave; cloner: 
        </td>
        <td colspan="2" rowspan="1" style="background-color:red; color: white;vertical-align:  middle; text-align:center; height: 0px; ">
	<?if(($_POST['etsy_id_old']!="" || $_POST['etsy_id_old']!=0) && $_POST['etsy_id_a_cloner']=="")
			$_POST['etsy_id_a_cloner']=$_POST['etsy_id_old'];?>
			<a href="pretlister.php?listetsy=oui&product_id=<?echo $_POST['product_id'];?>&sku=<?echo $_POST['sku'];?>" >Lister sur Etsy</a>
