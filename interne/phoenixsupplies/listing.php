...
if(isset($_GET['product_id'])){
	$_POST['sku']=$_GET['product_id'];
}
$_POST['sku']=isset($_POST['sku'])?(string)$_POST['sku']:null ; 
...
if(isset($_GET['changeupc']) && $_GET['changeupc']=="oui"){
		//if($_POST['sku']!="" || !isset($_POST['sku'])){
			$_POST['sku']=getRandomSku($db);
			//}

					$sql2 = 'UPDATE `oc_product`SET sku="'.$_POST['sku'].'",ebay_last_check="2020-09-01" WHERE `oc_product`.`product_id` ="'.$_GET['product_id'].'"';
					echo $sql2."<br>";
					$req2 = mysqli_query($db,$sql2);
					echo '<script>window.open("createbarcodetotal.php?type=both&qtemag=0&qtetot=1&product_id='.$_GET['product_id'].'&sku='.(string)$_POST['sku'].'","etiquette")</script>';
			if(isset($_GET['pagesource'])){
				echo '<script>window.open("trf_inventaire_location.php?anc_loc='.$_GET['anc_loc'].'","_self")</script>';
			}
...
if ($_GET['sku']!=""){
	...
	//echo 'allo';
			...
			$sql = 'SELECT * FROM `oc_product` AS P  where (P.quantity>0) AND P.ebay_id=0 order by P.product_id desc';
...
if(isset($_POST['nonlister']) && $_POST['nonlister']=="oui"){
			//echo $sql;
			if($_POST['nb_nonlistersurebay']>1){
				$choix=mt_rand ( 1 , $_POST['nb_nonlistersurebay']-1 );
			}else{
				$choix=$_POST['nb_nonlistersurebay'];
			}
			$sql = 'SELECT * FROM `oc_product` AS P  where (P.quantity>0) AND P.ebay_id=0 order by P.product_id desc';
			
			$req = mysqli_query($db,$sql);
			$product = mysqli_fetch_assoc($req);
			$_POST['sku'] =$product['sku'];
			//echo (string)$_POST['sku'];
}
if (isset($_POST['sku'] ) && (string)$_POST['sku'] !=""){
	//echo (strlen($_POST['sku']));
		/* if(strlen($_POST['sku'])<6){
			 $product=get_products_by_id($_POST['sku']);
		}else{ */
			//$product=get_products(substr((string)$_POST['sku'] ,0,12));
			if(strlen($_POST['sku'])<6){
				$product=get_products_by_id($_POST['sku']);
				
				$_POST['sku']=$product['sku'];
		   	}else{
				$product=get_product_by_sku((string)$_POST['sku']);
				$search="";
			//	//print("<pre>".print_r ($product,true )."</pre>");
				if(isset($product)==0){
					$product=get_products_by_search($_POST['sku']);
					$search="yes";
		...
		//	//print("<pre>".print_r ($product,true )."</pre>");
				...
				//	 exit();
						  ...
						  if($product['marketplace_item_id']>1){
						//	header('location: clonerproduct.php?etat='.$product['condition_id'].','.str_replace("_","",substr((string)$product['sku'] ,12,2)).'&sku='.(string)$product['sku'].'&product_id='.$product['product_id'].'&action=modificationitemusa'); 
							exit();
						  }
  ...
  <table style="text-align: left; width: 1053px; height: 50%; margin-left: auto; margin-right: auto;" border="1" cellpadding="2" cellspacing="2">
    ...
    <tbody>
      ...
      <tr>
		...
		<h1>Information Produit</h1><?if (isset($_POST['erreur_dimensions_poids']) && $_POST['erreur_dimensions_poids']!="") echo '<h3><font color="red">'.$_POST['erreur_dimensions_poids'].'</font></h3>'; ?>
        </td>
      </tr>
	  	<tr>
...
/* 				if (!file_exists($GLOBALS['SITE_ROOT']."image/catalog/product/".$product['product_id']."/") && $product['product_id']==21993) {
			...
			if($product['marketplace_item_id']>0){
				$ebayresult=get_ebay_product($connectionapi,$product['marketplace_item_id']);
			//	echo "allo";
				//unset($ebayresult['Item']['Description']);
				//$ebayresult->Item->Description="";
				$ebayresult=json_decode($ebayresult,true);
			//	//print("<pre>".print_r ($ebayresult,true )."</pre>");
				$ebay=$ebayresult['Item'];
			//	unset($ebay['Description']);
				//$ebayresult=json_encode($ebayresult);
			//	//print("<pre>".print_r (get_object_vars($ebay['ItemSpecifics']),true )."</pre>");
			}
			$info_supplementaire="";
		...
		//	$product['date_price_upd_magasin']=date_parse ($product['date_price_upd_magasin']);
		  ...
		  if($nu==0){
			  ...
			  $html='<tr>
				...
				<br><a href="listing.php?changeupc=oui&product_id='.(string)$_POST['product_id'].'">Corriger SKU</a>';
				}
				$html.='</td>
			...
			//	}else{
				  $html.=''."$ ".number_format($product['price_magasin'],2).' ';
				//  }
				  $html.='</td>		     
				</tr> 
			  <tr>
...
<td colspan="1" style="vertical-align:  middle; text-align: center; height: 16px; background-color: #e4bc03; width: 112px;"> 
				...
				<tr>
				  <td style="vertical-align:  middle; width: 160px;  background-color: #030282; color: white;">Unallocated quantity:<br>
				  </td>
				  <td colspan="2"style="vertical-align:  middle; text-align:center;width: 174px;">
				  ...
				  <td colspan="2"style="vertical-align:  middle;text-align:center;">'.$product['location'].'</td>
				</tr>
				<tr>
		...
		$html.='width: 33%;">Ebay ID: <a href="https://www.ebay.com/itm/'.$product['marketplace_item_id'].'" target="ebaynew">'.$product['marketplace_item_id'].'</a></td>
				...
				<tr>
				  ...
				  if ($product['image_product']!="")
					  $html.='<img src="https://www.phoenixsupplies.ca/image/'.$product['image_product'].'" width="150">';
				...
				<tr>
				  <td style="vertical-align:  middle; background-color: #030282; color: white;">Unallocated_quantity:<br>
				  </td>
			...
			a PLACER:<br>
				  ...
				  <td style="vertical-align:  middle;text-align:center;">'.$product['location'].'</td>
				</tr>
				<tr>
				  <td style="vertical-align:  middle; width: 160px;  background-color: #030282; color: white;">
				  Quantité entrep&ocirc;t:
  ...
  ?>
       <tr>
        <td colspan="1" rowspan="24" style="vertical-align:  middle; text-align: center;height: 24px; width: 342px;">
			<?if(isset($image_principal) && $image_principal!="")echo '<img src="https://www.phoenixsupplies.ca/image/'.$image_principal.'" width="200">
			<br>
	    ...
	    <td colspan="3" style="vertical-align:  middle; height: 16px; background-color: #e4bc03; width: 112px;"> 
		 ...
		 <input type="checkbox" name="nonlister" value="oui" <?if(isset($_POST['nonlister']) && $_POST['nonlister']=='oui')echo 'checked';?>/>	
		<input id="saveForm" class="button_text" type="submit" name="submit" value="Submit" /> 
		<br><a href="insertionitem.php?randomupc=oui" >Ajout d'item avec UPC aleatoire</a> 
		 <a href="insertionitem_ng.php?randomupc=oui" >Ajout National Geographic</a> 
