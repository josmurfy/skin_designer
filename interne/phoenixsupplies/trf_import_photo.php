...
$connectionapi['APIEBAYURL']='https://api.ebay.com/ws/api.dll';
	...
	$sql = "SELECT * FROM `oc_product` WHERE `ebay_id_old` NOT LIKE '0' AND `ebay_id_old` NOT LIKE 'ERREUR' AND 
	(product_id=1413 OR product_id=1412 OR product_id=1409 OR product_id=1427)
	...
	$req = mysqli_query($db,$sql);
			...
			while($data_import = mysqli_fetch_assoc($req))
			{
				...
				$json=import_ebay($connectionapi,$data_import['ebay_id_old'],$data_import['product_id'],$db);
				...
				//mise_en_page_description($connectionapi,$data_import['product_id'],$db);
				//revise_ebay_product($connectionapi,$data_import['marketplace_item_id'],$data_import['product_id'],$data_import['quantity'],$db,"non");
			unset($json);
...
function mise_en_page_description($connectionapi,$product_id,$db)
{
			...
			$prixcad=number_format($data9['price']*1.34, 2, '.', '');
			$prixusd=number_format($data9['price'], 2, '.', '');
			...
			$sql6 = 'SELECT * FROM `oc_product_to_category`,`oc_category_description` where language_id=1 and oc_product_to_category.category_id=oc_category_description.category_id and product_id="'.$product_id.'" and ebayyes=1';
			//echo $sql;
			$req6 = mysqli_query($db,$sql6);
			$data6 = mysqli_fetch_assoc($req6);
			$categoryname=$data6['name'];
			$sql6 = 'SELECT * FROM `oc_product_to_category`,`oc_category_description` where language_id=2 and oc_product_to_category.category_id=oc_category_description.category_id and product_id="'.$product_id.'" and ebayyes=1';
			//echo $sql;
			$req6 = mysqli_query($db,$sql6);
			$data6 = mysqli_fetch_assoc($req6);
			...
			$description.='<strong>Title : </strong>'.$name.'<br><strong>Model : </strong>'.strtoupper($data['model']);
			$descriptionf.='<strong>Titre : </strong>'.$namefr.'<br><strong>'.$modele.' : </strong>'.$data['model'];
			$description.='<br><strong>Brand : </strong>'.strtoupper($data2['name']).'<br>';
			$descriptionf.='<br><strong>Marque : </strong>'.$data2['name'].'<br>';
			if($data['color']=="")$data['color']="N/A";
			$description.='<strong>Color : </strong>'.strtoupper($data['color']).'<br>';
			$descriptionf.='<strong>Couleur : </strong>'.$data8['color'].'<br>';
			$description.='<strong>Shipping Dimension : </strong>'.doubleval ($data['length']).'x'.doubleval ($data['width']).'x'.doubleval ($data['height']).' Inch<br>';
			$descriptionf.='<strong>Dimension pour livraison : </strong>'.doubleval ($data['length']).'x'.doubleval ($data['width']).'x'.doubleval ($data['height']).' Inch<br>';
...
//echo $sql2;
			...
			$description.=addslashes(strtoupper($data2['condition_supp'])."<br>");
			$conditionname=strtoupper($data2['name']);
				...
				if($data['condition_supp']!="" && $data['condition_supp']!=","){
					...
					if(strpos($data['condition_supp'],",")===FALSE){
						$description.=addslashes('<font color="red"><strong>- '.$data['condition_supp'].'</strong></font><br>');
					}else{
						$conditionsupp=explode(',', $data['condition_supp']);
							foreach($conditionsupp as $conditioncheck){
								if($conditioncheck!=""){
									$description.=addslashes('<font color="red"><strong>- '.$conditioncheck.'</strong></font><br>');
									//echo $i;		
								}
							}	
					}
				...
				if($data['accessory']!="" && $data['accessory']!=","){
					...
					if(strpos($data['accessory'],",")===FALSE){
						$description.=addslashes('<font color="red"><strong>- '.$data['accessory'].'</strong></font><br>');
					}else{
				...
				if($data['test']!="" && $data['test']!=","){
					...
					if(strpos($data['test'],",")===FALSE){
						$description.=addslashes('<font color="red"><strong>- '.$data['test'].'</strong></font><br>');
					}else{
...
//echo $sql3;
			...
			$conditionnamefr=$data3['name'];
				if($data8['condition_supp']!="" && $data8['condition_supp']!=","){
					$descriptionf.='<b>Conditions Supplementaire:</b><br>';
				
					if(strpos($data8['condition_supp'],",")===FALSE){
						$descriptionf.=addslashes('<font color="red"><strong>- '.$data8['condition_supp'].'</strong></font><br>');
					}else{
						$conditionsupp=explode(',', $data8['condition_supp']);
							foreach($conditionsupp as $conditioncheck){
								if($conditioncheck!=""){
									$descriptionf.=addslashes('<font color="red"><strong>- '.$conditioncheck.'</strong></font><br>');
									//echo $i;		
								}
							}	
					}
...
//echo $descriptionf;
			...
			$sql2 = 'UPDATE `oc_product_description` SET tag="'.$conditionname.','. str_replace(' ', ',', addslashes(($name))).','.(string)$data['upc'].'",meta_keyword="'.$conditionname.','. str_replace(' ', ',', addslashes(strtoupper($name))).','.(string)$data['upc'].'", meta_title="'.$conditionname." ".addslashes(strtoupper($name)).'",name="'.addslashes(strtoupper($name)).'", description="'.$description.'", meta_description="Liquidation '.addslashes(strtoupper($categoryname.' '.$name)).' '.$conditionname.' at the lowest price of $'.$prixcad.' CAD or $'.$prixusd.' USD Model: '.$data['model'].' UPC:'.(string)$data['upc'].'" WHERE language_id=1 and `product_id` ='.$product_id;
//echo $sql2."<br>";
//'.strtoupper($keywordindex).' 
//'.strtoupper($keywordindex).',
			$req2 = mysqli_query($db,$sql2);
			//printf("Erreur : %s\n", mysqli_error($db));
			//echo "<br>";
...
function upload_from_ebay($product_id,$piclink,$principal,$db){
	...
	if ($piclink!="")$picexterne2=$piclink;	 
	 ...
	 if(($piclink!="") ){ //|| $piclink!=""
		...
		//echo '<br>'.$piclink;
		echo '<br>https://phoenixsupplies.ca/'.$product_id.$rdproduct_id.$pos.'<br>';
	 }
...
function revise_ebay_product_price($connectionapi,$ebay_id,$product_id,$updquantity,$db,$export_photo_to_ebay) {
			$result2= product_description_ebay($connectionapi,$product_id,$db,$export_photo_to_ebay);
			//echo $result2;
					if (is_numeric($updquantity))
					{
...
/* 		if($data['quantity_total']<($data['quantity']+$data['unallocated_quantity'])){
			...
			$updquantity=$data['quantity']+$data['unallocated_quantity'];
				...
				$result="";
				//if($inventaire==""){
					$result.="<Title>".htmlspecialchars(addslashes(substr($Name,0,80)))."</Title>";//substr($json['Item']['Title'],0,80);
