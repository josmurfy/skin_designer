		...
		foreach($_POST['product_select'] as $product_select)  
          ...
          //      $new_loc= str_replace('-','',$_POST['new_loc']);
			...
			//	echo $_POST['unallocated_quantity_hid'.$sku].'<br>';
				...
				$sql2 = 'UPDATE `oc_product`SET unallocated_quantity=unallocated_quantity-'.$_POST['quantity_val'.$sku].',quantity=quantity+'.$_POST['quantity_val'.$sku].',location="'.strtoupper($new_loc).'",anc_loc="'.strtoupper($_POST['anc_loc']).'" WHERE product_id ='.$product_select;
        	//	echo $sql2.'<br><br>';
				$req2 = mysqli_query($db,$sql2);  
				
			}
			if($_POST['createlabel']=="yes"){
				echo '<script>window.open("https://phoenixliquidation.ca/interne/createlabeltablette.php?tablette='.$new_loc.'","etiquette")</script>';
			}
    }
}
...
<form id="form_67341" class="appnitro" action="ADMIN_trf_location_bulk.php" method="post" enctype="multipart/form-data">
...
<?
		...
		where  PD.language_id=1  and P.unallocated_quantity <>0 and P.quantity <>0 and P.ebay_id>0 Order by name '; //LIMIT 200 and P.ebay_id=292612778604';  $sql.'<br><br>';/*P.username = "marissa"*/
...
<div  style="text-align: center;font-size: 15px; "><strong><?echo (string)$data['sku'] ;?></strong></div>
						...
						<a href="listing.php?product_id=<?echo $data['product_id'];?>&action=listing" target='listing' ><?echo $data['nameen'];?></a>
						</td>
						...
						<td style="vertical-align: middle; background-color: <?if($data['location_entrepot']!=""){echo "red";}else{echo $bgcolor;}?>;  text-align: center;">
