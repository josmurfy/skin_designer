...
$connectionapi['APIEBAYURL']='https://api.ebay.com/ws/api.dll';
	...
	$sql = "SELECT * FROM `oc_product` WHERE `price_with_shipping` <> `priceebayancienlisting` AND `ebay_id` NOT LIKE '0' AND `ebay_id` NOT LIKE '' AND quantity>0 AND priceebaynow=0 AND ebay_id_old NOT LIKE 'ERREUR' order by product_id ";// 
...
/* 				delete_photo($data_import['product_id'],"",$db);
				...
				$json=revise_ebay_product_price($connectionapi,$data_import['marketplace_item_id'],$data_import['product_id'],$data_import['priceebayancienlisting'],$db,"non");
				if(isset($json['Ack'])&& $json['Ack']=="Warning"){
					$new_ebay_id=$json['ItemID'];
					$sql2 = 'UPDATE `oc_product`SET price_with_shipping="'.$data_import['priceebayancienlisting'].'",priceebaynow="'.$data_import['price_with_shipping'].'" WHERE `oc_product`.`product_id` ='.$data_import['product_id']; 
					echo $sql2.'<br><br>';
...
function revise_ebay_product_price($connectionapi,$ebay_id,$product_id,$price,$db,$export_photo_to_ebay) {
