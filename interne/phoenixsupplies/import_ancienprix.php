...
$connectionapi['APIEBAYURL']='https://api.ebay.com/ws/api.dll';
	...
	$sql = "SELECT * FROM `oc_product` WHERE `ebay_id` NOT LIKE '' AND quantity >0 AND priceebayancienlisting=0 order by product_id DESC ";
	...
	$req = mysqli_query($db,$sql);
			...
			{
				$ebayresult=get_ebay_product($connectionapi,$data_import['ebay_id_old']);
				$json = json_decode($ebayresult, true);
				//print("<pre>".print_r ($json,true )."</pre>");
				//$json=add_ebay_item($connectionapi,$ebayresult,$data_import,$db);
				
				if(isset($json['Item']['StartPrice'])&& $json['Item']['StartPrice']!=""){
					$new_ebay_id=$json['ItemID'];
					$sql2 = 'UPDATE `oc_product`SET priceebayancienlisting="'.$json['Item']['StartPrice'].'",ebay_last_check="2020-09-01" WHERE `oc_product`.`product_id` ='.$data_import['product_id'];
					echo $sql2.'<br><br>';
					$req2 = mysqli_query($db,$sql2);
					echo $data_import['product_id']."<br>";
				}else{
					$sql2 = 'UPDATE `oc_product`SET ebay_id_old="ERREUR",ebay_last_check="2020-09-01" WHERE `oc_product`.`product_id` ='.$data_import['product_id'];
					//$req2 = mysqli_query($db,$sql2);
