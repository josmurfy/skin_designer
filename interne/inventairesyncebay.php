...
if(isset($_POST['process2'])){
		$nbmaj=0;
		$sql = 'SELECT P.product_id,P.ebay_id,P.upc,P.ebay_price,P.price_with_shipping,P.ebay_quantity,P.quantity_anc, P.sku AS psku,p.unallocated_quantity,P.location AS location_anc,P.quantity,P.location AS location_entrepot 
				FROM `oc_product` AS P
				
				
				where  (P.ebay_id !="0" and P.ebay_id is not null and P.ebay_id !="") and P.quantity<>P.ebay_quantity group by P.product_id ';//echo $sql.'<br><br>';
		...
		$req = mysqli_query($db,$sql);
			while($data = mysqli_fetch_assoc($req)){
					if(($data['quantity']+$data['unallocated_quantity'])<>$data['ebay_quantity']){						
						...
						revise_ebay_product($connectionapi,$data['marketplace_item_id'],$data['product_id'],$updquantity,$db,"non","bulk");
						$nbmaj++;
...
if(isset($_POST['check_ebay'])){
				...
				// correction ebay supprime

				$sql = 'SELECT * FROM `oc_product` WHERE `error_ebay` LIKE "%Plus dans ebay:%" and quantity >0 and (ebay_id ="0" or ebay_id is null or ebay_id ="")';
...
where   (ebay_id !="0" and ebay_id is not null and ebay_id !="") and ebay_last_check < "'.$dateverification.'"';
//Order by '.$_GET['order'].' '.$_GET['order2'].' '; //and P.ebay_id=292612778604';  $sql.'<br><br>';
// on envoie la requête
//echo $sql;
$req = mysqli_query($db,$sql);
...
//	$nbitem=20;
	...
	for($i=0; $i<=$nbitem-1;$i+=20){// and P.ebay_last_check < "'.$dateverification.'" 
			$sql = 'SELECT P.ebay_id 
					...
					where P.ebay_last_check < "'.$dateverification.'" and (P.ebay_id !="0" and P.ebay_id is not null and P.ebay_id !="")
...
//while($data = mysqli_fetch_assoc($req)){
	...
	//print("<pre>".print_r ($ebay_ids,true )."</pre>");
		if(count($ebay_ids)>0){
			...
			if($json['Ack']=='PartialFailure'){
				...
				foreach($ebay_id_received as $ebay_id){
					...
					if($ebay_id!=""){
						//echo "Plus dans ebay:";
						$sql2 = 'UPDATE `oc_product`SET ebay_last_check=NOW(),ebay_price=0,ebay_quantity=0,ebay_id=0,error_ebay="'.$json['Errors']['LongMessage'].': '.$ebay_id.'",ebay_last_check="2020-09-01" WHERE ebay_id="'.$ebay_id.'"';
					//	echo $sql2."<br>";
						$req2 = mysqli_query($db,$sql2);
					}
				}
			...
			foreach($json['Item'] as $item){
				...
				/*	if($today>$endtime_ver){
					...
					//	$sql2 = 'UPDATE `oc_product`SET ebay_last_check="'.$DateNOW.'",ebay_price="'.$item['CurrentPrice'].'",ebay_quantity="'.$qty_restant.'" WHERE `oc_product`.`ebay_id` ="'.$item['ItemID'].'"';
						$sql2 = 'UPDATE `oc_product`SET ebay_last_check=NOW(),ebay_price=0,ebay_quantity=0,ebay_id=0,error_ebay="Plus dans ebay: '.$item['ItemID'].'" WHERE ebay_id="'.$item['ItemID'].'"';
					}else{*/
						$sql2 = 'UPDATE `oc_product`SET ebay_last_check=NOW(),ebay_price="'.$item['CurrentPrice'].'",ebay_quantity="'.$qty_restant.'" WHERE `oc_product`.`ebay_id` ="'.$item['ItemID'].'"';
					//}
			//		$sql2 = 'UPDATE `oc_product`SET ebay_last_check="'.$DateNOW.'",ebay_price="'.$item['CurrentPrice'].'",ebay_quantity="'.$qty_restant.'" WHERE `oc_product`.`ebay_id` ="'.$item['ItemID'].'"';
				//	echo $sql2."<br>";
					$req2 = mysqli_query($db,$sql2);
...
// merge quantity
$itemcount=0;
$sql2 = 'SELECT count(*) nbitem FROM `oc_product`  
where   (ebay_id !="0" and ebay_id is not null and ebay_id !="") and ebay_last_check < "'.$dateverification.'"';
//Order by '.$_GET['order'].' '.$_GET['order2'].' '; //and P.ebay_id=292612778604';  $sql.'<br><br>';
// on envoie la requête
//echo $sql2;
...
//while($data = mysqli_fetch_assoc($req)){
	...
	echo "Nb de produits a verifier sur eBay:".$data2['nbitem'];
	//print("<pre>".print_r ($data,true )."</pre>");
?>
...
JsBarcode(".barcode").init();
						...
						<td <?if(($data['quantity']+$data['unallocated_quantity'])<>$data['quantity_anc']|| $data['quantity_anc']<0){?>style="background-color: red; color: white;"<?}?>>
						...
						<td <?if($data['quantity']>$data['quantity_anc']){?>style="background-color: yellow;"<?}elseif(strpos($data['location_entrepot'],"agasin") || $data['quantity']<0){?>style="background-color: red; color: white;"<?}?>>
						<?if($data['quantity']>$data['quantity_anc'] || strpos($data['location_entrepot'],"agasin")){?><input type="checkbox" name="maj[]" value="<?echo $data['product_id'];?>"/><?}?>
						<?echo $data['quantity'];?>
						</td>
						<?
						...
						?>
						<td <?if(($pos!==false || $pos2!==false) &&($data['unallocated_quantity']>0) || $data['unallocated_quantity']<0){?>style="background-color: red; color: white;"<?}?>>
