...
$db2 = mysqli_connect('127.0.0.1','n7f9655_n7f9655','jnthngrvs01$$','n7f9655_phoenixsupplies');
...
if (mysqli_num_rows($req) > 0) {
    while ($row = mysqli_fetch_assoc($req)) {
        ...
        if (count($matches) === 4) {
            ...
            $groupes1[$rangee][$tablette][$colonne] = array(
...
if (mysqli_num_rows($req2) > 0) {
    while ($row2 = mysqli_fetch_assoc($req2)) {
        // Extraire la rangée, la tablette et la colonne de la location (ex: A1-C)
        preg_match('/^([A-Za-z]+)(\d+)-([A-Za-z]+)$/', $row2['location'], $matches);
        ...
        if (count($matches) === 4) {
            ...
            $groupes2[$rangee][$tablette][$colonne] = array(
                'poids_total' => $row2['poids_total'],
                'volume_total_cubic_feet' => $row2['volume_total_cubic_feet']
            );
        } else {
            // Si la location contient un point-virgule, la diviser
            $locations = explode(';', $row2['location']);
            ...
            foreach ($locations as $location) {
                ...
                if (count($matches) === 4) {
                    ...
                    $groupes2[$rangee][$tablette][$colonne] = array(
                        'poids_total' => $row2['poids_total'] / count($locations),
                        'volume_total_cubic_feet' => $row2['volume_total_cubic_feet'] / count($locations)
                    );
                }
            }
        }
...
$groupes=fusionnerTableaux($groupes1, $groupes2);
//print("<pre>".print_r ($groupes,true )."</pre>");
...
{
    foreach ($tableau2 as $cle2 => $valeur2) {
        if (isset($tableau1[$cle2])) {
            // Si la clé existe déjà dans le premier tableau, fusionnez les valeurs
            $tableau1[$cle2] = fusionnerTableaux($tableau1[$cle2], $valeur2);
        } else {
            // Sinon, ajoutez la paire clé-valeur au premier tableau
            $tableau1[$cle2] = $valeur2;
        }
    }
