...
//echo "allo";
       ...
       //print("<pre>".print_r ($json,true )."</pre>");
         ...
         //  echo $sql2."<br>";
           $req2 = mysqli_query($db,$sql2);
           $result=revise_ebay_product($connectionapi,$json['ItemID'],$product_id,$_POST['quantity'][$product_id],$db,"oui","oui");
...
//	echo "list";
	$sql4 = 'select al.manufacturer_id,p.upc,p.product_id,p.quantity,al.brand,p.sku,
		al.weight ,al.length ,al.width,
		al.height ,p.condition_id,pd.condition_supp,p.remarque_interne,
		al.color,al.model ,al.image,al.name ,al.price,al.ebay_id 
		from oc_product_description pd  
		left join oc_product p on (p.product_id=pd.product_id) 
		left join oc_algopix al on (al.upc=p.upc)
		left join oc_product_to_category pc on (pc.product_id=p.product_id and (pc.category_id=139973 or pc.category_id=617))
		where al.image!="" and al.name!="" and al.name!="" and al.verif_fait =9 and pd.language_id=1 and p.quantity>0 and p.ebay_id=0 order by p.product_id limit 25'; 
//echo $sql4."<br>";
...
while($product = mysqli_fetch_assoc($req4)){
		...
		if($product['condition_id']!=9){
            ...
            $product['condition_supp']="Comes from a former rental store,Digital Code Expired";
        }
        ...
        //if(($product['price_with_shipping']-3.49)>0){
            $sql2 = "UPDATE `oc_product` SET `model`='".strtoupper( $product['model'] )."'";
            $sql2 .=",`verif_fait`=2,`manufacturer_id` = '0". $product['manufacturer_id']."'";
            $sql2 .=$ebay_id.", `weight`='". $product['weight'] ."',`height`='". $product['height'] ."'";
            $sql2 .=", `width`='". $product['width'] ."',`length`='". $product['length']."'" ;
            $sql2 .=",`price`=".( $product['price']-3.49).",`price_with_shipping`=".( $product['price_with_shipping']);
            $sql2 .=" ,ebay_last_check='2020-09-01'  WHERE product_id='".$product_id."'";
        //  echo $sql2."<br>";
            $req2 = mysqli_query($db,$sql2);
            $sql2 = "UPDATE `oc_product_description` SET `color`='".strtoupper( $product['color'] )."'";
            $sql2 .=", `name` = '".addslashes(strtoupper( $product['name'] ))."', `condition_supp` ='". $product['condition_supp']."'";
            $sql2 .=" WHERE language_id=1 and product_id='".$product_id."'";
           // echo $sql2."<br>";
            $req2 = mysqli_query($db,$sql2);
...
//print("<pre>".print_r ($result_ebay,true )."</pre>"); 
			if(isset($result_ebay['pricevariant'])){
				//print("<pre>".print_r ($result_ebay,true )."</pre>"); 
				$pricevariant=json_encode($result_ebay['pricevariant']);
			}else{
				$pricevariant=0;
			}
			if(isset($_POST['brand_hidden'][$product_id]) && $_POST['brand_hidden'][$product_id]==""){
			...
			if(!isset($json['Item']['ShippingDetails']['ShippingServiceOptions']['ShippingServiceCost']) && isset($json['Item']['ShippingDetails']['ShippingServiceOptions'][0]['ShippingServiceCost']))
			...
			if (
				isset($json['Item']['ListingDetails']['ConvertedStartPrice']) &&
				isset($json['Item']['ShippingDetails']['ShippingServiceOptions']['ShippingServiceCost']) &&
				isset($_POST['algopix_price_hidden'][$product_id])
			) {
				$convertedStartPrice = $json['Item']['ListingDetails']['ConvertedStartPrice'];
				$shippingServiceCost = $json['Item']['ShippingDetails']['ShippingServiceOptions']['ShippingServiceCost'];
				$algopixPriceHidden = $_POST['algopix_price_hidden'][$product_id];
			
				if (($convertedStartPrice + $shippingServiceCost) > $algopixPriceHidden) {
					// echo "SurEBAY".($convertedStartPrice + $shippingServiceCost);
					// echo "Algopix".$algopixPriceHidden;
					$_POST['price'][$product_id] = $convertedStartPrice + $shippingServiceCost - $algopixPriceHidden;
				}
			}
			if (
				isset($result_ebay['name']) &&
				isset($_POST['name'][$product_id]) &&
				strlen($result_ebay['name']) > strlen($_POST['name'][$product_id]) &&
				isset($_POST['majname'][$product_id])
			) {
				$_POST['name'][$product_id] = $result_ebay['name'];
			}
			
			$_POST['ebayname'][$product_id]=isset($result_ebay['name'])?$result_ebay['name']:'';
		}
		...
		UPDATE `oc_algopix` 
		SET 
			`model` = '" . strtoupper(isset($_POST['model'][$product_id]) ? $_POST['model'][$product_id] : '') . "',
			`color` = '" . strtoupper(isset($_POST['color'][$product_id]) ? $_POST['color'][$product_id] : '') . "',
			`name` = '" . strtoupper(isset($_POST['name'][$product_id]) ? $_POST['name'][$product_id] : '') . "',
			`manufacturer_id` = '" . (isset($_POST['manufacturer_id'][$product_id]) ? $_POST['manufacturer_id'][$product_id] : '0') . "',
			`image` = '" . (isset($_POST['image'][$product_id]) ? $_POST['image'][$product_id] : '') . "',
			`brand` = '" . (isset($_POST['brand'][$product_id]) ? $_POST['brand'][$product_id] : '') . "',
			`ebay_id` = '" . $ebay_id . "',
			`upc` = '" . $upc . "',
			`weight` = '" . (isset($_POST['weight'][$product_id]) ? $_POST['weight'][$product_id] : '0') . "',
			`height` = '" . (isset($_POST['height'][$product_id]) ? $_POST['height'][$product_id] : '0') . "',
			`pricevariant` = '" . $pricevariant . "',
			`width` = '" . (isset($_POST['width'][$product_id]) ? $_POST['width'][$product_id] : '0') . "',
			`length` = '" . (isset($_POST['length'][$product_id]) ? $_POST['length'][$product_id] : '0') . "',
			`date_update` = NOW() 
		WHERE 
			`product_id` = '" . $product_id . "' OR `upc` = '" . $upc . "'
	";
		$req2 = mysqli_query($db,$sql2);
	}
}
...
<form action="bulklisting.php" method="post"  enctype="multipart/form-data">
...
<table style="text-align: left; width: 100%; margin-left: auto; margin-right: auto;" border="1" cellpadding="2" cellspacing="2">
	...
	<input type="button" onclick='UnSelectAll()' value="Unselect All"/>
        ...
        <?
$sqlproduct = 'select p.error_ebay,pc.category_id,al.pricevariant,p.product_id,p.upc,p.sku,p.price,p.price_with_shipping,p.quantity,p.condition_id,p.model,p.ebay_id_old,p.ebay_id,p.image image,pd.name nameen,pd2.name namefr, 
        pd.condition_supp condition_suppen,pd2.condition_supp condition_suppfr,
        pd.color coloren,pd2.color colorfr, m.name brand,m.manufacturer_id, al.brand brand_algopix
		from oc_product_description pd  
		left join oc_product p on (p.product_id=pd.product_id) 
        left join oc_algopix al on (p.upc=al.upc) 
        left join oc_manufacturer m on (p.manufacturer_id=m.manufacturer_id) 
        left join oc_product_description pd2 on (pd.product_id=pd2.product_id and pd2.language_id=2) 
		left join oc_product_to_category pc on (pc.product_id=p.product_id and (pc.category_id=139973 or pc.category_id=617))
		where p.`verif_fait`=2 and pd.language_id=1 and p.quantity>0 order by p.product_id '; 
//echo $sqlproduct."<br>";
//echo $ebayoutputnametab[$i]['CustomLabel']."cl<br>"; 
...
<label>SKU:</label> 
		...
		<th style="vertical-align:  middle; height: 50px; background-color: #030282; color: white; width: 200px;">
			<label>Quantity:</label>
					...
					<a href="https://www.ebay.com/sch/i.html?_nkw=<?echo $product['upc'];?>&LH_ItemCondition=4&_sop=15" target="google"><?echo $product['sku'];?></a> 	
					</td>
...
<th style="vertical-align:  middle; height: 0px; background-color: #030282; color: white; width: 200px;">
    Ebay clone: 
            ...
            <td bgcolor="<?if($product['image']!=""){echo $bgcolor;}else{echo "red";}?>" id="champ3<?echo $product['product_id'];?>">
                    ...
                    <? 
								if(isset($product['pricevariant']) && $product['pricevariant']!=0 ){
									//print("<pre>".print_r ($product,true )."</pre>"); 
									$productvariants=json_decode($product['pricevariant']);
								//	echo $productvariants;		
								}else{
...
//echo "Pricelow:".$pricelowest;
					...
					<input type="hidden" name="price_hidden[<?php echo $product['product_id']; ?>]" value="<?php echo isset($product['algopix_price']) ? $product['algopix_price'] : 0; ?>" />
					...
					<?if($product['marketplace_item_id']>0){?>
						<a href="https://www.ebay.com//lstng?mode=ReviseItem&itemId=<?echo $product['marketplace_item_id'];?>&sr=wn&ReturnURL=https://www.ebay.com/sh/lst/active?offset=0" target='ebay' ><?echo $product['marketplace_item_id'];?></a>
...
<label>Extra Info:</label>		 </th>
					...
					<td></td>
                    <td bgcolor="<?echo $bgcolor;?>">
					<input id="length"  type="text" name="length[<?echo $product['product_id'];?>]" value="7" size="3" />
X					<input id="width"  type="text" name="width[<?echo $product['product_id'];?>]" value="5" size="3" />
X					<input id="height"  type="text" name="height[<?echo $product['product_id'];?>]" value="1" size="3" />
					</td>
                    <td bgcolor="<?echo $bgcolor;?>">
					<input id="weight"  type="text" name="weight[<?echo $product['product_id'];?>]" value=".25" size="5" />lbs
					</td>
                    <td bgcolor="<?echo $bgcolor;?>">
