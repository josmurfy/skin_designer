    ...
    if ((isset($_POST['new_loc']) && $_POST['new_loc']!=""))
    {
		foreach($_POST['product_select'] as $product_select)  
           ...
           //     $new_loc= str_replace('-','',$_POST['new_loc']);
			...
			//	echo $_POST['unallocated_quantity_hid'.$sku].'<br>';
				
				$sql2 = 'UPDATE `oc_product`SET unallocated_quantity=unallocated_quantity-'.$_POST['quantity_val'.$sku].',quantity=quantity+'.$_POST['quantity_val'.$sku].',location="'.strtoupper($new_loc).'",anc_loc="'.
				(isset ($_POST['anc_loc'])?strtoupper($_POST['anc_loc']):'').'" WHERE product_id ='.$product_select;
        		//echo $sql2.'<br><br>';
			//	//print("<pre>".print_r (value: '34:trf_inventaire_bulk' )."</pre>");
			//	//print("<pre>".print_r ($req2,true )."</pre>");
...
<form id="form_67341" class="appnitro" action="trf_inventaire_bulk.php" method="post" enctype="multipart/form-data">
NOUVELLE LOCATION <input id="new_loc"  type="text" name="new_loc" value="" size="30" />
<br>
...
<?
		$sql = 'SELECT *, PD.name AS nameen, (P.quantity + P.unallocated_quantity) AS quantity_total
        FROM `oc_product` AS P
        LEFT JOIN `oc_product_description` AS PD ON P.product_id = PD.product_id
        WHERE PD.language_id = 1 
          AND P.unallocated_quantity <> 0 
    
          AND P.ebay_id > 0
        ORDER BY name';  //    AND P.quantity <> 0 
...
// merge quantity
$itemcount=0;
//print("<pre>".print_r (value: '190:trf_inventaire_bulk' )."</pre>");
//print("<pre>".print_r ($req,true )."</pre>");
//print("<pre>".print_r ($sql,true )."</pre>");

	while($data = mysqli_fetch_assoc($req)){
		
		//if((($data['quantity']+$data['unallocated_quantity'])<>$data['ebay_quantity'])||($data['quantity']>$data['quantity_total']&&$data['location']=="" || strpos($data['location'],"agasin") ||strpos($data['location'],"MAGASIN"))){
	?>
...
<div  style="text-align: center;font-size: 15px; "><strong><?echo (string)$data['sku'] ;?></strong></div>
						...
						<td >
						<?echo $data['quantity_total'];?>
						<input id="qtytot<?echo $data['product_id'];?>"  type="hidden" name="qtytot<?echo $data['product_id'];?>"  value="<?echo $data['quantity_total'];?>" />
						</td>
                        <td >
						...
						<input id="unallocated_quantity_hid<?echo $data['sku'];?>" type="hidden" name="unallocated_quantity_hid<?echo $data['sku'];?>" value="<?echo $data['unallocated_quantity'];?>"/>


						</td>
						<td style="vertical-align: middle; background-color: <?if($data['location']!=""){echo "red";}else{echo $bgcolor;}?>;  text-align: center;">
						<?echo $data['location'];?>
						</td>	
						<td style="vertical-align: middle; background-color: <?echo $bgcolor;?>;  text-align: center;">
...
<script>
	...
	document.getElementById('entrepot_val'+rel).autofocus = true
     ...
     function checked_sku(sku) {
        ...
        // code that we will 'try' to run
			...
			qteactuel=parseInt(document.getElementById('unallocated_quantity_hid' + item).value);
	...
	function editEntrepot_val(sku,entrepot_val) {
		qteactuel=parseInt(document.getElementById('unallocated_quantity_hid' + sku).value);
