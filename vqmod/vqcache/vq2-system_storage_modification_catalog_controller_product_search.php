...
// Clear Thinking: Smart Search
				...
				if ($this->config->get($prefix . 'smartsearch_status')) {
					$ignore = array(
						'_route_',
						'route',
						'filter_name',
					...
					foreach ($this->request->get as $key => $value) {
						if (in_array($key, $ignore) || in_array('filter_' . $key, $ignore)) {
							continue;
						}
						if (is_array($value)) {
...
// Clear Thinking: Smart Search
			...
			$results = $this->model_catalog_product->getProducts($filter_data);
					...
					$smartsearch_results = $this->model_extension_module_smartsearch->smartsearch($filter_data);
					$product_total = count($smartsearch_results);
					$results = $this->model_extension_module_smartsearch->getProducts($smartsearch_results, $filter_data);
					
					if ($this->config->get($prefix . 'smartsearch_single_redirect') && $product_total == 1) {
						$only_product = array_pop($results);
						$product_url = $this->url->link('product/product', 'product_id=' . $only_product['product_id'] . $url);
			...
			foreach ($results as $result) {
				if ($result['image']) {
					...
					$soldout = $this->model_tool_image->resize('sold_out.png', $this->config->get($this->config->get('config_theme') . '_image_product_width'), $this->config->get($this->config->get('config_theme') . '_image_product_height'));
